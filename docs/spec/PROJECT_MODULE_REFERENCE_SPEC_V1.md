...
- `Class_Initialize` runs as part of instance initialization after storage is
  allocated and before the creation operation exposes or returns the new
  instance. Its ordering and failure behavior are relative to that creation
  operation, not unconditionally to a procedure named `Main`.
- `Class_Terminate` runs at the VBA-defined lifetime transition for the
  instance. It is not defined merely as “after Main”; normal return, error
  unwinding, project reset/disposal, reentrancy and reference cycles require
