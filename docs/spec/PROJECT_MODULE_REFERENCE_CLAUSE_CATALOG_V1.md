...
| `PMR-GEN-002` | global | MUST-level source constraints produce explicit diagnostics, never silent fallback. | implemented-partial | `compile_withevents_declaration_surfaces_project_model_diagnostic`; `compile_implements_directive_surfaces_project_model_diagnostic`; `compile_raiseevent_statement_surfaces_project_model_diagnostic`; `compile_project_preserves_withevents_diagnostic_gate`; `compile_project_preserves_raiseevent_diagnostic_gate`; `module_unit_rejects_malformed_attribute_line`; `compile_project_rejects_option_private_for_non_procedural_module` |
...
| `PMR-CLS-001` | class | `WithEvents` declarations are prohibited in procedural module declaration lists (`CONF-...-0056`). | implemented-partial | `resolve_withevents_declaration_emits_pmr_diagnostic` |
...
| `PMR-CLS-003` | class | Implements directive cannot occur in extension modules (`CONF-...-0095`). | implemented-partial | `resolve_implements_directive_emits_pmr_diagnostic` |
...
| `PMR-CLS-005` | class | Implements requires implemented-name declarations for all public interface members (`CONF-...-0097/0098`). | specified-pending | interface-contract tests (planned) |
| `PMR-CLS-006` | class | Implements method name prefix binding is enforced (`CONF-...-0143`). | specified-pending | implemented-name prefix tests (planned) |
| `PMR-CLS-007` | class | `RaiseEvent` statements are valid only inside class modules and declared events (`CONF-...-0176/0177`). | implemented-partial | `resolve_raiseevent_statement_emits_pmr_diagnostic` |
