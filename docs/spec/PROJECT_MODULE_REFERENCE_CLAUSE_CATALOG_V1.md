...
Date: 2026-03-02
Applies to: ProjectGraph integration across `oxvba-symbol`, `oxvba-bind`, and `oxvba-host`
...
| `PMR-GEN-002` | global | MUST-level source constraints produce explicit diagnostics, never silent fallback. | implemented-partial | `scanner_rejects_withevents_in_standard_modules`; `withevents_in_procedural_module_should_be_rejected`; `scanner_rejects_implements_in_standard_modules`; `implements_missing_member_is_bind_error`; `raise_event_outside_class_module_is_bind_error`; `raise_event_undeclared_event_is_bind_error`; `withevents_raise_event_routes_to_handler` |
...
| `PMR-CLS-003` | class | Implements directive cannot occur in extension modules (`CONF-...-0095`). | implemented-verified | `scanner_rejects_implements_in_standard_modules` |
...
| `PMR-CLS-005` | class | Implements requires implemented-name declarations for all public interface members (`CONF-...-0097/0098`). | implemented-verified | `implements_missing_member_is_bind_error`; `cross_project_call_const_enum_new_method_and_typeof`; `implements_dispatch_through_interface_var` |
| `PMR-CLS-006` | class | Implements method name prefix binding is enforced (`CONF-...-0143`). | implemented-partial | `implements_missing_member_is_bind_error`; `implements_dispatch_through_interface_var`; `implements_property_through_interface_var`; `implements_indexed_property_through_interface_var` |
| `PMR-CLS-007` | class | `RaiseEvent` statements are valid only inside class modules and declared events (`CONF-...-0176/0177`). | implemented-verified | `raise_event_outside_class_module_is_bind_error`; `raise_event_undeclared_event_is_bind_error`; `withevents_raise_event_routes_to_handler`; `raise_event_prefers_event_over_same_named_property` |
