...
| `HAL-DYN-005` | `VARIANT` byref legality checks | marshaling lane includes Automation types | byref discriminants obey `VT_BYREF` matrix constraints | illegal discriminant combinations -> deterministic adapter fault | specified-pending | `HAL_DECLARE_MARSHAL_CONFORMANCE_V1.md` (Lane C) |
| `HAL-DYN-006` | `SAFEARRAY` element-type legality checks | marshaling lane includes arrays across boundary | element-type compatibility enforced per in-scope matrix | illegal array element shape -> deterministic adapter fault | specified-pending | `HAL_DECLARE_MARSHAL_CONFORMANCE_V1.md` (Lane C) |
| `HAL-DYN-007` | pointer-string metadata/encoding requirements (`LPSTR`/`LPWSTR`) | native ABI lane enabled | string arguments include explicit semantics (encoding + length/termination model) | missing/invalid metadata -> deterministic compile-time or adapter error by mode | specified-pending | `HAL_DECLARE_MARSHAL_CONFORMANCE_V1.md` (Lane C) |
...
| `HAL-DYN-018` | pointer-string marshaling semantics (`LPSTR`/`LPWSTR`) require explicit encoding + length/termination policy | native string-pointer marshaling lane enabled | deterministic boundary conversion and diagnostics | ambiguous string semantics rejected deterministically | implemented-partial | conformance tracks deferred pointer-string lane explicitly; native ABI string lane pending |
| `HAL-DYN-019` | ByRef marshaling writeback semantics are deterministic and side-effect bounded | byref lane enabled | post-call writeback follows explicit contract | invalid writeback shape -> deterministic adapter/runtime failure | implemented-partial | conformance tracks deferred writeback lane explicitly; byref native lane pending |
