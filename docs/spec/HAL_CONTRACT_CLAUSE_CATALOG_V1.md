...
| `HAL-FS-003` | `seek(handle, position)` | valid handle; 1-based position (`position >= 1`) | updates the mapped cursor; byte-positioned EOF is reached at `len + 1`; bare seek does not extend logical length | invalid handle or nonpositive position -> `AdapterFault` | implemented-verified | `file_open_seek_eof_lof_close_roundtrip`; `seek_negative_returns_adapter_fault`; `seek_zero_returns_adapter_fault`; `prop_seek_eof_boundary` |
...
| `HAL-FS-008` | `write_bytes(handle, data)` | valid handle, mutation allowed | writes current `Write#`-style formatted field output with record delimiter at position, returns bytes written | policy denied / invalid handle | implemented-verified | conformance probe `fs.write_bytes` |
...
| Clause ID | Operation | Preconditions | Postconditions | Failure obligations | Status | Verification |
|---|---|---|---|---|---|---|
| `HAL-COM-001` | `create_object(prog_id)` | capability supported; COM activation allowed | deterministic token floor always available; Windows host-backed native lane may return bound object-handle token | unsupported/policy-denied | implemented-verified | conformance `com.create_object`; host mode tests; `process_com_dynlink_policy_denials_are_enforced` |
| `HAL-COM-002` | `dispatch_invoke_v2(request)` (with legacy scalar shim retained) | capability supported; COM activation allowed | deterministic token floor always available; Windows host-backed native lane may invoke constrained `IDispatch` subset with variadic argument vectors and property intent | unsupported/policy-denied | implemented-verified | `dispatch_invoke_deterministic_projection_contract`; policy test via `process_com_dynlink_policy_denials_are_enforced`; host multi-arg COM tests |
...
| `HAL-COM-007` | `describe_object(object)` returns stable identity/capability metadata for tracked COM bindings without exposing adapter internals. | capability supported; COM activation allowed | known objects return descriptor with transport kind, progid, and known member/event token sets; unknown objects return `None` | unsupported/policy-denied preserve stable contract | implemented-verified | adapter and host descriptor tests |
| `HAL-COM-008` | Typelib resolve/load/invalidate operations are part of `ComHal`, not a separate peer HAL trait. | capability supported; COM activation allowed | deterministic known-identity resolution, metadata loading, and cache invalidation remain available through COM boundary | unsupported/policy-denied and adapter-fault preserve stable contract | implemented-verified | HAL conformance `typelib.resolve_typelib_reference`; adapter metadata tests |
| `HAL-COM-005` | C2 late-bound client path requires explicit ProgID-text activation contract (`CreateObject`) alongside tokenized activation floor. | Windows COM lane planning active | activation selector semantics and deterministic failures are explicitly specified before implementation | unsupported/malformed selector/policy-denied must be deterministic | specified-pending | `docs/spec/COM_CLIENT_SERVER_SCOPE_V1.md`; `docs/spec/COM_CLIENT_SERVER_CONFORMANCE_V1.md` |
| `HAL-COM-006` | C2 late-bound invoke path requires explicit member-name resolution plus deterministic `VarResult`/`ExcepInfo`/`ArgErr` translation contract. | Windows COM lane planning active | invoke packing and error translation semantics are explicitly specified before implementation | unsupported invoke shapes and native invoke failures must map deterministically | specified-pending | `docs/spec/COM_CLIENT_SERVER_SCOPE_V1.md`; `docs/spec/COM_CLIENT_SERVER_CONFORMANCE_V1.md` |
