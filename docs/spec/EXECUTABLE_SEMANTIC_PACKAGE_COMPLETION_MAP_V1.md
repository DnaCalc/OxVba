...
| Procedure signatures | full `ProcedureSignatureDescriptor` | `ProcedureRuntimeMetadata::procedure_signature_descriptor`; `ParameterDescriptor`; `ProcedureSignature` and `VbaTypeDescriptor` in `project.rs`; `ProjectDynamicMemberRoute`; `VmExecutionPackage::procedure_signature_descriptors` | First package signature descriptor view exists for procedure kind, parameter order/slot/type, parsed ByRef/ByVal mode, Optional/default integer literals, ParamArray marker, return type/slot, and property group. Source mechanism vs omitted default, missing-optional Variant state, implicit `Me`, full property value ByVal runtime semantics, canonical descriptor ids/digests, and call-behavior evidence remain incomplete. | `metadata-missing` | compiler-project / compiler-emit / VM evidence | Capture source/resolved mechanisms, missing optional/default state, ParamArray element shape, `Me`, property value ByVal semantics, signature digests, and call comparison evidence. |
...
| VMR-03 | Signature descriptor surface | `ProcedureSignatureDescriptor`, `ParameterDescriptor`, return descriptor | metadata/evidence first | First metadata view is loaded through compiler package and VM package setup; remaining VMR-03 work must complete full call-relevant signature facts and VM evidence comparing current call behavior against signature metadata. |
