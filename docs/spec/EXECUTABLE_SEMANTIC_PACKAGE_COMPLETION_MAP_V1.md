...
| Slot declared type metadata | `SlotTypeDescriptor` | `ProcedureRuntimeSlotMetadata`; `ProcedureRuntimeSlotKind`; `ProcedureRuntimeMetadata.param_types`; compiler resolver declaration maps; `VmExecutionPackage::slot_type_descriptors` | First `SlotTypeDescriptor` view exists for current procedure metadata. It projects slot/name/role plus known parameter and return declared types, initial states, and carrier hints. Local declared types, temporaries, compiler-generated slots, descriptor ids, descriptor digests, and VM evidence rows remain incomplete or explicitly `Unknown`. | `metadata-missing` | compiler-emit / VM evidence | Populate descriptor rows for locals, temporaries, and compiler-generated slots; add descriptor digest/evidence without changing VM slot storage. |
...
| VMR-02 | Slot descriptor surface | `SlotTypeDescriptor`, declared type ids, roles, initial states, carrier hints | metadata loading plus snapshots | VM can load and expose descriptors for parameters, locals, return slots, and temporaries while still executing existing slots. First view exists for parameters and return slots; locals/temporaries remain the next population and evidence steps. |
