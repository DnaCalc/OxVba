...
| Arrays | `ArrayShapeDescriptor` / bounds / storage kind | `crates/oxvba-runtime/src/safe_array.rs`; `RuntimeArrayElementType`; resolver `BoundArrayDescriptor`; `ProcedureRuntimeMetadata::array_shapes`; `UdtTypeDescriptor` fixed array field bounds; `OxBundle` v10; VM array instructions; `VmPackageIdentityEvidence::array_shape_evidence`; `conformance/vm_package/identity_seed/vmr05_array_shape_bounds.bas`; `conformance/vm_package/identity_seed/vmr05_udt_descriptor_members.bas` | Package array-shape rows now record array name, base slot, rank, declared bounds where known, `Option Base`, element type/carrier, and storage kind (`StaticFixed`, `Dynamic`, `ParamArrayPack`). VM evidence asserts fixed/static descriptor bounds, explicit `0 To 2` bounds, dynamic `ReDim 2 To 4` runtime SAFEARRAY bounds, unallocated fixed-array base slots, and ByRef scalar observations in a VM-runnable seed. UDT field descriptors now record fixed-array field bounds and flattened field-alias slots. Multi-rank fixtures, fixed-array `LBound`/`UBound` runtime introspection, bounds-error evidence, lifecycle cleanup, and COM/native SAFEARRAY projection descriptors remain incomplete. | `metadata-missing`; `VM-limitation`; `test-shortcoming` | runtime / compiler-resolve / VM evidence | Add multi-rank and bounds-error fixtures, then decide whether fixed-array bound introspection is implemented in the VM or remains descriptor-only for package/JIT consumption. |
| UDTs | nominal type, fields, copy/init/cleanup | resolver/project UDT metadata; `ProcedureRuntimeMetadata::udt_types`; `UdtTypeDescriptor`; VM UDT field/copy paths; type-system draft; `VmPackageIdentityEvidence::udt_descriptor_evidence`; `conformance/vm_package/identity_seed` | UDT behavior and compiler metadata are partial. VM-runnable seed fixtures now record the current flattened base-slot plus field-alias descriptor shape and first nominal `UdtTypeDescriptor` rows, including descriptor ids, instances, field order, primitive/string/nested carriers, fixed string lengths, fixed array field bounds, field aliases, fieldwise-copy classification, and first cleanup ownership flags. Descriptor-backed field offsets/layout, branch/error/deopt cleanup maps, and descriptor-driven VM field/copy execution are still missing. | `metadata-missing`; `test-shortcoming` | compiler-resolve / VM / runtime | Add offset/layout and lifecycle cleanup evidence, then route selected UDT field/copy behavior through descriptors only after VM fixtures prove parity. |
| Objects/classes/interfaces | object/class/interface descriptors | `ObjectRef`; `RuntimeClassDescriptor`; `RuntimeInterfaceDescriptor`; `ProcedureRuntimeMetadata::object_types`; `ObjectTypeDescriptor`; project dynamic object routes; COM `WithEvents` routes; COM typelib projection; `VmPackageIdentityEvidence::object_descriptor_evidence`; `conformance/vm_package/identity_seed/vmr05_object_descriptor_identity.bas` | Generic `Object` slot descriptors now record `Nothing` initial state, `ObjectRef` carrier, no activation/event binding, and per-slot instance evidence. VM runtime project evidence records VM-capable source-project class route identities, implemented interface aliases, and imported COM `WithEvents` route identities when compiled project route tables are supplied. Descriptor-driven `As New` activation, default instances, class/interface declared slot ids, imported COM class/interface descriptors, object cleanup/lifetime maps, and object/member binding consumption remain incomplete. | `metadata-missing`; `test-shortcoming` | runtime-object / compiler-project / COM | Fill descriptor coverage for `As New`, default instances, imported COM classes/interfaces, object lifetime, and object/member binding before behavior-driving consumption. |
...
| Evidence schema | VM/JIT/package evidence | `scripts/run-jit-v2-tracer-fixtures.ps1`; `crates/oxvba-host/tests/jit_v2_tracer_vm_seed.rs`; `conformance/vm_package/identity_seed`; VM snapshot helpers; `crates/oxvba-host/src/engine.rs`; `VmPackageIdentityEvidence` | VM seed fixtures, retained snapshots, package/procedure/bytecode identity evidence, host package-identity snapshot/session evidence, first slot descriptor evidence with per-procedure digests, descriptor-token assertions for primitive/String/Variant/current-UDT-field/array/object slots, signature/call observations, call-site descriptor evidence, array-shape descriptor evidence, UDT descriptor evidence, and first object descriptor evidence exist. Lifecycle, interop boundary observations, host-policy observations, and broader expression-call/object-member descriptor coverage are not yet emitted. | `test-shortcoming` | VM evidence / conformance | Record lifecycle observations, boundary observations, host-policy diagnostics, and remaining expression-call/object-member descriptor coverage in VM evidence. |
...
| TB02 UDT struct field/copy | `vm-ready` | UDT descriptor id; nominal type identity; field order, offsets, and carriers; whole-copy semantics; field cleanup/deopt maps; descriptor evidence | `metadata-missing`; `test-shortcoming` | VM source and projected field snapshots run. Package seed fixtures now record nominal UDT descriptor ids, instances, field order/carriers, nested UDT references, fixed string lengths, fixed array field bounds, field-alias slots, fieldwise-copy classification, and first cleanup ownership flags. Field offsets/layout, cleanup/deopt maps, and descriptor-driven whole-copy/field execution remain open. |
...
| VMR-04 | Expression and call descriptor seeds | `CallSiteDescriptor`, `ArgumentBindingDescriptor`, expression category descriptors | metadata/evidence first | Call-site descriptor rows and VM evidence now cover top-level project call lowering for ByRef alias/writeback, ByRef expression temp, ByVal copy with a declared-`Double` coercion gap, Optional default, Optional `Variant` missing-policy metadata, empty/non-empty ParamArray, named fixed arguments with positional ParamArray packs, property value ByVal classification, return copyout, and compile-diagnostic evidence for the current named/arity/ParamArray rejection set. The current call-entry coercion, Optional-missing, property/default-member, COM/native, and export gaps are classified in the VMR-04 ledger and routed to later VMR-05/VMR-06/semantic-table/JIT-readiness work before any behavior-changing call consumption. |
| VMR-05 | Array, UDT, object descriptor seeds | `ArrayShapeDescriptor`, `UdtTypeDescriptor`, `ObjectTypeDescriptor`, object/class/interface route descriptors | metadata/evidence first | Array shape evidence now captures fixed/static and dynamic local array descriptors plus dynamic runtime SAFEARRAY bounds where current VM can execute. UDT evidence now captures nominal type descriptors, nested/fixed-member facts, field aliases, and first cleanup ownership flags. Object evidence now captures generic `Object` slot identity plus source-project class/interface route identities and imported COM `WithEvents` route identities for current VM-capable route tables. Remaining VMR-05 slices must add broader array/bounds-error evidence, UDT offset/layout/lifecycle evidence, `As New`/default-instance/imported-COM object descriptors, and explicit gap classifications before behavior-driving consumption. |
