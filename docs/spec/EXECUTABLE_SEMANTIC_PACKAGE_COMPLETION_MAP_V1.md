...
| Call sites | `CallSiteDescriptor` / `ArgumentBindingDescriptor` | compiler call lowering; `ProcedureRuntimeMetadata::call_sites`; `VmExecutionPackage::call_site_descriptors`; `ProjectDynamicParamRoute`; VM call binding | Seed package call-site rows exist for top-level project procedure/default-member fallback calls, including target kind, named/positional/omitted/ParamArray source shape, ByRef alias/writeback, ByVal copy, Optional default, ParamArray pack, fixed-array materialization, default-member fallback policy, and return copyout. Expression call-site rows, external Declare/COM coverage, canonical descriptor ids, and descriptor-driven VM call binding remain incomplete. | `metadata-missing` | compiler-emit / VM call binding | Expand coverage to expression calls, external/COM calls, and default-member policy details, then route selected VM call binding through package facts under VMR-06. |
...
| VMR-04 | Expression and call descriptor seeds | `CallSiteDescriptor`, `ArgumentBindingDescriptor`, expression category descriptors | metadata/evidence first | First call-site descriptor rows exist for top-level project call lowering; remaining VMR-04 work must extend expression-call coverage and classify gaps before behavior changes. |
