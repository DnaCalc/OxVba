...
|---|---|---|---|---|---|
| `VMR01_TYPED_FUNCTION`; `VMR04_CALL_ARGUMENT_BINDING` ByRef literal/expression arguments | `CallSiteDescriptor` records `binding=byrefexpressiontemp` and `no-writeback-temp`; signature evidence records `gap:byref-copyback-not-observed` for the corresponding ByRef parameter. | The compiler lowers the argument value into the callee parameter slot and emits no caller writeback. | `oracle-required`; `test-shortcoming` for the remaining source syntax variants. This is not treated as a VM bug unless the spec/oracle says the source form should reject or route differently. | Add MS-VBAL/Office-backed fixtures for ByRef variable, literal, parenthesized expression, function/property result, and type-mismatch forms, then decide whether each form is a legal temporary, diagnostic, or exact alias case. | `bd-iave.6.4`; VMR-06 only after the oracle decision. |
