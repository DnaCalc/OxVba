...
- [x] ODG-040 — Host extension modules. Still open, but the model boundary is enforced and the HAL/runtime callback-backed project catalog/reference/mutation path now exists; remaining work is the live Excel/VBIDE callback provider plus oracle capture.
...
- [ ] INTP-013 — Host extension module lifecycle. Blocked on the remaining ODG-040 live Excel/VBIDE callback-provider and oracle-capture work.
