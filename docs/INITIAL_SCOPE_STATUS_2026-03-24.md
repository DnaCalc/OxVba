...
- [ ] ODG-043 — Startup/entrypoint. In progress. Loaded `.basproj` projects now honor configured `<EntryPoint>Module.Procedure</EntryPoint>`, unconfigured `OutputType=Exe` projects now resolve a unique top-level executable mainline before the unique `Sub Main` fallback, and direct single-file execution now supports a bounded top-level mainline path with later helper `Sub` declarations. The CLI/stdIO host lane also has a dedicated cross-platform `ConsoleIo` HAL path for plain `Print`, `Input`, and `Line Input`, with `Debug.Print` kept on diagnostics/stderr. The gate remains open pending startup oracle coverage across the remaining project configurations and deeper top-level declaration/module-scope semantics.
...
- [ ] INTP-012 — Startup metadata / auto-entrypoint. In progress under ODG-043; configured `.basproj` entrypoints, unique top-level mainline fallback, and unique `Sub Main` fallback now execute through the loaded-project path, and direct single-file execution now has a bounded top-level mainline path. The CLI stdIO host also exposes a dedicated console HAL lane for plain `Print` / `Input` / `Line Input`, but startup oracle breadth and deeper top-level declaration fidelity remain outstanding.
