...
- [x] ODG-041 — TypeLib/importlib resolution. Correctly deferred beyond the now-proved baseline, unresolved-identity, clean two-reference order, clean three-reference order, mixed broken-selected-reference, qualified-valid, qualified broken-first, and unqualified-valid-with-later-broken subsets. The baseline `.basproj` foldback lane is oracle-evidenced with `com_testeventserver_basproj_oracle_20260326T181500Z`, the unresolved identity lane is oracle-evidenced with `com_testeventserver_unresolved_typelib_oracle_20260326T174738Z`, the two-reference order lane is oracle-evidenced with `com_testeventserver_reference_order_oracle_20260326T171500Z`, the three-reference order lane is oracle-evidenced with `com_testeventserver_three_reference_order_oracle_20260327T060926Z`, the mixed broken-reference lane is oracle-evidenced with `com_testeventserver_mixed_broken_reference_oracle_20260327T034413Z`, the explicit qualified-valid lane is oracle-evidenced with `com_testeventserver_qualified_broken_reference_oracle_20260327T040256Z`, the qualified broken-first lane is oracle-evidenced with `com_testeventserver_qualified_broken_first_reference_oracle_20260327T052111Z`, and the unqualified later-broken lane is oracle-evidenced with `com_testeventserver_unqualified_broken_later_oracle_20260327T050754Z`. Together these prove that loaded `.basproj` file-backed references match Excel for baseline `New TestEventServer : Ping() = 42`, unknown `LIBID` / missing `.tlb` identity failure classification, first-reference wins across two registered typelibs (`42` / `84`), first-reference wins across three registered typelibs (`42` / `84` / `126`), coherent fail/fail behavior when the selected reference is the broken filesystem-backed importlib, continued success when a later saved reference is broken but code explicitly targets the still-valid qualified typelib (`42` / `84`), continued success when an earlier saved reference is broken but code explicitly targets the later still-valid qualified typelib (`84` / `42`), and continued success when an earlier saved reference remains valid and unqualified `New TestEventServer` still binds to that first valid typelib (`42` / `84`). Harness-side popup handling is treated as oracle quality-of-life support for bounded Excel automation, not a user-facing parity target. Remaining open work is broader broken/ambiguous multi-reference breadth beyond these proved clean-order and bounded broken-reference subsets, not popup-shape emulation.
