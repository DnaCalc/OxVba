...
- [x] ODG-032 — Stateful file I/O. Correctly deferred. The supported host-backed subset is now oracle-evidenced through `Output` / `Print` / `Close` / `Line Input`, `Write#` / `Input#` quoted-string roundtrip, `Write#` / `Input#` multi-field typed roundtrip (`42|True|hello,world`), and `EOF` / `LOF` / `Seek` on the `Input` path (`file_io_oracle_20260326T220420Z`); broader file parity remains open.
...
- [x] ODG-041 — TypeLib/importlib resolution. Correctly deferred beyond the now-proved baseline, unresolved-identity, clean two-reference order, clean three-reference order, mixed broken-selected-reference, widened three-reference mixed broken-first, qualified-valid, qualified broken-first, unqualified-valid-with-later-broken, and widened three-reference unqualified-valid-with-middle-broken subsets. The baseline `.basproj` foldback lane is oracle-evidenced with `com_testeventserver_basproj_oracle_20260326T181500Z`, the unresolved identity lane is oracle-evidenced with `com_testeventserver_unresolved_typelib_oracle_20260326T174738Z`, the two-reference order lane is oracle-evidenced with `com_testeventserver_reference_order_oracle_20260326T171500Z`, the three-reference order lane is oracle-evidenced with `com_testeventserver_three_reference_order_oracle_20260327T060926Z`, the mixed broken-reference lane is oracle-evidenced with `com_testeventserver_mixed_broken_reference_oracle_20260327T034413Z`, the widened three-reference mixed broken lane is oracle-evidenced with `com_testeventserver_three_reference_mixed_broken_oracle_20260327T062044Z`, the explicit qualified-valid lane is oracle-evidenced with `com_testeventserver_qualified_broken_reference_oracle_20260327T040256Z`, the qualified broken-first lane is oracle-evidenced with `com_testeventserver_qualified_broken_first_reference_oracle_20260327T052111Z`, the unqualified later-broken lane is oracle-evidenced with `com_testeventserver_unqualified_broken_later_oracle_20260327T050754Z`, and the widened three-reference unqualified later-broken lane is oracle-evidenced with `com_testeventserver_three_reference_unqualified_broken_later_oracle_20260327T063542Z`. Together these prove that loaded `.basproj` file-backed references match Excel for baseline `New TestEventServer : Ping() = 42`, unknown `LIBID` / missing `.tlb` identity failure classification, first-reference wins across two registered typelibs (`42` / `84`), first-reference wins across three registered typelibs (`42` / `84` / `126`), coherent fail/fail behavior when the selected reference is the broken filesystem-backed importlib in both two-reference and widened three-reference matrices, continued success when a later saved reference is broken but code explicitly targets the still-valid qualified typelib (`42` / `84`), continued success when an earlier saved reference is broken but code explicitly targets the later still-valid qualified typelib (`84` / `42`), continued success when an earlier saved reference remains valid and unqualified `New TestEventServer` still binds to that first valid typelib (`42` / `84`), and the same unqualified first-reference-wins success when a middle saved same-name reference is broken in a three-reference matrix (`42` / `126`). Harness-side popup handling is treated as oracle quality-of-life support for bounded Excel automation and coarse failure classification, not a user-facing parity target. Remaining open work is broader broken/ambiguous multi-reference breadth beyond these proved clean-order and bounded broken-reference subsets, including the newly evidenced mismatch `com_testeventserver_three_reference_qualified_broken_first_oracle_20260327T064416Z` where Excel surfaces `Can't find project or library` but OxVba still binds the later valid qualified target.
- [x] ODG-042 — MS-OVBA storage roundtrip. Correctly deferred.
- [ ] ODG-043 — Startup/entrypoint. In progress. Loaded `.basproj` projects now honor configured `<EntryPoint>Module.Procedure</EntryPoint>`, unconfigured `OutputType=Exe` projects now resolve a unique top-level executable mainline before the unique `Sub Main` fallback, direct single-file execution supports a bounded top-level mainline path with later helper `Sub` / `Function` declarations including visibility-prefixed procedure headers, and both the loaded-project rewrite and direct single-file compiler extractor now preserve `Option ...` directives, `Def*` default-type directives, module `#Const` directives, module-scope variable declarations (`Dim`, `Public`, `Private`, `Global`, `Static`), and declaration-only module forms (`Const`, `Declare`, scoped `Type`/`Enum`) as non-mainline content instead of sweeping them into the synthetic startup proc. The current documented rule is now loader-enforced: top-level mainlines are allowed for program/script lanes and rejected for `Library`, `Addin`, `ComServer`, and `ComExe`, while `Addin` no longer invents an artificial `EntryPoint` requirement when no startup mainline exists. `oxvba run-project <dir>` now follows the documented discovery rule: prefer a unique discovered `.basproj`, else a unique discovered `.vbp`, else auto-load all `.bas`/`.cls` files as an executable convention project, with deterministic ambiguity failure when multiple candidate project files of the selected kind are present, and `oxvba build` now follows that same discovery/adapter path instead of remaining `.basproj`-only. Convention-mode execution is now directly covered as an execution lane, with auto-discovered modules stamped with filename-derived `VB_Name` metadata so the compiler path matches the real loaded-module contract instead of only the lighter discovery path. `oxvba run-project <file.vbp>` is now also a real VBP-S0 execution lane through the adapter, and the current strict VBP-S0 lane now supports `Startup="Sub Main"` via the fallback ladder, explicit `Startup="Module.Procedure"`, ordered `Reference=*\G...` type-library references, and ordered `Reference=*\A...` project references that target `.vbp` / `.basproj` artifacts while rejecting unsupported startup-object forms, known deferred designer keys (`Form`, `UserControl`, `PropertyPage`), and other unknown `.vbp` keys with stable `VBP-E-UNSUPPORTED-*` diagnostics instead of silently inventing invalid entrypoints or ignoring metadata it does not implement. The covered matrix now also includes actual `.basproj` explicit-entrypoint execution, CLI `run-project --entry Module.Procedure` override execution, convention-mode directory execution for both `Sub Main` and unique top-level mainline, actual `.vbp` load/run checks for explicit `Module.Procedure`, `Sub Main` fallback, unique top-level mainline, referenced `.vbp` project graphs, directory `.vbp` discovery, deterministic directory-discovery ambiguity handling, build-path `.vbp` parity, legacy type mapping (`OleDll`, `Control`, `OleExe`), non-`Exe` rejection of top-level executable mainlines, and a standalone project/hosting example suite covering direct-file, convention, `.basproj`, `.vbp`, and library lanes. The CLI/stdIO host lane also has a dedicated cross-platform `ConsoleIo` HAL path for plain `Print`, `Input`, and `Line Input`, with `Debug.Print` kept on diagnostics/stderr. Startup-object/forms/designer-backed startup remain intentionally unsupported in the current strict VBP-S0 lane. The gate remains open pending startup oracle coverage across the remaining supported project configurations and deeper cross-procedure module-scope shared-state semantics for the top-level extension.
...
- [ ] INTP-012 — Startup metadata / auto-entrypoint. In progress under ODG-043; configured `.basproj` entrypoints, unique top-level mainline fallback, and unique `Sub Main` fallback now execute through the loaded-project path; direct single-file execution now has a bounded top-level mainline path; `run-project` now supports convention-mode directories without `.basproj` and direct `.vbp` adapter execution; both top-level lowering paths now preserve `Option ...` directives, `Def*` default-type directives, module `#Const` directives, module-scope variable declarations, and declaration-only module forms as non-mainline content; and the current strict VBP-S0 lane now treats `Startup="Sub Main"`, explicit `Startup="Module.Procedure"`, ordered `Reference=*\G...` type-library references, and ordered `Reference=*\A...` project references deterministically while failing known deferred designer/startup-object surfaces and other unsupported `.vbp` keys with stable `VBP-E-UNSUPPORTED-*` diagnostics instead of emitting invalid entrypoints or dropping metadata silently. The current rule is now loader-enforced: top-level mainlines are allowed for program/script lanes and rejected for `Library`, `Addin`, `ComServer`, and `ComExe`, and `Addin` no longer requires an explicit `EntryPoint` in the absence of a mainline. Covered checks now include actual `.basproj`, convention-mode, and `.vbp` execution/load scenarios across the supported startup ladder, referenced `.vbp` project graphs, legacy type mapping, non-`Exe` top-level rejection, and a standalone project/hosting example suite. The CLI stdIO host also exposes a dedicated console HAL lane for plain `Print` / `Input` / `Line Input`, but startup oracle breadth and deeper top-level cross-procedure module-state fidelity remain outstanding; startup-object/forms/designer-backed startup remain intentionally unsupported in the current strict VBP-S0 lane.
