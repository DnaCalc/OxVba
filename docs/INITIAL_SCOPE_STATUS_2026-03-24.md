...
- [x] ODG-041 — TypeLib/importlib resolution. Correctly deferred beyond the baseline file-backed `.tlb` oracle subset. The baseline `.basproj` foldback lane is now oracle-evidenced with `com_testeventserver_basproj_oracle_20260326T181500Z`, proving that a loaded `.basproj` file-backed typelib reference executes the same `New TestEventServer : Ping() = 42` baseline as Excel. The broader unresolved/ambiguous identity and reference-order matrix remains open.
