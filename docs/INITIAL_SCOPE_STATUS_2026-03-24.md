...
- [x] ODG-040 — Host extension modules. Still open, but the model boundary is enforced, the `PMR-MOD-007` target-name floor now exists in the host project model, and a first ignored live Excel/VBIDE callback-backed test is present; remaining work is paired oracle capture plus broader conflict evidence.
...
- [ ] INTP-013 — Host extension module lifecycle. Blocked on the remaining ODG-040 paired oracle-capture and conflict-matrix work.
