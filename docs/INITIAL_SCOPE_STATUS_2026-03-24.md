...
- [x] ODG-041 — TypeLib/importlib resolution. Correctly deferred beyond the now-proved baseline and reference-order subsets. The baseline `.basproj` foldback lane is oracle-evidenced with `com_testeventserver_basproj_oracle_20260326T181500Z`, and the two-reference order lane is now oracle-evidenced with `com_testeventserver_reference_order_oracle_20260326T171500Z`, proving that loaded `.basproj` file-backed references match Excel both for the baseline `New TestEventServer : Ping() = 42` case and for first-reference wins across two registered typelibs (`42` / `84`). Loaded `.basproj` execution now also preserves unresolved LIBID identity as `PMR-E-TYPELIB-LIBID-UNRESOLVED` instead of degrading into late name-resolution failure. Remaining open work is the broader unresolved/missing importlib identity oracle matrix, not lack of foldback, reference-order, or integrated diagnostic propagation.
