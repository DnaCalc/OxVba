...
- [x] ODG-041 — TypeLib/importlib resolution. Correctly deferred beyond the now-proved baseline, unresolved-identity, reference-order, mixed broken-selected-reference, and qualified-valid subsets. The baseline `.basproj` foldback lane is oracle-evidenced with `com_testeventserver_basproj_oracle_20260326T181500Z`, the unresolved identity lane is oracle-evidenced with `com_testeventserver_unresolved_typelib_oracle_20260326T174738Z`, the two-reference order lane is oracle-evidenced with `com_testeventserver_reference_order_oracle_20260326T171500Z`, the mixed broken-reference lane is oracle-evidenced with `com_testeventserver_mixed_broken_reference_oracle_20260327T034413Z`, and the explicit qualified-valid lane is oracle-evidenced with `com_testeventserver_qualified_broken_reference_oracle_20260327T040256Z`. Together these prove that loaded `.basproj` file-backed references match Excel for baseline `New TestEventServer : Ping() = 42`, unknown `LIBID` / missing `.tlb` identity failure classification, first-reference wins across two registered typelibs (`42` / `84`), coherent fail/fail behavior when the selected reference is the broken filesystem-backed importlib, and continued success when a later saved reference is broken but code explicitly targets the still-valid qualified typelib (`42` / `84`). Harness-side popup handling is treated as oracle quality-of-life support for bounded Excel automation, not a user-facing parity target. Remaining open work is wider multi-reference breadth beyond these proved subsets, not popup-shape emulation.
