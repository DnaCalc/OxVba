...
|---|----|-----|-----|-----------|
|◐|vm3-dynamic-array-access-on (bead **bd-us4v**, blocked-by bd-4ktq)|High/Perf|Reading `arr(i)` of a module-level **dynamic** array inside a loop is **O(N)** in array length → array loops are **O(N²)** (100-elem loop=130ms, 400-elem=2.2s; native ~1µs). Isolated to the element read (ScanArray vs ScanPlain). From OxForms — `docs/handovers/HANDOVER_OxVba_vm3_dynamic_array_access_perf.md` (commit 9e3dbd6a)|**PARTIAL (still OPEN).** Round 1 (019a39df) fixed **slot-held** arrays (Global/Local/Temp): O(1) raw-descriptor access via `SafeArray::raw_safearray_variant_element`/`raw_safearray_bounds_len` → `Variant::safearray_element`/`safearray_bounds_len` → vm3 `array_get_fast`/`array_set_fast` (borrow slot, read/write ONE element). Module-level now flat ~13µs/elem. **RESIDUAL (confirmed 2026-06-30): class-instance-field arrays (`Private mX()` in `.cls` — the OxForms shape) are STILL O(N)** (270→486→961→1920 µs/elem at N=250→2000): the field-held array bypasses the slot fast path and deep-clones on the field-load each iteration. Diagnostic `array_perf_diagnose.rs`. REVISIT scheduled. Fix dir: make `obj.field(i)`/`Me.field(i)` element access O(1) (extend fast paths to field-of-object/record receivers, reuse the descriptor-borrow primitives); check UDT-field arrays too|
