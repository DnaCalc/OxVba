...
|☐|statement-call-paren-not-byval|High/SilentWrong|M|`Foo(x)` statement call doesn't force ByVal|Oracle fixture/evidence added in `bd-4ktq.10.1`: Excel leaves caller unchanged for statement-form `Inc (x)`, while vm3 still writes back. Owning fix bead: `bd-4ktq.10.2`; carry paren-arg flag parser→`bind_one_arg`.|
|☐|byref-type-mismatch-accepted|High/SilentWrong|M|`ByRef` type mismatch silently accepted+retypes caller|Oracle fixture/evidence added in `bd-4ktq.10.1`: Excel compile error `ByRef argument type mismatch`, selected line `TakeLong x`, token `x`. Owning fix bead: `bd-4ktq.10.3`; bind-time reject when ByRef param type ≠ l-value type.|
|☐|no-call-arity-validation|High/SilentWrong|S|extra args dropped; missing→late wrong error|Oracle fixture/evidence added in `bd-4ktq.10.1`: extra arg => `Wrong number of arguments or invalid property assignment` on `TakeOne 1, 2`; missing required arg => `Argument not optional` on `TakeTwo 1`. Owning fix bead: `bd-4ktq.10.4`; arity check in `bind_proc_args`.|
