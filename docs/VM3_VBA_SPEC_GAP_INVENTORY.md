...
~~hms-round-crosses-boundary~~ *(DONE: bead `bd-4ktq.39.3`; seconds rounding at the end of a day now carries into the displayed date instead of wrapping time to midnight on the old date; coverage: `date_time_residuals_vm3` and `oxvba-runtime vba_date`.)* · array-byval-accepted-lost · array-assign-into-fixed-lhs ·
...
on-goto-out-of-range-no-5 · exit-do-in-while-accepted · ~~numeric-string-parse-rust-f64~~ *(DONE: bead `bd-4ktq.39.4`; implicit arithmetic string coercion now shares the VBA full numeric-string parser with explicit conversions, accepting VBA radix/numeric forms and rejecting Rust-only `NaN`/`inf`; coverage: `numeric_string_coercion_residuals_vm3` plus existing `val`/conversion checks.)* ·
leftb-rightb-byte-fns-absent · sendkeys-appactivate-absent · debug-assert-no-break ·
cc-constants-hardwired-64bit · ~~sgn-nan-double~~ *(DONE: bead `bd-4ktq.39.4`; `Sgn` now rejects a host-supplied `Double` NaN with error 5 instead of treating it as zero; source-level NaN creation is blocked by the exponentiation fix; coverage: oxvba-lib unit test.)* · option-explicit-awareness ·
~~strconv-byte-modes-passthrough~~ *(DONE: bead `bd-4ktq.39.4`; `StrConv(..., vbFromUnicode)` now returns a typed Byte SAFEARRAY through the shared ANSI codec, and `StrConv(byteArray, vbUnicode)` decodes Byte arrays back to strings; coverage: `numeric_string_coercion_residuals_vm3`.)* · shell-blocks-until-exit · resolve-library-import-dead-guard
