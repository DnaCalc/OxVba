...
~~date-string-parser-inconsistent(M)~~ *(DONE: bead `bd-4ktq.39.3`; shared `CDate`/`TimeValue` string handling now rejects invalid time strings, accepts AM/PM time strings, and shares combined date-time parsing including trailing AM/PM; coverage: `date_time_residuals_vm3` plus existing `date_conversion_vm3`.)* · cstar-null-error-13-not-94(S) · pow-negative-base-fractional-nan(S) ·
