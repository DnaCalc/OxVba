...
|◐|integer-literal-surfaces-as-long|High/SilentWrong|M|IN PROGRESS (bead `bd-4ktq.11`): truth surface captured in `docs/evidence/conformance/vm3_integer_literal_oracle_20260701T1200Z/` and fixture added in `crates/oxvba-differential/tests/integer_literal_carrier_vm3.rs`. Oracle says unsuffixed decimal `7`/`32767` and `7%` are `2:Integer`, `32768`/`2147483647`/`7&` are `3:Long`, unsuffixed `2147483648` is `5:Double`, `7^` is `20:LongLong`, Integer-width radix `&HFFFF`/`&O177777` are `2:Integer:-1`, Long-width radix and `&`-suffixed radix are Long, `&HFFFFFFFFFFFFFFFF^` is `20:LongLong:-1`, and unsuffixed radix beyond Long width is a syntax error. Active vm3 baselines cover the currently correct Long-width subset; ignored assertions track the remaining carrier/syntax work for `bd-4ktq.11.2`.|CoreConst/OxConst typed literal carrier; parser/radix width diagnostics; vm3 `const_variant`|
