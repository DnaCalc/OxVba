...
|◐|integer-literal-surfaces-as-long|High/SilentWrong|M|IN PROGRESS (bead `bd-4ktq.11`): truth surface captured in `docs/evidence/conformance/vm3_integer_literal_oracle_20260701T1200Z/`. Direct literal carrier implementation landed in `bd-4ktq.11.2`: decimal/radix Integer-width literals now surface as `2:Integer`; decimal Long-width and explicit `&` literals surface as Long; explicit `^` literals surface as LongLong; unsuffixed decimal beyond Long is Double; unsuffixed radix beyond Long width is rejected. The oracle-backed fixture `crates/oxvba-differential/tests/integer_literal_carrier_vm3.rs` now has all 8 rows active and passing. Remaining work is the folded constant/default audit in `bd-4ktq.11.3` to confirm no constant-table/default path still erases the carrier.|CoreConst/OxConst typed literal carrier; parser/radix width diagnostics; vm3 `const_variant`; const/default audit|
