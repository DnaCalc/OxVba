...
|---|----|-----|-----|-----------|
|☑|vm3-dynamic-array-access-on (bead **bd-us4v**, blocked-by bd-4ktq)|High/Perf|Reading `arr(i)` of a module-level **dynamic** array inside a loop is **O(N)** in array length → array loops are **O(N²)** (100-elem loop=130ms, 400-elem=2.2s; native ~1µs). Isolated to the element read (ScanArray vs ScanPlain). From OxForms — `docs/handovers/HANDOVER_OxVba_vm3_dynamic_array_access_perf.md` (commit 9e3dbd6a)|**DONE.** Round 1 (019a39df) fixed **slot-held** arrays (Global/Local/Temp): O(1) raw-descriptor access via `SafeArray::raw_safearray_variant_element`/`raw_safearray_bounds_len` → `Variant::safearray_element`/`safearray_bounds_len` → vm3 `array_get_fast`/`array_set_fast` (borrow slot, read/write ONE element). Round 2 (2026-06-30) closed the **field-held** residual: `obj.field(i)`/`Me.field(i)`/`rec.items(i)` lowered to a `FieldGet`/`RecordGet` that deep-cloned the WHOLE field array per iteration. Fix fuses the field-load+index into `ArrayGetField`/`ArraySetField` (object fields, borrow via new `ObjectRef::with_project_field_ref`/`_mut`) and `ArrayGetRecordField`/`ArraySetRecordField` (UDT `Variant`-kind members, borrow via new `VbaRecord::field_variant_ref`/`_mut`), reading/writing ONE element through the descriptor with no whole-array clone; the in-place write persists (object reference / record value semantics) so no field write-back. Both rows now flat (~11µs/elem at N=250..1000). Guard `class_field_array_access_perf_vm3.rs` (class-field + UDT-field, sub-second ceiling + correct sum); diagnostic `array_perf_diagnose.rs`. (Inline fixed-array UDT members + nested compound bases keep the cloning path — bounded, not the unbounded pathology.)|
