...
| `HAL-U-013` | Project catalog boundary | Host-project discovery and project-kind classification rules vary by host and are implementation-defined in MS-VBAL. | Keep language-level ProjectGraph normative and require explicit HAL capability contracts (`ProjectCatalog`/`ProjectReferenceProvider`) for host-supplied graph materialization. | High for project/module/reference compatibility claims and host portability. | PMR HAL integration phase | Open |
