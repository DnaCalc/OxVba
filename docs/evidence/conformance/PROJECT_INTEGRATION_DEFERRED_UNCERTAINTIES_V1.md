...
| Host project extension module lifecycle (open host project model) | Deferred/in-progress | `ODG-040`, `CCT-042`, `INTP-013` | Project-model extension legality is enforced and the HAL/runtime callback-backed project catalog/reference/mutation path now exists; remaining work is a live Excel/VBIDE callback provider plus oracle-backed host behavior capture. |
