...
| Stateful file statement parity (`Open`, `Input#`, `Print#`, `Write#`) | Deferred | `ODG-032`, `CCT-033`, `INTP-014` | A supported host-backed statement subset now exists and is oracle-evidenced for `Output` / `Print` / `Close` / `Line Input` plus `EOF` / `LOF` / `Seek` on the `Input` path, but broader stateful file semantics remain HAL-adjacent and oracle-sensitive. |
