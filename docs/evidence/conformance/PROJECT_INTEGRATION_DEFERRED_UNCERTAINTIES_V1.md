...
| Host project extension module lifecycle (open host project model) | Deferred/in-progress | `ODG-040`, `CCT-042`, `INTP-013` | Project-model extension legality is enforced, the `PMR-MOD-007` target-name floor now exists in the host project model, and a first ignored live Excel/VBIDE callback-backed test is present; remaining work is paired oracle-backed host behavior capture plus broader conflict evidence. |
