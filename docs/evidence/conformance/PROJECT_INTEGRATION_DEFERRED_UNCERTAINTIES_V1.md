...
|---|---|---|---|
| Project startup metadata and entrypoint selection beyond `Sub Main` | In progress | `ODG-043`, `CCT-045`, `INTP-012` | Loaded `.basproj` projects now honor configured `<EntryPoint>Module.Procedure</EntryPoint>` and unique `Sub Main` fallback for `OutputType=Exe`, but startup semantics are still not fully evidenced across the broader project-configuration oracle matrix. |
