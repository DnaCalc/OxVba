...
| Full COM/type-library runtime parity in integrated project lane | Deferred/partial | `ODG-041`, `CCT-043` | Deterministic scaffolding exists, the baseline file-backed `.tlb` oracle lane is proved, and the baseline `.basproj` foldback lane is now oracle-evidenced; broader unresolved/ambiguous importlib behavior, reference-order behavior, and full bridge breadth remain open. |
