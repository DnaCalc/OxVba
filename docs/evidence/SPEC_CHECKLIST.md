...
| `[~]` | Class/project event model | `WithEvents`/`Implements`/`RaiseEvent` legality + runtime dispatch semantics | PMR compiler tests + `DIV-0003`/`DIV-0004` | Compile-time legality and interface coverage diagnostics are implemented; full runtime event subscription/dispatch ordering remains deferred. |
