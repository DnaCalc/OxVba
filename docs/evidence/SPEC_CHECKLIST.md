...
| `[~]` | Error object | Full `Err` object surface | `stdlib_error_err_raise_resume.bas`, `err_clear_basic.bas` | Current subset includes `Err.Number`, `Err.Raise`, and `Err.Clear`; full object surface still pending. |
