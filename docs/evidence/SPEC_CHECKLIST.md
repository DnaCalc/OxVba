...
| `[~]` | Financial expansion | `NPV`, `IRR`, `MIRR`, `Rate`, `NPer`, and related suite | `stdlib_random_financial_expansion.bas` | Algorithmic subset now active for `NPV`/`IRR`/`MIRR`; `Rate`/`NPer` and full parity/tolerance model remain pending. |
