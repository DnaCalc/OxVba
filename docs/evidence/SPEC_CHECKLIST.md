...
| `[~]` | Financial expansion | `NPV`, `IRR`, `MIRR`, `Rate`, `NPer`, and related suite | `stdlib_random_financial_expansion.bas`, `financial_tolerance_non_convergence.bas` | Algorithmic subset active for `NPV`/`IRR`/`MIRR`/`Rate`/`NPer` with deterministic solver-failure error tags; full host parity remains pending. |
