...
| DG-V87-001 | v87 | v87-kani | dg-fail | 2026-03-03T19:51:44Z | v92 | `remote/linux-runner` | Remote lane completed `fail` (`selected_count=4`, `failures=2`, `timeouts=1`, `cache_hits=6`) at `2026-03-04T02:00:59Z`; dominant failure class is timeout/resource pressure (`FO-V2-001`, `FO-V4-001`). |
| DG-V88-001 | v88 | v88-kani | dg-fail | 2026-03-03T19:51:44Z | v92 | `remote/linux-runner` | Remote lane completed `fail` (`selected_count=4`, `failures=2`, `timeouts=1`) at `2026-03-04T02:00:58Z`; lane evidence includes explicit CBMC OOM notice for comparator harness. |
| DG-V89-001 | v89 | v89-kani | dg-running | 2026-03-01T20:23:46Z | v92 | `remote/linux-runner` | Remote lane remains running in retry round (`FO-V2-001`, extended timeout path); live progress and liveness are tracked via `run-formal-kani-remote.ps1 -Action Status|Tail`. |
...
| DG-V146-001 | v146 | v146-kani | dg-fail | 2026-03-01T04:43:56Z | v146 | `remote/linux-runner` | Remote lane completed `fail` (`selected_count=4`, `failures=2`, `timeouts=2`) at `2026-03-01T10:44:09Z`; failures are timeout/resource-pressure dominated. |
...
| DG-V287-001 | v287 | v287-kani | dg-fail | n/a | v290 | `remote/linux-folded` | Foldback from pinned remote lane `v287-kani-pinned-fast` (job `20260303T201117Z_manual-v287-kani-pinned-fast`) produced `selected_count=3`, with `FO-V287-001/002` timeout at 600s and `FO-V287-003` pass. Bridge-retirement gate remains closed pending host harness timeout remediation. Evidence: `docs/evidence/language/PMR_DG_V287_FOLDBACK_2026-03-03.md`. |
