...
- Procedure-call syntax, multiline and single-line If/ElseIf syntax, front-checked Do While syntax,
  basic Exit and non-label error-control statements, identifier/numeric-label `GoTo`, `GoSub` /
  `Return`, and simple single-value Select Case syntax are no longer themselves route blockers. The
  call/coercion
