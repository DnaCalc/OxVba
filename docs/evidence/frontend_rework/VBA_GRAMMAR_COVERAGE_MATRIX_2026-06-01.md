...
`frontend_grammar_matrix_route_audit` maps all 110 grammar productions, including all 44 currently
anchored matrix productions, to current HIR-production findings from
...
route-proof overlay for the matrix.
...
- FE-9.7 follow-up:
  - `cargo test -p oxvba-compiler frontend_grammar_matrix_route_audit --quiet`: passed for all 110
    grammar productions, including all 44 currently anchored productions.
