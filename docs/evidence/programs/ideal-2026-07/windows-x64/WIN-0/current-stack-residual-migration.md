...
`scripts/test-windows-current-stack-residuals.ps1` supplies a positive case and ten mutations. The mutations prove rejection of a missing row, capability-state advancement, JIT credit sourced only from VM3/historical material, historical evidence credited as current code or a current test, a support-only capability owner, a removed imported route, an unresolved anchor, a hidden ByRef event blocker, and a closed imported callback route.
