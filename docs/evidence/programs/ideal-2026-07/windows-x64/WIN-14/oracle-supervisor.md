...
- Every operation requires a guardian arm acknowledgement and a strictly sequenced heartbeat after the invocation completes. Evidence flush fails if the heartbeat does not span the invocation, including the ready/benign-then-hang shape. On controlled shutdown, the guardian itself appends a final state containing its exact PID/start/name/path, stop-file observation, still-live Excel identity, and exit reason; any non-controlled exit is nonzero.
...
The focused suite completed in approximately thirty-eight seconds. It parsed every supervisor script and exercised:
...
- supervisor guardian mutations for worker-copy disagreement, wrong byte digest, missing guardian final state, final-state/helper identity mismatch, and a final timestamp after result generation;
- case-sensitive full-`Err` mutations independently separating returned JSON from persisted `runtime_err` and persisted `runtime_err` from the sealed descriptor, plus non-null success transport, forged pre-ownership runtime payload, future measurement timestamp, and unrelated bootstrap/module paths;
...
Independent review has proceeded in several repair rounds. The fourth fresh-context review found the missing supervisor guardian-ledger authority boundary, case-insensitive full-`Err` text comparisons, worker-authored artifact paths, unconstrained success transport, incomplete pre-ownership exclusion, silent failed-entry claim cleanup, and noncausal fixture timestamps. The current revision contains the coherent offline authority changes and exact adversarial regressions described above. It has not yet received the fifth independent fresh-context review, and no further Excel run is authorized before that review.
