...
- The supervisor seals the selected case sequence before worker launch. Each immutable descriptor binds case order/ID, module name and normalized source hash, expected compile/run statuses, nullable run procedure, diagnostic mode, evidence contract, expected value/token/line, and full expected `Err` state. The worker echoes the descriptor hash and bound fields, but a pure post-cleanup validator independently recomputes behavior and required guardian evidence. Worker-supplied case/aggregate `passed` Booleans have no authority. Complete cases require `observed_excel_pid == owned_excel_pid == ledger.pid`; versioned nullable compile-command/execution/context, runtime-measurement/`Err`, and evidence-status objects are schema/type checked. Process exit alone never bypasses ledger binding. The only partial-result disposition is exactly the first selected case failing with `harness-error`, no durable owned PID, nonempty transport, both ledgers exactly empty, and exit code 1 after successful Job cleanup.
- Exact Excel launch arguments and attachment decisions are separately testable pure functions. `ProcessStartInfo` construction produces exactly `['/x', bootstrap-path]`. Attachment adjudication rejects incomplete/foreign enumeration, visible owned blockers, non-`EXCEL7` candidates, failed/null native object retrieval, and returned `Application.Hwnd` PID mismatch. Worker Job creation and process start share one cleanup scope; injected start failure proves deterministic Job disposal before authority publication.
...
The focused suite completed in approximately eleven seconds. It parsed every supervisor script and exercised:
...
- Job containment for pre-ledger children, dispose-only closure, abrupt supervisor death, and injected process-start failure inside the Job/start cleanup scope;
...
- behavioral post-cleanup envelopes for complete success, fully owned behavioral failure, and five-case first-case transport, plus foreign worker/token, string Boolean, case schema/type, result/ledger order, aggregate, exit, partial-result, nonempty-ledger, missing/modified bootstrap, process-exit-only, observed/owned/ledger PID disagreement, compile-status-with-`passed=true`, malformed compile/evidence objects, and descriptor hash/procedure/status/source mutations;
...
Independent review has proceeded in several repair rounds. It identified exception-safe claim release, observed-entry adjudication, retained worker cleanup, borrowed-RCW ownership, partial-result reachability, process-exit bypass, durable ownership, bounded enumeration, bootstrap persistence/OPC closure, worker-trusted pass flags, ID-only case authority, PID disagreement, and launch/attachment/start cleanup testability. The current revision contains the corresponding offline repairs and adversarial tests described above. It has not yet received a fresh independent re-review, and no further Excel run is authorized before that review.
