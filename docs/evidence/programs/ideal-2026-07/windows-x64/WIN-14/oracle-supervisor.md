...
- The pure post-cleanup validator derives compile status from exact command ID/enabled state, execution exception, active project/module/code-pane/source snapshots, and guardian modal evidence. It derives run status from the descriptor-bound invocation/target, measured existence, AutomationSecurity/configuration, observed/runnable entry, `Err`, and modal evidence. Worker status labels and case/aggregate `passed` Booleans have no authority and must agree with those derived outcomes. Complete cases require `observed_excel_pid == owned_excel_pid == ledger.pid`; versioned nullable compile-command/execution/context, runtime-measurement/`Err`, and evidence-status objects are schema/type checked. Process exit alone never bypasses ledger binding. The only partial-result disposition is exactly the first selected case failing with `harness-error`, no durable owned PID, nonempty transport, both ledgers exactly empty, and exit code 1 after successful Job cleanup.
...
The focused suite completed in approximately fourteen seconds. It parsed every supervisor script and exercised:
...
- behavioral post-cleanup envelopes for all five default descriptor outcomes, fully owned behavioral failure, and five-case first-case transport, plus foreign worker/token, string Boolean, case schema/type, result/ledger order, aggregate, exit, partial-result, nonempty-ledger, missing/modified bootstrap, process-exit-only, observed/owned/ledger PID disagreement, command-enabled/forced-exception compile contradictions, attacker runtime entry/false observation/configuration, malformed compile/evidence objects, and descriptor hash/procedure/status/source mutations;
...
Independent review has proceeded in several repair rounds. The latest round found that status labels could still contradict nested compile/runtime evidence, the worker rebuilt a parallel case catalog instead of consuming the supervisor's exact sealed sequence, and process-start failure testing did not exercise post-launch Job assignment/membership cleanup. The current revision contains the corresponding offline repairs and adversarial tests described above. It has not yet received the next independent re-review, and no further Excel run is authorized before that review.
