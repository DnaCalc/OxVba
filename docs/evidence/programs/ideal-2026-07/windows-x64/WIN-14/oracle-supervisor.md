...
- Runtime invocation and status derive without consulting `run_status`: the sealed descriptor, derived compile outcome, guardian run ledger, exact entry/target/configuration measurements, and exact outcome-specific observation must form one mutually exclusive shape. Return success excludes modal and `Err`; full-`Err` success binds the exact six-field returned JSON to the separately persisted typed `runtime_err`; ambiguous macro failure binds the sentinel suffix to a freshly derived missing-macro disposition and excludes `Err`; the diagnostic runtime-modal shape excludes value and `Err`. Only after derivation are worker `compile_status`, `run_status`, macro-disposition, per-case `passed`, and aggregate `passed` labels compared. Complete cases require `observed_excel_pid == owned_excel_pid == ledger.pid`; versioned nullable compile-command/execution/context, runtime-measurement/`Err`, and evidence-status objects are schema/type checked. Process exit alone never bypasses ledger binding. The only partial-result disposition is exactly the first selected case failing with `harness-error`, no durable owned PID, nonempty transport, both ledgers exactly empty, and exit code 1 after successful Job cleanup.
...
The focused suite completed in approximately fifteen seconds. It parsed every supervisor script and exercised:
...
- atomic equal-run rejection, distinct-run isolation, forced post-claim failure, injected marker-deletion failure with preserved primary context, and no-op deletion leaving a detected stale marker;
...
Independent review has proceeded in several repair rounds. The third repair review found remaining precedence, guardian-liveness, mutually exclusive runtime-shape, retained worker-identity, and run-claim cleanup gaps. The current revision contains the coherent offline authority changes and the exact adversarial regressions described above. It has not yet received the fourth independent fresh-context review, and no further Excel run is authorized before that review.
