...
The focused suite completed in approximately thirty seconds. It parsed every supervisor script and exercised:
...
- behavioral post-cleanup envelopes for all five default descriptor outcomes, fully owned behavioral failure, and five-case first-case transport, plus foreign worker/token/start-time/path, timestamp-order, string Boolean, case schema/type, result/ledger order, aggregate, exit, partial-result, nonempty-ledger, missing/modified bootstrap, process-exit-only, observed/owned/ledger PID disagreement, malformed compile/evidence objects, and descriptor hash/procedure/status/source mutations;
...
- case-sensitive full-`Err` mutations independently separating returned JSON from persisted `runtime_err` and persisted `runtime_err` from the sealed descriptor, plus non-null success transport, a valid evidence-only wrong-token case failure with synthesized transport, bootstrap-bearing pre-ownership attachment failure, forged pre-ownership compile/runtime payloads, timestamps outside both causal bounds, and unrelated bootstrap/module paths;
