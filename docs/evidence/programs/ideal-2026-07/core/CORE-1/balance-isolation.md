...
| side effects | Clean fixtures perform only in-memory carrier work and the protocol itself writes only its stdout report. The policy fixture selects host-policy error 5, but this protocol does not carry a HAL journal and therefore does not independently claim that no native side effect occurred. |
...
Every child is owned by the test harness, polled against a 30-second deadline,
killed and reaped on timeout/status failure, and drained concurrently through
separate stdout/stderr readers. Each stream retains at most 64 KiB while still
being drained to avoid pipe deadlock; overflow and all failures name the exact
fixture.
