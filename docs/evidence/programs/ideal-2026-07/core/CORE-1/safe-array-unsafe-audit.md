...
Status: targeted repair and audit complete; two public safety questions are
escalated as exact residuals.
...
The review did find one pre-existing allocation-contract defect outside the six
Clippy sites. `alloc_header` accepts a rank representable by `u16`, while the
public `dimensions()` projection narrows `c_dims` to `u8` and `Drop` uses that
narrowed value to reconstruct the owner allocation layout. Rank 256 therefore
wraps to zero and rank 257 can be deallocated with a layout different from the
one used to allocate it. The six repairs remain invariant documentation rather
than behavior changes, but the audit is not evidence that the wider SafeArray
implementation is defect-free. The exact layout and thread-trait residuals are
recorded below.
...
The dimension/layout defect is owned by P0 successor `bd-59co.2.7.3`. That bead
must establish the supported VBA/product rank contract, prevent narrowing in
destruction, cover hostile boundary ranks and prove descriptor/balance cleanup.
Any external COM SAFEARRAY rank/adoption compatibility question is explicitly
routed to WIN-2 rollout producer `bd-59co.3.3.1`.
...
escalated as `bd-59co.2.7.2` for a high-risk CORE-5/host-session decision and a
Variant/interop transit audit. Windows apartment attachment/reentry is related
to `bd-59co.3.3.1`; apartment/lifecycle evidence is related to
`bd-59co.3.14.1`. It must not be treated as resolved by the six Clippy repairs.
