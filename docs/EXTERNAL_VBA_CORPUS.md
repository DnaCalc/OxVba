...
| VW-21 | broad live-COM harness passing, residuals explicit | WebHelpers/WebRequest/WebResponse/WebClient/authenticator execution | The broadened no-shim harness now runs inside the VBA-Web core project and exercises host root calls, URL encode/decode/join helpers, format/media helpers, URL-encoded parsing, scalar JSON parsing, key-value dictionary helpers, Collection cloning/default-member indexing, Dictionary cloning, WebRequest URL segments/query/header/cookie/body-parameter mutation, WebRequest cloning, `CreateFromOptions` for `UrlSegments`, WebResponse update, WebClient URL/proxy fields, and Basic/Digest authenticator setup. This exposed general fixes for ByRef arguments supplied from object default-member index expressions, `TypeOf` on Variant-held COM objects, built-in Collection default-member metadata, class header `MultiUse` instancing, and enum declared-type normalization. | Residuals remain deliberately recorded instead of hidden in the passing lane: `ConvertToJson(Dictionary)`/`WebRequest.Body` exact JSON serialization still returns blank in this path; nested `ParseJson` object/array storage into `Scripting.Dictionary` still needs project-object/COM `PropertyPutRef` ownership; `ConvertToUrlEncoded(Dictionary)` and `WebResponse.ExtractHeaders` need separate loop/default-member/string-array scrutiny. The broad harness imports `Scripting` as a normal COM reference and only host-injects Excel `Application`; this is still an ignored live-COM proof, not deterministic CI. |
