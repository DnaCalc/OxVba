...
- **Riff**: upstream fetched into the ignored corpus area from
  ...
  and fixed the general bare standard-module `Property Let` binder gap. A separate native
  `Declare` probe now covers Riff-shaped scalar `VarPtr` memory copying through `memmove`,
  including declared-width scalar pins and writeback to the original VBA variable. The native
