...
| VW-01 | fixed | Module top-level classification | Loading VBA-Web as `OutputType=Library` reported `PROJ-E-TOP-LEVEL-MAINLINE-UNSUPPORTED` for `WebHelpers`, even though the relevant early lines are declarations, conditional declarations, module variables, public constants, enums, and types, not executable mainline code. | Continue testing loader/scanner boundaries for exported `.bas` modules with `Attribute VB_Name`, `#Const`, `#If` declaration blocks, `Declare`, `Type`, `Enum`, module variables, public object globals, and continued declaration lines. |
| VW-02 | fixed, audit remains | Colon statement separators and single-line `If` grammar | VBA-Web uses idioms such as `If Len(x) > 0 Then: x = x & "&"` and `If cache Is Nothing Then: Set cache = New Dictionary`. The parser treated a colon immediately after `Then` as the start of an unexpected statement. | Broaden grammar coverage for statement separator vs label delimiter, leading colon after `Then`, multiple inline statements, `Else` on the same logical line, empty inline segments, and interaction with `On Error`, `Exit`, `Set`, calls, and assignments. |
| VW-03 | fixed, audit remains | Multi-line `If`/inline `If` boundary around `Else` | Nested inline `If` statements inside multiline `If` bodies could steal the enclosing `Else` after the `Then:` separator fix. | Keep the `If` grammar audit active: branch body termination, nested `Select Case`, inline `If ... Then: ...` immediately before an enclosing `Else`, and preprocessed source offset reporting. |
...
| VW-05 | covered for this fixture, broader lane open | Conditional compilation breadth | VBA-Web has nested `#If Mac Then` / `#ElseIf VBA7 Then` / `#Else` declaration blocks and procedure-local conditional sections. The Windows/VBA7 fixture now compiles through active branches. | Add whole-project tests for conditional declarations, procedure-local `#If`, nested conditionals, `#Const`, and offset-preserving diagnostics. Avoid assuming binder-level unit tests cover project-loader behavior. |
| VW-06 | partial proof | COM/library object model dependencies | `New Dictionary` required Scripting typelib metadata to expose a library-level coclass. The fixture now imports `Scripting` and compiles through dictionary creation and use. Other late-bound COM creation sites are admitted syntactically but not yet a runtime compatibility claim. | Split proofing into hermetic lanes: pure JSON/URL helpers first, then late-bound COM creation, then `WithEvents`/async WinHTTP. Each lane needs explicit policy flags and fallback expectations. |
| VW-07 | fixed | Continued parameter-list parsing and keyword identifiers | VBA-Web declarations and procedures use continued parameter lists and parameter names such as `Name`. These surfaced parser/scanner assumptions about newline trivia inside parameter lists and keyword tokens as identifiers. | Continue auditing contexts where keywords are legal identifiers: parameter names, named arguments, fields, enum members, labels, and property names. Keep declaration-continuation tests separate from ordinary newline recovery. |
...
| VW-10 | fixed | `Mid$` assignment form | `Mid$(target, start, len) = replacement` is a statement form, not a normal function call or property assignment. | Add parity tests for `Mid`/`Mid$` with omitted length, bounds behavior, ByRef/writeback cases, and qualified `VBA.Mid$` spelling. |
| VW-11 | fixed | Dynamic `Err.Raise` numbers | VBA-Web raises stored or computed error numbers such as `Err.Raise Err.Number` and module constants, not only literals. | Add coverage for `Number:=`, `Source`, `Description`, and runtime type mismatch/error-range behavior. The current runtime preserves the error number path; richer `Err` metadata remains a follow-up. |
...
| VW-13 | scaffolded, not parity | Ambient host `Application` root | The ignored local fixture includes a tiny `Application` shim for `Application.Run` and `Application.OnTime` so the core library can compile without claiming Excel host identity or scheduling semantics. | Replace the shim with a real host-injected Excel `Application` lane when host object roots and runtime identity are ready. Do not report this fixture as full Excel-host compatibility. |
| VW-14 | smoke passing, broader lane open | Harnessed referenced-library execution | An ignored `VbaWebHarness.basproj` host project now references `VbaWebCore.basproj` and calls selected pure helper functions from `Sub Main`; `UrlEncode` with explicit optional arguments, `JoinUrl`, and `MethodToName(1)` currently self-check and exit successfully. This proved that the previous core fixture was only a library compile/admission proof, not an execution proof. | Keep a host-project harness for every external library proof. Harnesses should self-check expected values with `Err.Raise`, then graduate from pure helpers to object/COM/host lanes. |
| VW-15 | open | Referenced-project enum type exposure | The first harness used `WebHelpers.MethodToName(WebMethod.HttpPost)` and failed binding with unresolved `WebMethod`, even though `WebMethod` is a public enum in the referenced VBA-Web library. | Ensure referenced project surfaces expose enum type names as qualifiers, not only enum member constants. Test both `ReferencedEnum.Member` and project/module-qualified enum access across project boundaries. |
| VW-16 | open | Cross-project optional defaults at runtime | Calling `WebHelpers.UrlEncode("a b+c")` from the harness reached runtime and failed with error 13, `unsupported coercion from Error to Double`. The omitted optional parameters arrived as the Missing/Error sentinel, then `If SpaceAsPlus = True` tried to coerce that sentinel numerically. Passing explicit optional values avoids this specific failure. | Cross-project calls need the same optional-default behavior as same-project calls: explicit defaults, typed zero defaults, and Variant Missing only where VBA semantics require it. Surface metadata probably needs to carry default values or the VM needs a callee-side default-fill contract for extern calls. |
| VW-17 | open | Conversion functions and VBA radix-prefixed strings | `WebHelpers.UrlDecode("a%20b%2Bc", True, 0)` reaches `VBA.CInt("&H" & web_Temp)` and fails at runtime with error 13, `expected a numeric value`. VBA conversion functions accept hex/octal-prefixed numeric strings such as `&H20`; OxVBA currently treats that as non-numeric text. | Extend numeric string parsing used by `CInt`/`CLng`/related conversions to support VBA radix prefixes, signs where valid, whitespace, overflow behavior, and parity diagnostics. |
...
- **VBA-Web**: first pass gathered upstream at commit
  ...
  smoke covers selected pure helpers only; blocked adjacent cases are cataloged as VW-15/VW-16
  and later rows.
