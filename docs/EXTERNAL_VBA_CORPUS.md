...
| VW-06 | partial proof, COM lanes cataloged | COM/library object model dependencies | `New Dictionary` required Scripting typelib metadata to expose a library-level coclass. The fixture now imports `Scripting` and compiles through dictionary creation and use. Other late-bound COM creation sites are admitted syntactically but not yet a VBA-Web runtime compatibility claim. | Existing COM proof layers are now explicit: non-ignored HAL tests cover COM activation policy denial and a Windows Dictionary invoke-when-available lane; ignored `oxvba-host` COM matrix tests cover live Dictionary methods/properties/collections across late-bound, early `PreferVtable`, and early `DispatchOnly` legs. Remaining VBA-Web work: graduate the harness from pure helpers into a policy-gated Dictionary lane, then `WithEvents`/async WinHTTP as separate live lanes. |
