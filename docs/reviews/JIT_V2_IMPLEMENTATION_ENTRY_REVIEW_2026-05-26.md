...
| TB03 Error routing | Fixture/design ready; VM seed proves runtime behavior; executable work blocked on package error-frame evidence | Needs package error/resume maps, failing-helper descriptor evidence, `Err` state equality fields, and resume target equality. |
| TB04 BSTR lifetime | Fixture/design ready; lifecycle seed rows and selected UDT BSTR lifecycle evidence exist; executable work blocked on non-UDT BSTR lifetime evidence | Needs declared String/BSTR layout facts, concat/Len helper descriptors, allocation/release counters, and branch/failure/deopt cleanup evidence. |
| TB05 SAFEARRAY | VM seed exists for store, index, For Each, array descriptors, dynamic bounds, and selected rank-1 descriptor-backed bounds lookup; executable work blocked on remaining array evidence | Runtime bounds-error fixture/evidence, multi-rank coverage, lifecycle ownership observations, and COM/native SAFEARRAY projection are still required before TB05 can close. |
| TB06 Late-bound COM | Hosted VM seed and first package instruction evidence exist; executable work blocked on COM boundary descriptor evidence | Needs package-owned HRESULT/EXCEPINFO/ArgErr projection, named/default member facts, object identity observations, and cleanup evidence. |
| TB07 Early-bound COM | Hosted VM seed and early-bound instruction evidence exist; executable work blocked on typelib/imported member descriptor evidence | Needs imported COM class/interface/member descriptors, typelib descriptor digest, dispatch/vtable strategy evidence, object identity observations, and argument/return projection evidence. |
| TB08 Native Declare | Hosted Windows native VM seed and descriptor evidence exist for the current subset; executable work blocked on generic ABI and cleanup evidence | Current VM seed covers scalar, BSTR pointer, SAFEARRAY buffer pointer, Variant cell pointer, and scalar ByRef writeback; general Automation `Variant`/`SAFEARRAY` declared-parameter ABI support, cleanup/buffer ownership, and native error policy remain tracer-closure gaps. |
| TB09 Exported callable | Internal VM projection seed exists; executable work blocked on export descriptor evidence | External inbound/outbound ABI projection, ByRef writeback, cleanup/error return policy, and unsupported-shape diagnostic evidence are still required before TB09 can close. |
