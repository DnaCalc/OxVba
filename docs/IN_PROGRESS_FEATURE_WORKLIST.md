...
2. explicit `DispatchInvoke(...)`, natural internal class `Property Get` / `Property Let` / `Property Set`, and natural bare default-member `Get` / `Let` / `Set` now execute on the shared PMR/dynamic-object substrate when authoritative metadata exists, but this still does not amount to full `Set`/`Let`/default-member parity,
