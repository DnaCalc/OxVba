...
Update (2026-03-19, current pass): the imported zero-arg getter subset now also includes controlled object-valued `PropertyGet` members, so direct and parenthesized read-assignment syntax for `SelfDispatch` and `SelfUnknown` now has direct compiler + host evidence across explicit `Set` on typed `Object` targets plus implicit / explicit-`Let` assignment on `Variant` targets.
