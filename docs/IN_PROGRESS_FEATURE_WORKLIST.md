...
Update (2026-03-19, current pass): imported `WithEvents` declarations on typelib-qualified COM classes now fail deterministically on `BIND-E-TYPELIB-WITHEVENTS-UNSUPPORTED` instead of compiling as silent plain-object declarations, so the current `IP-05`/`IP-07` boundary is explicit while true imported COM event subscription lowering remains unimplemented.
