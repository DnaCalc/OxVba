...
Latest note (2026-03-15): the active `IP-03A` late-bound COM transport subset now includes controlled `VT_R4` / `Single` and `VT_DATE` scalar and one-dimensional typed SAFEARRAY result lanes on the shared semantic `f64` carrier; broader external `VARIANT`, non-`IDispatch` interface, and multi-dimensional SAFEARRAY parity remains `in-progress`.
