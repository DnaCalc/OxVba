...
3. bounded single-candidate fallback now covers native non-authoritative default-member `Get` / `Let` / `Set`, including the current no-parentheses-argument getter subset, bounded ambiguous scalar/indexed getter/let/set rejection exists across the proven read-assignment plus statement-context, explicit `Call`, no-parentheses-argument, and zero-arg parenthesized getter contexts where applicable, and bounded scalar/indexed `no viable candidate` getter/let/property-set rejection also exists across the proven read-assignment, statement-context, explicit `Call`, no-parentheses-argument, zero-arg parenthesized getter, and indexed property-set contexts where applicable, but broader fallback/recovery parity is still open,
