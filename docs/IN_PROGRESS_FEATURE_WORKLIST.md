...
| `IP-03` | Windows late-bound COM client (`IDispatch`) parity | in-progress | invoke transport now carries named/omitted metadata; `oxvba-com` now owns a first semantic request/callback value carrier (`ComValue`) for the recoverable subset; array/null/error intent, runtime strings, native COM-backed object arguments, native `VT_DISPATCH` results, `VT_UNKNOWN` results that expose `IDispatch`, and owned one-dimensional `VT_ARRAY | VT_VARIANT` payloads now survive into and back out of the runtime-facing COM boundary; named method/property-get and member-known property-put/property-putref lanes execute; controlled dispatch now roundtrips `VT_NULL`, `VT_ERROR`, `VT_I2`, `VT_UI2`, BSTR strings, and bounded semantic SAFEARRAY payloads; bounded `ArgErr`/`EXCEPINFO` fidelity is now preserved in the controlled native lane; explicit metadata-backed default-member dispatch via `DispatchInvoke(obj, 0, ...)` now resolves when authoritative identity exists; controlled/registered lanes and deterministic member resolution subset remain in place | natural/default-member syntax without authoritative identity, broader non-`IDispatch` interface-pointer/`VARIANT`/multi-dimensional-or-non-`VT_VARIANT` SAFEARRAY marshalling, faithful external `ArgErr`/`ExcepInfo`/`VarResult`, and practical Office automation parity remain open | `docs/worksets/WORKSET_2026-03-10_IDISPATCH_LATEBOUND_COM_COMPLETION.md`, `docs/worksets/WORKSET_2026-03-11_UNIFIED_DYNAMIC_OBJECT_PROTOCOL_AND_VALUE_CARRIER.md`, `docs/spec/COM_CLIENT_SERVER_SCOPE_V1.md` |
| `IP-04` | `oxvba-com` architectural repurpose and HAL COM extraction | in-progress | shared COM transport/types, typelib catalog pieces, the first semantic request/callback value carrier, an executable generic dynamic-object protocol API, and the first runtime value-model migration slices now exist; active VM host intrinsics now enter HAL through semantic runtime-value wrappers instead of raw-token reads; `FileSystemHal::{open,close,seek,eof,lof,free_file}`, `EventPumpHal::do_events()`, `TimeLocaleHal::{date_serial_now,time_serial_now,timer_ticks}`, `UiInteractionHal::{msg_box,input_box}`, `ProcessEnvHal::{shell,environ,dir}`, `DynamicLinkHal::{invoke_symbol,invoke_descriptor}`, `DiagnosticsHal::emit`, `DynamicLinkHal::{prepare_invoke,invoke_bound}`, and `ComHal::{subscribe_event,unsubscribe_event,event_callback_subscription,event_callback_arity,event_callback_arg,release_event_callback}` now use direct semantic `RuntimeValue` contracts on the VM/conformance-facing path; COM activation now also defaults to semantic `RuntimeValue` ProgID input and semantic `RuntimeValue::ObjectHandle(...)` output, COM release/cache-maintenance now default to semantic `RuntimeValue` return paths, and the old COM maintenance `*_legacy(...)` wrappers have been removed from the HAL contract; the raw scalar `dispatch_invoke_legacy(object, member, arg)` helper and the request-shaped `dispatch_invoke_legacy_v2(...)` compatibility entrypoint have both been removed from the public HAL contract and reduced to test-local helpers; COM member/event identity is now typed on the shared boundary via `ComMemberToken`, the standard-adapter COM state store now also uses typed object/subscription/callback tokens internally, `ComBinding` now types its member/event metadata maps by `ComMemberToken`, queued COM callback args now remain in the shared `ComValue` carrier inside the adapter instead of degrading to `Vec<i32>` until poll time, native COM event sink/request scaffolding now also preserves typed COM identity end to end, the shared runtime-facing COM model now uses `ObjectHandle` for object identity instead of `ComObjectToken`, and the standard dynlink binding registry now allocates opaque `BindingHandle` values instead of aliasing them directly to symbol tokens; `RuntimeValue` now also carries `BindingHandle(...)` semantically so dynamic-link binding identity no longer has to masquerade as plain `I32`; VM `WithEvents` owner iteration now preserves `ObjectHandle` identity internally, and host COM callback dispatch now preserves `ComCallbackToken` / `ComSubscriptionToken` identity instead of normalizing them back to bare integers; the runtime now also has an explicit owned `RuntimeValue` <-> `Variant` bridge for the currently honest scalar subset (`Empty`, `Null`, `I32`, `Bool`); COM token and runtime handle wrappers are now explicitly `repr(transparent)` so typed bookkeeping no longer depends on accidental layout; native COM-backed object arguments and `VT_DISPATCH`/dispatch-capable `VT_UNKNOWN` results now cross the Windows dispatch boundary through the semantic carrier instead of degrading to plain integers; owned one-dimensional `VT_ARRAY | VT_VARIANT` payloads now also cross the controlled invoke/result boundary semantically; the extracted Windows `VARIANT`/bounded `SAFEARRAY` translation, result-classification, invoke-failure/`EXCEPINFO`, and canonical runtime-value `IDispatch::Invoke` helper surface now lives in `oxvba-com` instead of `standard.rs`; the standard Windows COM adapter now treats `dispatch_invoke_runtime_value_v2(...)` as the canonical implementation seam; host public snapshot APIs, direct `Vm` observation APIs, and VM/JIT execution helpers are now value-first by name, and the old VM/JIT/engine legacy snapshot wrapper APIs have been removed; JIT value snapshots now preserve full semantic `RuntimeValue` shape on VM fallback instead of flattening unsupported lanes back through `Vec<i32>`; VM `WithEvents` binding state now preserves semantic `RuntimeValue` payloads rather than flattening bound source/object values to integers; VM `DispatchInvoke` now reads object slots from semantic runtime state and preserves object handles rather than re-reading them through the raw slot lane | major COM ownership slices now live in `oxvba-com`, including deterministic member/event metadata lookup, source-interface support policy, metadata-driven binding assembly, the Windows COM event/callback state wrapper, activation/named-DISPID helper glue, activation-time binding insertion, bound-dispatch/object-release lifecycle ownership, resolved-member DISPID cache lookup/update, subscription callback-pruning bookkeeping, callback payload polling/metadata access/release helpers, event transport-choice resolution, bound/unbound invoke-policy planning, member-spec/direct-DISPID runtime invoke execution helpers, and the generic runtime-value invoke execution helper; remaining work is final invoke-result lifecycle ownership, HAL contract contraction/rebinding, runtime-wide protocol wiring, and broader interface-pointer/SAFEARRAY closure | `docs/worksets/WORKSET_2026-03-09_OXVBA_COM_REPURPOSE_AND_HAL_COM_EXTRACTION.md`, `docs/worksets/WORKSET_2026-03-11_UNIFIED_DYNAMIC_OBJECT_PROTOCOL_AND_VALUE_CARRIER.md`, `docs/worksets/WORKSET_2026-03-11_RUNTIME_VALUE_MODEL_MIGRATION.md`, `docs/worksets/WORKSET_2026-03-11_COM_REFERENCE_FACADE_AND_TYPELIB_BINDING_COMPLETION.md`, `docs/ARCHITECTURE.md` |
...
3. HAL still carries the active COM contract surface and bootstrap/delegation seams, so the remaining work is contraction and rebinding rather than another metadata extraction slice.`r`n4. callback payload polling/metadata access now also delegates through `oxvba-com`, further narrowing the remaining live Windows execution seam.`r`n5. parity-complete closure still depends on moving the remaining COM behavior/contract authority behind the intended `oxvba-com` boundary.
