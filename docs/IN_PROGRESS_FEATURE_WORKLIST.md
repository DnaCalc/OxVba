...
| `IP-03` | Windows late-bound COM client (`IDispatch`) parity | in-progress | invoke transport now carries named/omitted metadata and named method/property-get lanes execute; controlled/registered lanes and deterministic member resolution subset remain in place | named property-put/putref, default-member named dispatch, optional-arg/source-surface closure, broad `VARIANT`/object/SAFEARRAY marshalling, faithful `ArgErr`/`ExcepInfo`/`VarResult`, and practical Office automation parity remain open | `docs/worksets/WORKSET_2026-03-10_IDISPATCH_LATEBOUND_COM_COMPLETION.md`, `docs/spec/COM_CLIENT_SERVER_SCOPE_V1.md` |
