...
3. bounded single-candidate fallback now covers native non-authoritative default-member `Get` / `Let` / `Set`, bounded ambiguous getter/let/set rejection exists, and bounded scalar `no viable candidate` getter/let/property-set rejection also exists, but broader fallback/recovery parity is still open,
