...
| `IP-03` | Windows late-bound COM client (`IDispatch`) parity | in-progress | widened request/vector invoke path, multi-arg subset, controlled/registered lanes, deterministic member resolution subset | named args, optional args, broad `VARIANT`/object/SAFEARRAY marshalling, faithful `ArgErr`/`ExcepInfo`/`VarResult`, and practical Office automation parity remain open | `docs/worksets/WORKSET_2026-03-10_IDISPATCH_LATEBOUND_COM_COMPLETION.md`, `docs/spec/COM_CLIENT_SERVER_SCOPE_V1.md` |
