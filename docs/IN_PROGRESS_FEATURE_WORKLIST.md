...
| `IP-08` | Host project / Office-style hosting parity | in-progress | host bridge and object/event ingress contracts are now locked at design level; host event ingress now dispatches into live runtime sessions for the current zero/one-argument subset; host-injected referenced class modules marked `VB_PredeclaredId` or `VB_GlobalNamespace` now have direct compiler and host evidence for bounded named property-get and authoritative default-member read lowering; the same host-injected root path now has direct evidence for matching named and authoritative-default-member `Property Let` writes across both exposure modes; the bounded invoke floor now covers explicit `Call` plus bare statement-context execution on named property-get and authoritative default-member zero-arg forms across both exposure modes; named object-valued host-root `Property Get` members now have bounded assignment evidence into `Object` targets across both exposure modes; and the same bounded host object-model floor now also proves typed child-local follow-on named/default-member reads, parenthesized zero-arg getter reads, indexed scalar getter reads, named/default-member scalar writes, indexed scalar writes, plus explicit `Call` and bare statement-context execution on plain, parenthesized, and indexed getter forms after `Set child = Application.Value` across both exposure modes, while plain project references remain on the ordinary unresolved-name path | broader host project semantics, broader child-object object-write/default-member breadth after host-root object return, Office-style root/global exposure rules, host object identity/callback routing, deterministic diagnostics for invalid host-looking roots, and final integration with the completed COM/property/event model remain open | `docs/spec/HOSTING_PROJECT_TOOLING_PROPOSAL.md`, `docs/worksets/WORKSET_2026-03-08_VBA71_WINDOWS_OFFICE_FULL_COMPLIANCE.md`, `docs/worksets/WORKSET_2026-03-19_IP-08A_EXECUTION_CHECKLIST.md` |
