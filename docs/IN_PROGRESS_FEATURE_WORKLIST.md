...
3. bounded single-candidate fallback now covers native non-authoritative default-member `Get` / `Let` / `Set`, bounded ambiguous getter/let/set rejection exists, and bounded scalar/indexed `no viable candidate` getter rejection plus scalar/indexed property-set and scalar let rejection also exists across the proven read-assignment, statement-context, explicit `Call`, and indexed property-set contexts, but broader fallback/recovery parity is still open,
