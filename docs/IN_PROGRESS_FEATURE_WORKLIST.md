...
3. the event-side `ComHal` contract is now typed end-to-end across HAL, VM, host, and adapter stubs, and the bound runtime invoke orchestration now also delegates through `oxvba-com`, so the remaining work is no longer callback-token normalization or invoke-policy routing but raw Windows activation plus the final invoke-result/object-lifecycle extraction.
