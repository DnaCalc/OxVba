...
2. explicit `DispatchInvoke(...)`, natural internal class `Property Get` / `Property Let` / `Property Set`, natural indexed member `Get` / `Let` / `Set` syntax, statement-context member/default-member `Property Get` calls, and natural bare/indexed default-member `Get` / `Let` / `Set` now execute on the shared PMR/dynamic-object substrate when authoritative metadata exists, but this still does not amount to full `Set`/`Let`/default-member parity,
