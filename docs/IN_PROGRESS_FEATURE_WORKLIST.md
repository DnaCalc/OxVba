...
2. explicit `DispatchInvoke(...)` plus natural internal class `Property Get` / `Property Let` now execute on the shared PMR/dynamic-object substrate, but this still does not amount to full `Set`/`Let`/default-member parity,
