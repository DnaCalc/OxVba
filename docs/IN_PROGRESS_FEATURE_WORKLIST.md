...
| `IP-05` | Windows early-bound COM and type-library parity | in-progress | implemented rewrite-based early-binding subset with typelib identity hints, binder lowering, executable conformance lanes, metadata-backed external member-token plus `As New` activation-selector lookup through `oxvba-com`, deterministic metadata-backed external arity validation for the current supported imported types, direct required-argument `PropertyGet` evidence, authoritative imported default-member lookup for parenthesized call syntax, and deterministic compile-time rejection for imported `PropertyPut` / `PropertyPutRef` member shapes on the current supported path | full VBA/Excel early-binding parity, richer typelib/member coverage, broader synthetic reference-facade binder integration, and final architectural migration out of transitional HAL-owned execution remain open | `docs/spec/COM_EARLY_BINDING_TYPELIB_SCOPE_V1.md`, `docs/worksets/WORKSET_2026-03-11_COM_REFERENCE_FACADE_AND_TYPELIB_BINDING_COMPLETION.md`, `docs/worksets/WORKSET_2026-03-18_IP-05A_EXECUTION_CHECKLIST.md`, `docs/worksets/WORKSET_2026-03-08_VBA71_WINDOWS_OFFICE_FULL_COMPLIANCE.md` |
