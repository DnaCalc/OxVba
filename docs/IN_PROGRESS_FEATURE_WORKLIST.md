...
3. bounded single-candidate fallback now covers native non-authoritative default-member `Get` / `Let` / `Set`, and bounded ambiguous getter/let rejection also exists, but broader fallback/recovery parity is still open,
