...
Latest note (2026-03-18): the active `IP-03A` late-bound COM transport subset now includes controlled `VT_R4` / `Single`, `VT_R8` / `Double`, and `VT_DATE` scalar and one-dimensional typed SAFEARRAY result lanes on a tagged semantic `f64` carrier that now also preserves outward `Single` and `Date` vartype fidelity plus controlled `VT_CY` / `Currency` scalar and one-dimensional typed SAFEARRAY result lanes on an exact scaled-`i64` currency carrier and controlled `VT_DECIMAL` scalar and one-dimensional typed SAFEARRAY result lanes on an exact `Decimal96` carrier; named-result host evidence for scalar `VT_BOOL`, `VT_BSTR`, `VT_EMPTY`, `VT_NULL`, and `VT_ERROR` plus outbound classifier evidence for scalar `VT_BOOL`, `VT_BSTR`, `VT_EMPTY`, `VT_NULL`, and `VT_ERROR` arguments are also in place; scalar `VT_I8` / `VT_UI4` / `VT_UI8` / `VT_UINT` values, one-dimensional typed `VT_ARRAY | VT_I8` / `VT_UI4` / `VT_UI8` / `VT_UINT` overflow, rank-2 `VT_ARRAY | VT_VARIANT` results, nested non-`IDispatch` `VT_UNKNOWN` elements inside one-dimensional `VT_ARRAY | VT_VARIANT` results, and scalar or typed-array `VT_BYREF` result payloads now fail with deterministic bounded diagnostics instead of silently wrapping or drifting through the current adapter surface; bounded invoke-failure evidence now also covers stable `DISP_E_MEMBERNOTFOUND` and `DISP_E_BADPARAMCOUNT` classification on the host fault surface plus stable `DISP_E_UNKNOWNNAME` classification both at the adapter boundary for raw `GetIDsOfNames` failures and on the host fault surface for runtime string member selectors, and the runtime-string subset now also has bounded success evidence for zero-arg method, named-argument method, zero-arg property-get, indexed property-get, named-argument indexed property-get, metadata-backed default-member-name dispatch, metadata-backed property put/property putref selectors, and metadata-backed indexed property put/property putref selectors; `IP-02A` also now has bounded end-to-end evidence that `Set` accepts object-producing call results for both `Object` and `Variant` targets, directly rejects scalar sources on both typed `Variant` and `Object` targets, rejects object-producing call results on scalar targets with stable diagnostics, directly rejects omitted-`Set` object-call assignment on typed `Object` targets with a stable `Set required ...` diagnostic, rewrites lowered external `As New` initialization as `Set obj = CreateObject(...)`, and now also directly routes explicit `Set target = widget.Value`, `Set target = widget.Value(x)`, `Set target = widget`, `Set target = widget()`, and `Set target = widget(x)` through the native PMR object-property-get path for object-returning internal-class properties plus the bounded single-visible-candidate non-authoritative default-member fallback, while authoritative object-returning native property/default-member getters now also have direct `Variant`-target success evidence for explicit `Set`, explicit `Let`, and implicit assignment across named, zero-arg parenthesized, indexed, and authoritative default-member syntax, authoritative object-returning native property/default-member getters now also have direct `Object`-target rejection evidence for explicit `Let` and implicit assignment across the same named, zero-arg parenthesized, indexed, and authoritative default-member syntax matrix, bounded single-visible-candidate non-authoritative object-returning default members now also have direct `Variant`-target success evidence for explicit `Set`, explicit `Let`, and implicit assignment plus direct `Object`-target rejection evidence for explicit `Let` and implicit assignment across bare, zero-arg parenthesized, and indexed syntax, and object-returning native property/default-member getter results now also have direct scalar-target rejection evidence for explicit `Let` and implicit assignment across named, zero-arg parenthesized, indexed, authoritative default-member, and landed single-visible-candidate non-authoritative default-member syntax, `Let` accepts object-producing call results for `Variant` targets, rejects scalar sources on `Object` targets, rejects object-producing call results on both `Object` and scalar targets with stable diagnostics, directly routes `Let valueOut = widget.Value()`, `Let valueOut = widget()`, and the bounded non-authoritative `Let valueOut = widget()` parenthesized zero-arg getter forms through the same PMR-backed read-assignment path, and implicit assignment still accepts object-producing call results for `Variant` targets while rejecting known `CreateObject(...)` object results on scalar targets and plain scalar sources on `Object` targets in the current bounded subset rather than full assignment parity; authoritative native zero-arg and indexed property/default-member getters now also execute through explicit `Call ...()` / `Call ...(...)`, natural parenthesized statement-context form, and no-parentheses-argument form, native non-authoritative default-member syntax now falls back in scalar and indexed form when there is exactly one visible candidate of the requested kind across assignment, statement-context, explicit `Call` getter contexts, no-parentheses-argument getter contexts, bounded explicit `Set`/`Let` read-assignment contexts, scalar/indexed default-member `Property Set`, and object-returning `Variant`/`Object`-target read-assignment contexts across bare/parenthesized/indexed syntax where landed, while the bounded ambiguous native non-authoritative getter, let-assignment, and property-set cases now fail with deterministic `PMR-E-DEFAULT-MEMBER-RESOLUTION-AMBIGUOUS` compile-time diagnostics across scalar/indexed read-assignment plus statement-context, explicit `Call`, no-parentheses-argument, and zero-arg parenthesized getter contexts where applicable, and the bounded scalar/indexed getter, scalar/indexed let-assignment, and scalar/indexed property-set `no viable candidate` cases now fail with deterministic `PMR-E-DEFAULT-MEMBER-RESOLUTION-MISSING` diagnostics across the proven read-assignment, statement-context, explicit `Call`, no-parentheses-argument, zero-arg parenthesized getter, and indexed property-set lanes where applicable; the active `IP-02` execution contract is now tracked in `docs/worksets/WORKSET_2026-03-18_IP-02_EXECUTION_CHECKLIST.md`; broader external `VARIANT`, non-`IDispatch` interface, multi-dimensional SAFEARRAY parity, and wider `Set`/`Let` / default-member closure remain `in-progress`.
...
| `IP-02` | VBA property model and default-member semantics | in-progress | compiler/runtime and COM metadata foundations exist; native internal project-class `Property Get` / `Property Let` / `Property Set` now execute end to end through the shared PMR/dynamic-object substrate for explicit `DispatchInvoke(...)`, natural member syntax, natural indexed member `Get` / `Let` / `Set` syntax, statement-context, `Call`-form, and no-parentheses-argument member/default-member `Property Get` calls, natural bare/indexed default-member `Get` / `Let` / `Set` syntax when authoritative metadata exists, and a bounded non-authoritative native fallback in both scalar and indexed form when exactly one visible candidate of the requested kind exists, now including scalar and indexed default-member `Property Set` plus bounded no-parentheses-argument default-member getter execution, while the bounded ambiguous native getter, let-assignment, property-set, and object-valued read-assignment cases fail deterministically with `PMR-E-DEFAULT-MEMBER-RESOLUTION-AMBIGUOUS` across scalar/indexed read-assignment plus statement-context, explicit `Call`, no-parentheses-argument, and zero-arg parenthesized getter contexts where applicable, and the bounded scalar/indexed getter, scalar/indexed let-assignment, scalar/indexed property-set, and object-valued read-assignment `no viable candidate` cases fail deterministically with `PMR-E-DEFAULT-MEMBER-RESOLUTION-MISSING` across the proven read-assignment, statement-context, explicit `Call`, no-parentheses-argument, zero-arg parenthesized getter, and indexed property-set lanes where applicable; explicit `Set` / `Let` keywords on plain assignment and assignment-from-call statements now survive into bounded typechecking instead of being erased, bounded host/compiler evidence now proves `Set` object-call success for both `Variant` and `Object` targets, bounded `Set` rejection for scalar-source and scalar-target mismatch lanes, bounded scalar-typed native property/default-member getter-result rejection for explicit `Set` across typed `Variant`, `Object`, and scalar targets on named, zero-arg parenthesized, indexed, authoritative default-member, and landed single-candidate non-authoritative default-member syntax, bounded scalar-typed native property/default-member getter-result support for explicit `Let` and implicit assignment into typed `Variant` and scalar targets plus bounded explicit-`Let` and implicit-assignment rejection on typed `Object` targets across the same named, zero-arg parenthesized, indexed, authoritative default-member, and landed single-candidate non-authoritative default-member syntax, bounded authoritative object-returning native property/default-member getter `Variant`-target success for explicit `Set`, explicit `Let`, and implicit assignment across named, zero-arg parenthesized, indexed, and authoritative default-member syntax, bounded authoritative object-returning native property/default-member getter `Object`-target rejection for explicit `Let` and implicit assignment across the same named, zero-arg parenthesized, indexed, and authoritative default-member syntax matrix, bounded single-visible-candidate non-authoritative object-returning default-member `Variant`-target success for explicit `Set`, explicit `Let`, and implicit assignment across bare, zero-arg parenthesized, and indexed syntax, bounded single-visible-candidate non-authoritative object-returning default-member `Object`-target rejection for explicit `Let` and implicit assignment across the same bare, zero-arg parenthesized, and indexed syntax matrix, bounded scalar-target rejection for object-returning native property/default-member getter results on explicit `Set`, explicit `Let`, and implicit assignment across named, zero-arg parenthesized, indexed, authoritative default-member, and landed single-candidate non-authoritative default-member syntax, bounded ambiguous/missing non-authoritative object-valued default-member source-resolution diagnostics for explicit `Set`, explicit `Let`, and implicit assignment on typed `Object`, typed `Variant`, and scalar targets across bare, zero-arg parenthesized, and indexed syntax, bounded zero-arg parenthesized explicit-`Set` read-assignment execution for named/default-member object getters in both authoritative and single-visible-candidate non-authoritative native lanes, bounded `Let` object-call success for `Variant` targets, bounded zero-arg parenthesized explicit-`Let` read-assignment execution for named/default-member getters in both authoritative and single-visible-candidate non-authoritative native lanes, bounded `Let` rejection for `Object`-target scalar/object lanes and scalar-target object-call lanes, bounded implicit-assignment rejection for known `CreateObject(...)` results on scalar targets, plain scalar sources on `Object` targets, and omitted-`Set` object-call assignment on `Object` targets, lowered external `As New` initialization now emits explicit `Set`, and bounded host evidence now proves explicit `Let` on natural native property/default-member assignment and read-assignment follows the same PMR-backed route as the implicit syntax when authoritative metadata exists; the active lane register now lives in `docs/worksets/WORKSET_2026-03-18_IP-02_EXECUTION_CHECKLIST.md` | broader `Set` vs `Let` intent parity, broader non-authoritative default member resolution beyond the bounded single-candidate plus deterministic missing/ambiguous scalar/indexed getter/let/set/object-target read subsets, and Office-style call-vs-value parity still need end-to-end closure | `docs/worksets/WORKSET_2026-03-08_VBA71_WINDOWS_OFFICE_FULL_COMPLIANCE.md`, `docs/worksets/WORKSET_2026-03-11_UNIFIED_DYNAMIC_OBJECT_PROTOCOL_AND_VALUE_CARRIER.md`, `docs/worksets/WORKSET_2026-03-14_COM_PARITY_PROPERTY_SERVER_HOSTING_EXECUTION_SEQUENCE.md`, `docs/worksets/WORKSET_2026-03-18_IP-02_EXECUTION_CHECKLIST.md` |
...
3. bounded single-candidate fallback now covers native non-authoritative default-member `Get` / `Let` / `Set`, including the current no-parentheses-argument getter subset, bounded ambiguous scalar/indexed getter/let/set plus typed-`Object` read-assignment rejection exists across the proven read-assignment plus statement-context, explicit `Call`, no-parentheses-argument, and zero-arg parenthesized getter contexts where applicable, bounded scalar/indexed `no viable candidate` getter/let/property-set plus typed-`Object` read-assignment rejection also exists across the proven read-assignment, statement-context, explicit `Call`, no-parentheses-argument, zero-arg parenthesized getter, and indexed property-set contexts where applicable, both authoritative and bounded single-visible-candidate non-authoritative object-returning default-member getter results now also have bounded direct `Variant`-target success evidence for explicit `Set`, explicit `Let`, and implicit assignment across the landed bare/parenthesized/indexed syntax matrix, authoritative object-returning native property/default-member getter results now also have bounded direct `Object`-target rejection evidence for explicit `Let` and implicit assignment across named, zero-arg parenthesized, indexed, and authoritative default-member syntax, and object-returning native getter results now also have bounded direct scalar-target rejection evidence for explicit `Let` and implicit assignment across named, zero-arg parenthesized, indexed, authoritative default-member, and landed single-candidate non-authoritative default-member syntax, but broader fallback/recovery parity is still open,
4. this is one of the highest-risk semantic areas for “looks implemented but is not parity-complete” confusion.
