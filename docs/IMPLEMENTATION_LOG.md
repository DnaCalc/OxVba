...
- In [project.rs](C:\Work\DnaCalc\OxVba\crates\oxvba-compiler\src\project.rs), added direct compiler evidence that an active-project same-name `Application` class module outranks the host-injected root when the winning root returns a COM-backed object and the caller then executes imported parenthesized named-argument explicit-`Call` and bare statement-context method/property/default-member lanes.
- In [engine.rs](C:\Work\DnaCalc\OxVba\crates\oxvba-host\src\engine.rs), added matching host runtime evidence proving the same precedence boundary executes end to end even when the same-name host root only offers a scalar `Value` getter, so the imported parenthesized named-argument invoke subset completes on the project-owned COM object instead of failing during the root handoff.
- This narrows the remaining `IP-08B` root/global/project frontier again: active-project precedence is now explicit through the current positional invoke subset across both parenthesized and no-paren forms plus the current parenthesized named-argument invoke subset, while broader imported breadth and the remaining no-paren named-argument neighbors still remain open.

## 2026-03-20 - Extend IP-08B active-project precedence through host-returned COM no-paren statement-context forms

- Continued `IP-08B` by widening the active-project-versus-host-root identity register from imported read assignments plus the current explicit `Call` subsets into the matching no-paren bare statement-context invoke subset on host-returned COM-backed objects.
- In [project.rs](C:\Work\DnaCalc\OxVba\crates\oxvba-compiler\src\project.rs), added direct compiler evidence that an active-project same-name `Application` class module outranks the host-injected root when the winning root returns a COM-backed object and the caller then executes imported no-paren bare statement-context positional method/property/default-member lanes.
- In [engine.rs](C:\Work\DnaCalc\OxVba\crates\oxvba-host\src\engine.rs), added matching host runtime evidence proving the same precedence boundary executes end to end even when the same-name host root only offers a scalar `Value` getter, so the imported no-paren bare statement-context subset completes on the project-owned COM object instead of failing during the root handoff.
- This narrows the remaining `IP-08B` root/global/project frontier again: active-project precedence is now explicit through the current imported read-assignment subset plus the current explicit-`Call` and bare statement-context subsets across both parenthesized and no-paren forms, while broader imported breadth and remaining named-argument neighbors still remain open.

## 2026-03-20 - Extend IP-08B active-project precedence through host-returned COM no-paren Call forms

- Continued `IP-08B` by widening the active-project-versus-host-root identity register from imported read assignments plus the parenthesized explicit `Call` subset into the matching no-paren explicit `Call` invoke subset on host-returned COM-backed objects.
- In [project.rs](C:\Work\DnaCalc\OxVba\crates\oxvba-compiler\src\project.rs), added direct compiler evidence that an active-project same-name `Application` class module outranks the host-injected root when the winning root returns a COM-backed object and the caller then executes imported no-paren explicit `Call` positional method/property/default-member lanes.
- In [engine.rs](C:\Work\DnaCalc\OxVba\crates\oxvba-host\src\engine.rs), added matching host runtime evidence proving the same precedence boundary executes end to end even when the same-name host root only offers a scalar `Value` getter, so the imported no-paren explicit `Call` subset completes on the project-owned COM object instead of failing during the root handoff.
- This narrows the remaining `IP-08B` root/global/project frontier again: active-project precedence is now explicit through the current imported read-assignment subset plus the current explicit-`Call` subset across both parenthesized and no-paren forms, while broader imported breadth and remaining statement-context neighbors still remain open.

## 2026-03-20 - Extend IP-08B active-project precedence through host-returned COM statement-context forms

- Continued `IP-08B` by widening the active-project-versus-host-root identity register from imported read assignments and explicit `Call` into the matching bare statement-context invoke subset on host-returned COM-backed objects.
- In [project.rs](C:\Work\DnaCalc\OxVba\crates\oxvba-compiler\src\project.rs), added direct compiler evidence that an active-project same-name `Application` class module outranks the host-injected root when the winning root returns a COM-backed object and the caller then executes imported bare statement-context positional method/property/default-member lanes.
- In [engine.rs](C:\Work\DnaCalc\OxVba\crates\oxvba-host\src\engine.rs), added matching host runtime evidence proving the same precedence boundary executes end to end even when the same-name host root only offers a scalar `Value` getter, so the imported bare statement-context subset completes on the project-owned COM object instead of failing during the root handoff.
- This narrows the remaining `IP-08B` root/global/project frontier again: active-project precedence is now explicit through the current imported read-assignment subset plus the matching explicit-`Call` and bare statement-context invoke subsets on host-returned COM-backed objects.
...
- In [project.rs](C:\Work\DnaCalc\OxVba\crates\oxvba-compiler\src\project.rs), added direct compiler evidence that an active-project same-name `Application` class module outranks the host-injected root when the winning root returns a COM-backed object and the caller then executes imported scalar, named-argument, and positional read-assignment lanes.
- In [engine.rs](C:\Work\DnaCalc\OxVba\crates\oxvba-host\src\engine.rs), added matching host runtime evidence proving the same precedence boundary executes end to end even when the same-name host root only offers a scalar `Value` getter, so the returned COM handle and all follow-on imported read witnesses stay project-owned instead of drifting back to the host root.
- This narrows the remaining `IP-08B` root/global/project frontier again: active-project precedence is now explicit through the current host-returned imported read-assignment subset, not only on the simpler root property read/write floor.
...
- In [project.rs](C:\Work\DnaCalc\OxVba\crates\oxvba-compiler\src\project.rs), added direct compiler evidence that host-returned COM objects preserve imported positional method/property/default-member read-assignment rewrites with both implicit and explicit-`Let` syntax after `Set obj = Application.Value` across both `VB_PredeclaredId` and `VB_GlobalNamespace`.
- In [engine.rs](C:\Work\DnaCalc\OxVba\crates\oxvba-host\src\engine.rs), added matching host runtime evidence proving those imported positional read-assignment lanes execute end to end with deterministic method/property/default-member witnesses on the returned COM object.
- This widens the bounded host/COM coexistence floor beyond statement-only positional coverage and makes the imported positional read-assignment subset explicit on host-returned COM objects.
...
- In [project.rs](C:\Work\DnaCalc\OxVba\crates\oxvba-compiler\src\project.rs), added direct compiler evidence that host-returned COM objects preserve imported zero-arg property-get, parenthesized zero-arg property-get, and zero-arg method read-assignment rewrites after `Set obj = Application.Value` across both `VB_PredeclaredId` and `VB_GlobalNamespace`.
- In [engine.rs](C:\Work\DnaCalc\OxVba\crates\oxvba-host\src\engine.rs), added matching host runtime evidence proving those imported scalar read-assignment lanes execute end to end with deterministic getter and method witnesses on the returned COM object.
- This widens the bounded host/COM coexistence floor beyond invoke-only coverage for those members and makes the imported scalar read-assignment subset explicit on host-returned COM objects.

## 2026-03-19 - Extend IP-08B host-root parenthesized default-member object return

- Continued `IP-08B` by widening the host-root object-return syntax breadth from direct authoritative default-member assignment into the matching parenthesized zero-arg neighbor.
...
- In [engine.rs](C:\Work\DnaCalc\OxVba\crates\oxvba-host\src\engine.rs), added matching host runtime evidence proving those six parenthesized authoritative default-member `Variant` lanes execute end to end and preserve the same bounded object-handle witness as the already-proved direct default-member neighbor.
- This completes the current bounded host-root object-return syntax floor across direct and parenthesized zero-arg named-property and authoritative default-member forms.
...
- 2026-03-15: extended controlled COM scalar result evidence to `VT_I8`, `VT_UI8`, `VT_ARRAY | VT_I8`, and `VT_ARRAY | VT_UI8` via new ReturnHyper / ReturnUnsignedHyper fixture members, with compiler member-token coverage (45-48), checked narrowing through the then-current compatibility `RuntimeValue::I32` carrier lane, and host VM/JIT parity assertions for both scalar and typed SAFEARRAY results.
- 2026-03-15: extended controlled COM scalar result evidence to VT_INT, VT_UINT, VT_ARRAY | VT_INT, and VT_ARRAY | VT_UINT via new platform-int fixture members, with compiler member-token coverage (41-44) and host VM/JIT parity through the then-current compatibility `RuntimeValue::I32` carrier lane.
- 2026-03-15: extended controlled COM scalar result evidence to VT_I1 and VT_ARRAY | VT_I1 via ReturnSignedByte / ReturnSignedByteArray, with compiler member-token coverage (39/40) and host VM/JIT parity through the then-current compatibility `RuntimeValue::I32` carrier lane.
- 2026-03-15: extended controlled COM scalar result evidence to VT_UI1 and VT_ARRAY | VT_UI1 via ReturnByte / ReturnByteArray, with compiler member-token coverage (37/38) and host VM/JIT parity through the then-current compatibility `RuntimeValue::I32` carrier lane.
- 2026-03-15: extended controlled COM scalar result evidence to VT_I4 and VT_UI4 via ReturnLong / ReturnUnsignedLong, with compiler member-token coverage (35/36) and host VM/JIT parity through the then-current compatibility `RuntimeValue::I32` carrier lane.
- 2026-03-15: extended controlled COM typed SAFEARRAY result evidence to VT_ARRAY | VT_I4 and VT_ARRAY | VT_UI4 via new ReturnLongArray / ReturnUnsignedLongArray fixture members, compiler member-token coverage (33/34), and host VM/JIT parity assertions through the then-current compatibility `RuntimeValue::I32` carrier lane.
...
# 2026-03-20 - Extend host-returned COM precedence through imported property put/get traffic

- Continued `IP-08B` by tightening the active-project-versus-host-root identity register around the next broader imported member/property row instead of stopping at read and invoke traffic alone.
- In [project.rs](C:\Work\DnaCalc\OxVba\crates\oxvba-compiler\src\project.rs), added direct compiler evidence that an active-project same-name `Application` class module outranks the host-injected root when the winning root returns a COM-backed object and the caller then executes imported `PropertyPut` plus zero-arg `PropertyGet` traffic on that returned receiver.
- In [engine.rs](C:\Work\DnaCalc\OxVba\crates\oxvba-host\src\engine.rs), added matching host runtime evidence proving the same precedence boundary executes end to end even when the same-name host root only offers a scalar `Value` getter, so the imported property put/get lane completes on the project-owned COM object instead of failing during the root handoff.
- This narrows the remaining `IP-08B` host-returned COM breadth again: active-project precedence is now explicit not only for the current imported read and invoke rows but also for the imported property put/get neighbor on the same host-returned COM-backed object, while broader imported member/property/default-member breadth still remains open.

# 2026-03-20 - Extend host-returned COM precedence through imported property putref traffic

- Continued `IP-08B` by tightening the active-project-versus-host-root identity register around the next imported setter neighbor instead of leaving object-valued setter traffic implied by the scalar setter row.
- In [project.rs](C:\Work\DnaCalc\OxVba\crates\oxvba-compiler\src\project.rs), added direct compiler evidence that an active-project same-name `Application` class module outranks the host-injected root when the winning root returns a COM-backed object and the caller then executes imported `PropertyPutRef` traffic on that returned receiver.
- In [engine.rs](C:\Work\DnaCalc\OxVba\crates\oxvba-host\src\engine.rs), added matching host runtime evidence proving the same precedence boundary executes end to end even when the same-name host root only offers a scalar `Value` getter, so the imported property-putref lane completes on the project-owned COM object instead of failing during the root handoff.
- This narrows the remaining `IP-08B` host-returned COM breadth again: active-project precedence is now explicit for the current imported scalar read, invoke, property-put/get, and property-putref rows on the same host-returned COM-backed object, while broader imported member/property/default-member breadth still remains open.

# 2026-03-20 - Extend host-returned COM precedence through imported indexed setter traffic

- Continued `IP-08B` by tightening the active-project-versus-host-root identity register around the indexed setter neighbor instead of leaving the broader setter matrix split between plain-project precedence and active-project precedence.
- In [project.rs](C:\Work\DnaCalc\OxVba\crates\oxvba-compiler\src\project.rs), added direct compiler evidence that an active-project same-name `Application` class module outranks the host-injected root when the winning root returns a COM-backed object and the caller then executes imported indexed `PropertyPut` plus indexed `PropertyPutRef` traffic on that returned receiver across positional and named-argument forms.
- In [engine.rs](C:\Work\DnaCalc\OxVba\crates\oxvba-host\src\engine.rs), added matching host runtime evidence proving the same precedence boundary executes end to end even when the same-name host root only offers a scalar `Value` getter, so the indexed setter lanes complete on the project-owned COM object instead of failing during the root handoff.
- This narrows the remaining `IP-08B` host-returned COM breadth again: active-project precedence is now explicit for the current imported scalar read, invoke, property-put/get, property-putref, and indexed setter rows on the same host-returned COM-backed object, while broader imported member/property/default-member breadth still remains open.

# 2026-03-20 - Extend host-returned COM precedence through imported exception invoke forms

- Continued `IP-08B` by tightening the active-project-versus-host-root identity register around the observable runtime-fault row instead of leaving imported exception traffic covered only by the plain-project precedence proof.
- In [project.rs](C:\Work\DnaCalc\OxVba\crates\oxvba-compiler\src\project.rs), added direct compiler evidence that an active-project same-name `Application` class module outranks the host-injected root when the winning root returns a COM-backed object and the caller then executes imported `RaiseException` call and bare statement forms on that returned receiver across both parenthesized and no-paren syntax.
- In [engine.rs](C:\Work\DnaCalc\OxVba\crates\oxvba-host\src\engine.rs), added matching host runtime evidence proving the same precedence boundary still preserves the stable imported COM exception surface instead of falling back to the scalar host root and failing for the wrong reason.
- This narrows the remaining `IP-08B` host-returned COM breadth again: active-project precedence is now explicit for the current imported scalar read, invoke, property-put/get, property-putref, indexed setter, and exception-invoke rows on the same host-returned COM-backed object, while broader object-result and object-property breadth still remains open.

# 2026-03-20 - Extend host-returned COM precedence through imported object property gets

- Continued `IP-08B` by tightening the active-project-versus-host-root identity register around the non-parenthesized object-property-get assignment-intent row instead of leaving object-valued getter rebinding covered only by the plain-project precedence proof.
