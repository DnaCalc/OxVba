...
## Scope Summary
- Financial tolerance model: bounded solver policy and deterministic error-tag signaling for `Rate`/`NPer` non-convergence or invalid domains.
...
## Closure Signals
- Profile is complete when solver-failure paths are stable and observable as deterministic error tags with conformance/formal evidence.
