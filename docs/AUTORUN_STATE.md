...
Queue certification: certified — three current leaves are active; the actual ready queue contains exactly the five current leaves listed below and no stale work.
Claim queue: br ready -l ideal-2026-07 -t task
Current tasks: `bd-59co.2.2.9` (versioned cross-platform gate runner), `bd-59co.2.2.14` (Windows VARIANT/BSTR ownership accounting), and `bd-59co.3.1.5` (owned Windows test-resource policy).
Certified ready queue: `bd-59co.2.2.3`, `bd-59co.2.2.18`, `bd-59co.2.2.22`, `bd-59co.3.1.6`, and `bd-59co.3.15.4`.
...
Last completed tasks: `bd-59co.2.2.17` closed after destructor-free BSTR/SAFEARRAY borrow projections, explicit raw ownership transfers, Windows unchanged/null/replacement balance evidence, controller checks and clean independent Sol re-review; the separate native VARIANT/VariantClear counter seam is exact active successor `.14`. `bd-59co.2.2.16` remains closed after transactional VbaRecord/SAFEARRAY mutation and armed constructor-payload ownership passed strict all-feature Clippy, full runtime tests and three Miri lanes. `bd-59co.2.2.8` remains closed after the digest-pinned Linux x64 CI contract; exact baseline execution remains blocking under `.11`. Exact open successors `.9`, `.11`, `.14`, `.18`, `.22`, `.2.4.4`, and `bd-59co.2.7.2` own the portable gate runner, Linux execution, native VARIANT/BSTR accounting, VMR05 provenance, VM3 fixture provenance, compiler UDT size/depth limits, and hidden object-carrier thread/apartment transit; no terminal row is closed by these slices.
