...
- bounded ambiguous/missing non-authoritative object-valued default-member read-assignment diagnostics on the source-resolution path for explicit `Let`/implicit assignment to `Object` targets across bare, zero-arg parenthesized, and indexed syntax
- bounded ambiguous/missing non-authoritative object-valued default-member read-assignment diagnostics on the source-resolution path for explicit `Let`/implicit assignment to typed `Variant` targets across bare, zero-arg parenthesized, and indexed syntax
