...
- bounded scalar-target rejection for object-returning native property/default-member getter read-assignment across named member syntax and bare default-member syntax for explicit `Let` and implicit assignment in authoritative and landed single-candidate non-authoritative lanes
