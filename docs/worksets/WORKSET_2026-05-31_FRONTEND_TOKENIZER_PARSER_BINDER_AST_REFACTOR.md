...
- FE-8.5.h Project/class array-field carrier retirement: replace the remaining generated
  ...
  `__oxvba_array_get(__oxvba_withevents_get(...), ...)` source; upper-bound-only field-array
  `ReDim`/`ReDim Preserve` now use direct internal `__oxvba_array_field_redim*` bytecode-lowered
...
- FE-8.5.e Compile-time declarations and module options: implement HIR-owned `Option Explicit`,
  ...
  `Attribute VB_Name` and Boolean module attribute lines preserved as `BoundModule` metadata, basic
  typed `Const Name As Long = ...`
...
- FE-7.6/FE-8.5.f Reference/imported COM construction and member binding: route imported
  ...
  named dispatch except for true `Object`/`Variant` late binding. A further 2026-07-02 continuation
  ...
  no longer needs a separate fake reference for the returned interface. A subsequent
...
- FE-9.7 Broad matrix/corpus route audit: extend the route audit from selected fixtures to the
  ...
  work: broaden corpus execution beyond the current route-overlay and route-checked seed evidence,
  broaden document/host semantics beyond the
...
- FE-8.5 Production HIR lowering: implement production bytecode and `ProcedureRuntimeMetadata`
  ...
  for those completed lowering shapes, and later continuations route read/write dynamic-array
  ...
  Follow-up project-boundary continuation routes all-procedural active projects with no reference
  projects, or with only HIR-safe synthetic/declared typelib references, through the HIR-capable
  metadata compiler, preserving legacy routing for class/document, forced-object-local, and
  unresolved reference-project shapes.
...
- FE-8.5.c Property/default-member/writeback lowering: finish the semantic and lowering route for
  ...
  assignments, named/indexed imported COM property put/putref assignments, and selected imported
  COM property-get read/invoke routes now use accessor-specific internal early-invoke carriers and,
  ...
  leaking as duplicate IDE properties; the coalesced symbol prefers the getter accessor when
  present, preserving the read type and getter definition span. The IDE callable surface also hides
  canonical `property_get_*`/`property_let_*`/`property_set_*` implementation names and keeps the
  logical getter alias for signature help. Broader project/class/COM/default-member writeback and
...
- FE-8.5.e Compile-time options/declarations/constants: route `Option Explicit`,
  ...
  attribute lines such as `Attribute Value.VB_UserMemId` / `Attribute Value.VB_MemberFlags`
  preserved on front-end `BoundModule`/`BoundProcedure` facts, basic typed
  `Const Name As Long = ...`
  ...
  floating, Currency, and deterministic Date text constants, exact both-integer relational
  comparison folding for covered large `LongLong` constants, and explicit
...
- FE-8.5.f Broader declaration and type surface: finish `Property` procedure declarations,
  ...
  references plus checked nonnegative exponentiation and integer division/`Mod`, through the
  default HIR route within the existing `ExplicitI32` metadata shape,
  ...
  `SYM-E-INVALID-PROPERTY-SET-REFERENCE`, and known UDT final reference parameters reject through
  the same diagnostic after provider type-index construction, while default/explicit `Variant`,
  ...
  `SYM-E-INVALID-AS-NEW-DECLARATION` before normalized signature metadata is published, and UDT
  `Type` fields now reject `As New` with the same diagnostic before field-table metadata
  normalizes the type; duplicate UDT field names now reject with `SYM-E-DUPLICATE-TYPE-FIELD`
  before ambiguous folded record metadata is published; final `Property Let`/`Set` writer slots
  also publish `ByVal` signature metadata while preceding indexed arguments keep their
  ...
  diagnostics before event-field metadata is published, matching the Microsoft Learn `Private`
  statement rule
