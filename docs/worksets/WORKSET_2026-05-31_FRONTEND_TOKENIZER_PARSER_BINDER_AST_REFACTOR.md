...
- FE-8.5 Production HIR lowering: implement production bytecode and `ProcedureRuntimeMetadata`
  ...
  branch trees and records `If/Else` plus `If/ElseIf/Else` route-audit coverage. FE-8.5 remains open
...
- FE-9.6 Production legacy-route audit gate: prove before terminal closure that no scoped
  ...
  expands the audit with multiline `If/ElseIf` fixtures. The workset remains open for broader
