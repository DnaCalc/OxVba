...
- FE-8.5.f Broader declaration and type surface: finish `Property` procedure declarations,
  ...
  set. Follow-up expression routing covers `Mod` and `Like` through existing bytecode paths. The
  `Array(...)` helper proof is not array storage/indexing/`ReDim` closure; that remains in FE-8.5.d.
