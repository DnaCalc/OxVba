...
- FE-9.7 Broad matrix/corpus route audit: extend the route audit from selected fixtures to the
  ...
  HIR-production route, so the source-backed route gate now passes. Language-service
  ...
  document host-project, broaden imported COM beyond the `TestDispatch` seed route, cover broader
  class and deeper cross-workspace language-service route runners, and add an Excel-oracle route
  runner instead of skipped residual rows. This bead must reopen the owning
