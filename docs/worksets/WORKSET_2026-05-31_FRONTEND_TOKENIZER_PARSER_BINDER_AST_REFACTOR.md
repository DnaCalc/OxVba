...
- FE-9.7 Broad matrix/corpus route audit: extend the route audit from selected fixtures to the
  ...
  arbitrary default-expression and locale-sensitive Date breadth open; the same continuation proves
  the `ParamArray` flag on both surfaces.
...
- FE-8.5.c Property/default-member/writeback lowering: finish the semantic and lowering route for
  ...
  procedure calls; same-module indexed `Property Let`/`Property Set` writeback now lowers through
  HIR by appending the assigned value to the matching setter call, including named index arguments.
...
- FE-9.1 Per-construct default flip: route completed construct families through frontend v2 by
  ...
  literal/module-constant string/Boolean defaults, source-backed Date/Currency carriers,
...
- FE-9.7 Broad matrix/corpus route audit: expand FE-9.6 from selected route fixtures to the
  ...
  source-backed Date/Currency optional-default carriers plus deterministic Date literal defaults.
