...
- FE-9.7 Broad matrix/corpus route audit: expand FE-9.6 from selected route fixtures to the
  ...
  syntax bridge. Follow-up FE-9.7 grammar-matrix audit maps 40 anchored matrix productions to
  executable HIR-production route findings; this is route-proof overlay, not full matrix closure.
  Closure requires every accepted in-scope row to classify as HIR/SemanticModel production or to
  reopen/create the owning delivery bead.
