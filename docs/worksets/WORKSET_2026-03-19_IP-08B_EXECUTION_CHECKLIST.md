...
- active-project same-name `Application` precedence is now also explicit for the matching imported scalar, named-argument, and positional read-assignment lanes plus the current explicit-`Call` positional/default-member subset across both parenthesized and no-paren forms and the current bare statement-context positional/default-member subset on host-returned COM-backed objects,
