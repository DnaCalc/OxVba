...
- active-project same-name `Application` precedence is now also explicit for the matching imported scalar, named-argument, and positional read-assignment lanes plus the current explicit-`Call` and bare statement-context positional/default-member subsets on host-returned COM-backed objects,
