...
56. `RuntimeValue` now carries `BindingHandle(...)` semantically and `DynamicLinkHal::bind_descriptor_value(...)` returns that binding identity on the semantic path instead of flattening it into plain `I32`.
