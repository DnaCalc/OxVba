...
- `ComBinding` now retains the projected runtime class descriptor for typelib-backed native COM bindings, and retained runtime `ObjectRef` identities use that descriptor when created. Host snapshot canonicalization now preserves the first observed descriptor-bearing `ObjectRef` for a handle instead of replacing it with a descriptorless compatibility identity.
- `typelib_metadata_projects_to_runtime_dispatch_descriptor` covers the standalone projection; `binding_from_typelib_metadata_carries_runtime_descriptor_projection` covers the binding-level connection; `early_bound_project_executes_registered_scripting_dictionary_member_subset` now asserts the runtime `ObjectRef` for a real registered `Scripting.Dictionary` carries `IDispatch` descriptor metadata for `Count` and `Exists`.
