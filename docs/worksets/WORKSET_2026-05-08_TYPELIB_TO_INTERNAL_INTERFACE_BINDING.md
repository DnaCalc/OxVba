...
- Compiler-generated imported COM rewrites use an internal `__OxVbaEarlyInvoke(...)` marker during backend compilation and emit `Instruction::IntrinsicDispatchInvokeHost { early_bound: true, com_member: Some(...), ... }`, preserving dispatch-id/name selector and arity metadata in bytecode. Explicit user `DispatchInvoke(...)` remains `early_bound: false` mixed/late-bound traffic with no compiler-generated COM member descriptor (`compile_project_preserves_user_authored_dispatchinvoke_as_late_bound_metadata`).
...
- Validation after early-bound bytecode metadata: `cargo test -p oxvba-compiler --quiet` -> 818 passed; `cargo test -p oxvba-vm --quiet` -> 7 passed; `cargo test -p oxvba-jit --quiet` -> 8 passed; strict Access/JET host test -> 1 passed; full `cargo test -p oxvba-host --test com_early_project_end_to_end -- --test-threads=1` -> 122 passed.
- Validation after COM member descriptor metadata: `cargo test -p oxvba-compiler --quiet` -> 818 passed; `cargo test -p oxvba-vm --quiet` -> 7 passed; `cargo test -p oxvba-jit --quiet` -> 8 passed; strict Access/JET host test -> 1 passed.
- Validation after explicit user `DispatchInvoke` metadata guard: `cargo test -p oxvba-compiler --quiet` -> 819 passed.
