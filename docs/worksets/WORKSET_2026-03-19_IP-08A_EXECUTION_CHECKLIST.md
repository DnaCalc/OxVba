...
- neighboring COM-backed object handles also do not steal host-backed `WithEvents` ownership in the current subset; a bound host-backed `Emitter` still owns host event routing while a sibling `CreateObject("OxVba.TestDispatch")` handle remains non-routing on the same host event ingress path
- host-injected root getters may now also return bounded COM-backed objects in the current subset; a supported `Application.Value` getter can return `CreateObject("OxVba.TestDispatch")` and the caller can feed that returned object through `DispatchInvoke` on the shared object/value model
- the same bounded host/COM coexistence floor now also covers a first imported early-bind handoff; a supported `Application.Value` getter may return `CreateObject("OxVba.TestDispatch")` into `Dim obj As OxVba.TestDispatch` and the caller may execute `obj.Count()` through the imported metadata-backed member path
- the same bounded host/COM coexistence floor now also proves that a conflicting same-name plain-project `Application` reference does not steal that first imported early-bind handoff by reference order; `HostProject.Application.Value` still wins, returns `CreateObject("OxVba.TestDispatch")`, and feeds the bounded `obj.Count()` lane
- the same bounded host/COM coexistence floor now also covers imported property traffic on that returned COM object; a supported `Application.Value` getter may return `CreateObject("OxVba.TestDispatch")` into `Dim obj As OxVba.TestDispatch`, execute `obj.SetValue = 9`, and then execute `afterValue = obj.Value` on the shared object/value model
- the same bounded host/COM coexistence floor now also covers imported authoritative default-member traffic on that returned COM object; a supported `Application.Value` getter may return `CreateObject("OxVba.TestDispatch")` into `Dim obj As OxVba.TestDispatch` and then execute `echoValue = obj(41)` on the shared object/value model
- the same bounded host/COM coexistence floor now also covers imported object-result assignment-intent traffic on that returned COM object; a supported `Application.Value` getter may return `CreateObject("OxVba.TestDispatch")` into `Dim obj As OxVba.TestDispatch`, and `ReturnSelfDispatch()` / `ReturnSelfUnknown()` now preserve bounded object rebinding through explicit `Set` on `Object` targets plus implicit / explicit-`Let` assignment on `Variant` targets
- the same bounded host/COM coexistence floor now also covers imported zero-arg object-result assignment-intent traffic without parentheses on that returned COM object; a supported `Application.Value` getter may return `CreateObject("OxVba.TestDispatch")` into `Dim obj As OxVba.TestDispatch`, and `ReturnSelfDispatch` / `ReturnSelfUnknown` now preserve the same bounded object rebinding through explicit `Set` on `Object` targets plus implicit / explicit-`Let` assignment on `Variant` targets
- the same bounded host/COM coexistence floor now also covers imported object-valued zero-arg `PropertyGet` assignment-intent traffic on that returned COM object; a supported `Application.Value` getter may return `CreateObject("OxVba.TestDispatch")` into `Dim obj As OxVba.TestDispatch`, and `SelfDispatch` / `SelfUnknown` now preserve bounded object rebinding through explicit `Set` on `Object` targets plus implicit / explicit-`Let` assignment on `Variant` targets
- the same bounded host/COM coexistence floor now also proves that a conflicting same-name plain-project `Application` reference does not steal that imported object-valued `PropertyGet` handoff by reference order; `HostProject.Application.Value` still wins, returns `CreateObject("OxVba.TestDispatch")`, and feeds the bounded `SelfDispatch` / `SelfUnknown` assignment-intent lanes
- the same bounded host identity floor now also proves that a conflicting same-name plain-project `Application` reference does not steal imported zero-arg object-result rebinding without parentheses on that returned COM object by reference order; `HostProject.Application.Value` still wins, returns `CreateObject("OxVba.TestDispatch")`, and preserves the bounded `ReturnSelfDispatch` / `ReturnSelfUnknown` assignment-intent lanes
- the same bounded host/COM coexistence floor now also covers parenthesized imported object-valued zero-arg `PropertyGet` assignment-intent traffic on that returned COM object; a supported `Application.Value` getter may return `CreateObject("OxVba.TestDispatch")` into `Dim obj As OxVba.TestDispatch`, and `SelfDispatch()` / `SelfUnknown()` now preserve bounded object rebinding through explicit `Set` on `Object` targets plus implicit / explicit-`Let` assignment on `Variant` targets
- the same bounded host/COM coexistence floor now also covers imported `RaiseException` `Call` plus bare statement forms on that returned COM object; a supported `Application.Value` getter may return `CreateObject("OxVba.TestDispatch")` into `Dim obj As OxVba.TestDispatch`, and `Call obj.RaiseException()`, `obj.RaiseException()`, `Call obj.RaiseException`, and `obj.RaiseException` now all surface the controlled `com-dispatch-exception-raised` fault on the shared object/value model
- the same bounded host/COM coexistence floor now also covers imported no-paren positional `Call` and bare statement-context invocation subsets on that returned COM object; a supported `Application.Value` getter may return `CreateObject("OxVba.TestDispatch")` into `Dim obj As OxVba.TestDispatch`, and `Call obj.Count`, `Call obj.Exists 42`, `Call obj.Lookup 42`, `Call obj.Value`, `Call obj 42`, `obj.Exists 42`, `obj.Lookup 42`, and `obj 42` now execute through the same imported metadata-backed method/property/default-member path on the shared object/value model
- the same bounded host/COM coexistence floor now also proves that a conflicting same-name plain-project `Application` reference does not steal those imported `RaiseException` invoke forms by reference order; `HostProject.Application.Value` still wins, returns `CreateObject("OxVba.TestDispatch")`, and surfaces the same controlled exception payload across all four bounded invoke forms
- the same bounded host identity floor now also proves that a conflicting same-name plain-project `Application` reference does not steal those imported no-paren positional `Call` and statement-context lanes by reference order; `HostProject.Application.Value` still wins, returns `CreateObject("OxVba.TestDispatch")`, and preserves the same bounded no-paren invoke subset
- the same bounded host/COM coexistence floor now also covers imported parenthesized positional `Call` and bare statement-context invocation subsets on that returned COM object; a supported `Application.Value` getter may return `CreateObject("OxVba.TestDispatch")` into `Dim obj As OxVba.TestDispatch`, and `Call obj.Count()`, `Call obj.Exists(42)`, `Call obj.Lookup(42)`, `Call obj.Value()`, `Call obj(42)`, `obj.Count()`, `obj.Exists(42)`, `obj.Lookup(42)`, `obj.Value()`, and `obj(42)` now execute through the same imported metadata-backed method/property/default-member path on the shared object/value model
- the same bounded host identity floor now also proves that a conflicting same-name plain-project `Application` reference does not steal those imported parenthesized positional `Call` and statement-context lanes by reference order; `HostProject.Application.Value` still wins, returns `CreateObject("OxVba.TestDispatch")`, and preserves the same bounded parenthesized invoke subset
- the same bounded host/COM coexistence floor now also covers imported named-argument `Call` and bare statement-context invocation subsets on that returned COM object across both parenthesized and no-paren forms; a supported `Application.Value` getter may return `CreateObject("OxVba.TestDispatch")` into `Dim obj As OxVba.TestDispatch`, and `Call obj.SumPair(rhs := 14, lhs := 3)`, `Call obj.LookupPair(rhs := 9, lhs := 5)`, `Call obj(value := 41)`, `obj.SumPair(rhs := 14, lhs := 3)`, `obj.LookupPair(rhs := 9, lhs := 5)`, `obj(value := 41)`, `Call obj.SumPair rhs := 14, lhs := 3`, `Call obj.LookupPair rhs := 9, lhs := 5`, `Call obj value := 41`, `obj.SumPair rhs := 14, lhs := 3`, `obj.LookupPair rhs := 9, lhs := 5`, and `obj value := 41` now execute through the same imported metadata-backed method/property/default-member path on the shared object/value model
- the same bounded host identity floor now also proves that a conflicting same-name plain-project `Application` reference does not steal those imported named-argument invoke lanes by reference order; `HostProject.Application.Value` still wins, returns `CreateObject("OxVba.TestDispatch")`, and preserves the same bounded parenthesized and no-paren named-argument subsets
- the same bounded host/COM coexistence floor now also covers imported indexed `PropertyPut` plus indexed `PropertyPutRef` traffic on that returned COM object across both positional and named-argument forms; a supported `Application.Value` getter may return `CreateObject("OxVba.TestDispatch")` into `Dim obj As OxVba.TestDispatch`, and `obj.SetIndexedValue(7) = 11`, `obj.SetIndexedValue(lhs := 7) = 11`, `Set obj.SetIndexedValueRef(8) = other`, and `Set obj.SetIndexedValueRef(lhs := 8) = other` now execute through the same imported metadata-backed setter path on the shared object/value model
- the same bounded host identity floor now also proves that a conflicting same-name plain-project `Application` reference does not steal those imported indexed setter lanes by reference order; `HostProject.Application.Value` still wins, returns `CreateObject("OxVba.TestDispatch")`, and preserves the same bounded positional and named-argument indexed setter subsets
...
- the same bounded host/COM coexistence floor now also covers assignment-form imported `PropertyPutRef` traffic on that returned COM object; a supported `Application.Value` getter may return `CreateObject("OxVba.TestDispatch")` into `Dim obj As OxVba.TestDispatch`, execute `Set other = CreateObject("OxVba.TestDispatch")`, execute `Set obj.SetValueRef = other`, and then preserve the deterministic shared-model getter witness `afterSetValueRef = obj.Value`
- the same bounded host/COM coexistence floor now also proves that a conflicting same-name plain-project `Application` reference does not steal that imported `PropertyPutRef` handoff by reference order; `HostProject.Application.Value` still wins, returns `CreateObject("OxVba.TestDispatch")`, and preserves the same bounded `Set obj.SetValueRef = other` lane
- the same bounded host identity floor now also proves that a conflicting same-name plain-project `Application` reference does not steal imported property-put/get traffic on that returned COM object by reference order; `HostProject.Application.Value` still wins, returns `CreateObject("OxVba.TestDispatch")`, and preserves the bounded `obj.SetValue = 9 : afterValue = obj.Value` lane
- the same bounded host identity floor now also proves that a conflicting same-name plain-project `Application` reference does not steal imported authoritative default-member traffic on that returned COM object by reference order; `HostProject.Application.Value` still wins, returns `CreateObject("OxVba.TestDispatch")`, and preserves the bounded `echoValue = obj(41)` lane
- the same bounded host identity floor now also proves that a conflicting same-name plain-project `Application` reference does not steal imported object-result rebinding on that returned COM object by reference order; `HostProject.Application.Value` still wins, returns `CreateObject("OxVba.TestDispatch")`, and preserves the bounded `ReturnSelfDispatch()` / `ReturnSelfUnknown()` assignment-intent lanes
- the same bounded host identity floor now also proves that a conflicting same-name plain-project `Application` reference does not steal parenthesized imported object-valued `PropertyGet` traffic on that returned COM object by reference order; `HostProject.Application.Value` still wins, returns `CreateObject("OxVba.TestDispatch")`, and preserves the bounded `SelfDispatch()` / `SelfUnknown()` assignment-intent lanes
