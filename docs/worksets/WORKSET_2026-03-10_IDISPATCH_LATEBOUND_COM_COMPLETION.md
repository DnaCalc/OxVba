...
Current gap:
1. the implemented lane is still centered around a constrained semantic carrier and only the current scalar/string/object/one-dimensional SAFEARRAY subset is covered; outbound VBA `Array(...)` expressions now preserve semantic one-dimensional `VT_ARRAY | VT_VARIANT` payloads into the COM boundary, but broader SAFEARRAY element/object/multi-dimensional parity is still open.
