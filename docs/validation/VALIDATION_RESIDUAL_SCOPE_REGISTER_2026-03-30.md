...
| `LANG-0002` | `in-progress` | remaining accepted scope | object-enumerator `For Each` breadth remains open beyond the currently proved project-dynamic/imported-collection, imported-COM, direct session-invocation, direct-session VM/JIT parity, registered `OxVba.TestDispatch` transport, registered direct-session VM/JIT, and loaded Excel-imported direct-session VM/JIT slices | `bd-cyr.2.13` |
