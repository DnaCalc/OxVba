...
| `LANG-0002` | `in-progress` | remaining accepted scope | object-enumerator `For Each` breadth remains open beyond the currently proved project-dynamic/imported-collection, imported-COM, direct session-invocation, direct-session VM/JIT parity, registered `OxVba.TestDispatch` transport, registered direct-session VM/JIT, and loaded Excel-imported direct/bundle session VM/JIT slices | `bd-cyr.2.15` |
...
| `LANG-0004` | `implemented-subset` | remaining accepted scope | broader late-bound default-member dispatch breadth remains open beyond the current one-visible-argument named/positional, runtime-string selector, positional call-form selector, named call-form selector, named statement-context selector, and positional statement-context selector subset | `bd-cyr.2.15` |
...
| `COM-0004` | `verified` | remaining accepted scope | broader real-library activation authority remains open beyond the current registered-host subset and newly active loaded `.basproj` resolvable-ordering subset | `bd-cyr.3.13` |
| `COM-0005` | `verified` | remaining accepted scope | broader dual-interface behavior remains open outside the mixed-server supported subset | `bd-cyr.3.13` |
| `COM-0006` | `verified` | remaining accepted scope | broader typelib/versioned-reference behavior remains open outside the current file-backed subset and newly active loaded `.basproj` ordering/unresolved-diagnostic subset, especially for broken-first/later-valid tolerance | `bd-cyr.3.13` |
