...
| `LANG-0002` | `in-progress` | remaining accepted scope | object-enumerator `For Each` breadth remains open beyond the currently proved project-dynamic/imported-collection, imported-COM, direct session-invocation, direct-session VM/JIT parity, and registered `OxVba.TestDispatch` slices | `bd-cyr.2.9` |
...
| `LANG-0004` | `implemented-subset` | remaining accepted scope | broader late-bound default-member dispatch breadth remains open beyond the current one-visible-argument named/positional, runtime-string selector, positional call-form selector, and named call-form selector subset | `bd-cyr.2.10` |
| `COM-0001` | `implemented-subset` | remaining accepted scope | broader late-bound COM invocation and marshalling behavior remains outside the proved scalar/object/array subset, after landing the wide-runtime `I64 -> VT_I8` scalar and variant-array normalization slice plus the registered late-bound arity-error, member-not-found, and `OxVba.TestEventServer` scalar-argument marshaling lanes | `bd-cyr.3.10` |
