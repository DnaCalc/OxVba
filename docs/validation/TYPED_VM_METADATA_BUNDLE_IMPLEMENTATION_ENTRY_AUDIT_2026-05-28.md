...
| Selected VM-consumption residuals | `VBA_VM_CONSUMPTION_EVIDENCE_SEED_TABLE_V1.csv` records Optional `Variant` missing, non-selected error/deopt cleanup, COM/native boundary instruction execution, and host-policy behavior as explicit strict unsupported rows; selected native, early-COM selector, and exported-callable descriptor identity rows remain supported metadata slices. |
