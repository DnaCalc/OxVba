...
must not turn source generation, JIT subset support, or registration skeletons
...
| NATIVE-V02-O003 | JIT fallback | Keep Cranelift as an accelerator with VM fallback semantics. | Scaffold runs supported-subset and fallback JIT tests. |
...
- Cranelift evidence may support accelerator claims only for named supported
  bytecode subsets.
