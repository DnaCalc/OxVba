...
- `PH-0010` remains `in-progress` for MS-OVBA storage roundtrip. Local loader/generator tests now verify the supported `.basproj` and VBP adapter subset. The Foundation extraction run `20260301-ms-ovba-pass01` yielded 6 spec items and 0 conformance candidates, and the oracle-evidence registration is captured in `docs/evidence/conformance/oracle_captures/ms_ovba_storage_extraction_20260301T203020Z/summary.md`; the remaining oracle-evidence lane is split to `bd-gm3.12.8.1`.
...
- the remaining imported-runtime parity tail was rolled into `bd-gm3.14.4` and `bd-gm3.14.4.1`, and `bd-gm3.14.4.1` is now closed by the refreshed oracle run `vba_attribute_oracle_20260329T233125Z` plus import/export roundtrip evidence showing Excel drops imported member-attribute lines on re-export, formally bounding the residual to the VBIDE import-retention boundary.
...
3. keep the refreshed oracle evidence linked so the imported Excel `ERR|438` gap stays explicitly bounded to the VBIDE import-retention boundary rather than drifting back into a generic runtime-mismatch claim,
