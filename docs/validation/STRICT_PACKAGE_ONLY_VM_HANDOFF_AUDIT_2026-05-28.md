...
| Legacy bundle format | `FORMAT_VERSION = 15`; serialized versions 1 through 14 reject with `unsupported legacy bundle version ...`, and current-version bundles missing strict sections reject with `BUNDLE-STRICT-MISSING-SECTIONS`. |
...
```powershell
rg -n "FORMAT_VERSION: u32 = 15|unsupported legacy bundle version|BUNDLE-STRICT-MISSING-SECTIONS" crates/oxvba-compiler/src/bundle.rs docs -g"*.rs" -g"*.md"
```
