...
`matrix-scaffold` and rollout-owned planned rows are directed-rollout states only. When a rollout bead closes, neither may remain. A capability epic must have an exact-row delivery successor; a support-only review, authority, rollout or terminal epic must instead have an exact-row support successor and explicit downstream capability paths. Before switching to AutoRun, all 15 matrices are nonempty, each has at least one required row, and every one of the 42 execution epics has an explicit row relationship.

A capability execution epic closes only after it has a delivery leaf, every required row it owns or advances is `verified`, and neither those rows nor any descendant residual owner retain remaining accepted scope. The manifest declares each epic's authoritative `effect`: `delivery` requires a delivery label and delivery proof, while `support` requires support-only labels and may close on exact-row support outcomes because its stated product is control/review rather than behavior delivery. A support epic must still verify every owned required row, clear its residuals, and leave believable downstream delivery paths. Support work never closes capability scope, and label edits cannot change the manifest classification.

An open AutoRun program is live when it has a ready leaf or a claimed `in_progress` leaf whose own and ancestor `blocks` dependencies are closed. A forced or stale blocked claim cannot mask an empty queue. At most three executable leaves may be active at once, matching the three-worker control plane; narrower resource limits still apply.
