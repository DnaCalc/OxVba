//! Structural intrinsics — compiler-internal plumbing that is NOT VBA library
//! surface (`Nothing`/`Null`/omitted-arg literals, `WithEvents` get/set, dynamic
//! dispatch, `VarPtr`/`StrPtr`/`ObjPtr`). Ported verbatim from the legacy
//! `frontend_structural_intrinsics`.
