...
mod tests {
    ...
    #[test]
    fn formal_host_project_active_project_application_wins_over_host_injected_root_receiver() {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            (
                "host predeclared",
    ...
    #[test]
    fn formal_host_project_active_project_application_returning_com_object_wins_over_host_injected_root_read_assignments()
     {
        let engine = Engine::new(HostConfig::default());
        let cases = [
    ...
    #[test]
    fn formal_host_project_active_project_application_returning_com_object_wins_over_host_injected_root_call_statements()
     {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("host predeclared", "Attribute VB_PredeclaredId = True"),
    ...
    #[test]
    fn formal_host_project_active_project_application_returning_com_object_wins_over_host_injected_root_statement_context()
     {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("host predeclared", "Attribute VB_PredeclaredId = True"),
    ...
    #[test]
    fn formal_host_project_active_project_application_returning_com_object_wins_over_host_injected_root_no_paren_call_statements()
     {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("host predeclared", "Attribute VB_PredeclaredId = True"),
    ...
    #[test]
    fn formal_host_project_active_project_application_returning_com_object_wins_over_host_injected_root_no_paren_statement_context()
     {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("host predeclared", "Attribute VB_PredeclaredId = True"),
    ...
    #[test]
    fn formal_host_project_active_project_application_returning_com_object_wins_over_host_injected_root_named_argument_call_statements()
     {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("host predeclared", "Attribute VB_PredeclaredId = True"),
    ...
    #[test]
    fn formal_host_project_active_project_application_returning_com_object_wins_over_host_injected_root_named_argument_statement_context()
     {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("host predeclared", "Attribute VB_PredeclaredId = True"),
    ...
    #[test]
    fn formal_host_project_active_project_application_returning_com_object_wins_over_host_injected_root_no_paren_named_argument_call_statements()
     {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("host predeclared", "Attribute VB_PredeclaredId = True"),
    ...
    #[test]
    fn formal_host_project_active_project_application_returning_com_object_wins_over_host_injected_root_no_paren_named_argument_statement_context()
     {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("host predeclared", "Attribute VB_PredeclaredId = True"),
    ...
    #[test]
    fn formal_host_project_active_project_application_returning_com_object_property_put_and_get_wins_over_host_injected_root()
     {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("host predeclared", "Attribute VB_PredeclaredId = True"),
        ...
        for (label, host_attr) in cases {
            ...
            let snapshot = engine
                ...
                .unwrap_or_else(|err| {
                    panic!(
                        "{label} active-project Application should outrank same-name host-injected root on host-returned COM property-put/get traffic: {err}"
                    )
                });
            match snapshot.as_slice() {
                [
    ...
    #[test]
    fn formal_host_project_active_project_application_returning_com_object_indexed_property_setters_win_over_host_injected_root()
     {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("host predeclared", "Attribute VB_PredeclaredId = True"),
        ...
        for (label, host_attr) in cases {
            ...
            match snapshot.as_slice() {
                [
                    ...
                    && *after_set_indexed_value_ref == 5_013
                    && *after_set_named_indexed_value_ref == 5_013 => {}
                other => panic!(
                    "{label}: expected active-project Application to own the returned COM indexed setter handoff and preserve deterministic getter witness 5013 on handles 5004/5004 instead of failing on the scalar host root, got: {other:?}"
                ),
            }
        }
    }
    ...
    #[test]
    fn formal_host_project_active_project_application_returning_com_object_exception_invoke_forms_win_over_host_injected_root()
     {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("host predeclared", "Attribute VB_PredeclaredId = True"),
        ...
        for (label, host_attr) in cases {
            for (invoke_label, invoke_line) in invoke_cases {
                ...
                assert!(
                    ...
                    "{label} {invoke_label}: expected stable imported exception prefix, got: {}",
                    err.message()
                );
                assert!(
                    err.message()
                    ...
                    "{label} {invoke_label}: expected imported EXCEPINFO source/description, got: {}",
                    err.message()
                );
            }
        }
    ...
    #[test]
    fn formal_host_project_host_injected_object_property_get_variant_assignment_intents_execute() {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            (
        ...
        for (label, exposure_attr, assignment_line, expected_line) in cases {
            ...
            let snapshot = engine
                ...
                .unwrap_or_else(|err| {
                    panic!(
                        "{label} host-injected object property-get to Variant should execute: {err}"
                    )
                });
            assert_eq!(
                snapshot.first(),
    ...
    #[test]
    fn formal_host_project_host_injected_parenthesized_object_property_get_variant_assignment_intents_execute()
     {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            (
        ...
        for (label, exposure_attr, assignment_line, expected_line) in cases {
            ...
            let snapshot = engine
                ...
                .unwrap_or_else(|err| {
                    panic!(
                        "{label} host-injected parenthesized object property-get to Variant should execute: {err}"
                    )
                });
            assert_eq!(
                snapshot.first(),
    ...
    #[test]
    fn formal_host_project_host_injected_parenthesized_object_default_member_variant_assignment_intents_execute()
     {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            (
        ...
        for (label, exposure_attr, assignment_line, expected_line) in cases {
            ...
            let snapshot = engine
                ...
                .unwrap_or_else(|err| {
                    panic!(
                        "{label} host-injected parenthesized object default-member get to Variant should execute: {err}"
                    )
                });
            assert_eq!(
                snapshot.first(),
    ...
    #[test]
    fn formal_host_project_event_ingress_does_not_route_com_object_handle() {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("predeclared", "Attribute VB_PredeclaredId = True"),
            ("global namespace", "Attribute VB_GlobalNamespace = True"),
        ...
        for (label, exposure_attr) in cases {
            ...
            let (bound_handle, com_handle) = match snapshot.as_slice() {
                [bound, com_obj, _sink] => {
                    ...
                    match (bound_handle, com_handle) {
                        (Some(bound_handle), Some(com_handle))
                            if bound_handle.raw() != com_handle.raw() =>
                        {
                            (bound_handle, com_handle)
                        }
                        _ => panic!(
                            "{label} should snapshot distinct host-backed and COM-backed object handles: {snapshot:?}"
                        ),
                    }
                }
                _ => panic!(
                    "{label} should snapshot bound host-backed object, COM-backed object, and sink instance: {snapshot:?}"
                ),
            };
            ...
            let unmatched = engine
                ...
                .unwrap_or_else(|err| {
                    panic!("{label} COM-backed object should not fail host event ingress: {err}")
                });
            assert!(
                unmatched,
                "{label}: host-backed binding set should still exist while COM-backed handle stays non-routing"
            );
            ...
            assert!(
                err.message().contains("runtime error: 204"),
                "{label}: host-backed source should route callback, got: {}",
                err.message()
            );
        }
    }
    ...
    #[test]
    fn formal_host_project_one_arg_event_ingress_routes_only_bound_host_backed_source_handle() {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("predeclared", "Attribute VB_PredeclaredId = True"),
            ("global namespace", "Attribute VB_GlobalNamespace = True"),
        ...
        for (label, exposure_attr) in cases {
            ...
            let (bound_handle, other_handle) = match snapshot.as_slice() {
                [bound, other, _sink] => {
                    ...
                    match (bound_handle, other_handle) {
                        ...
                        _ => panic!(
                            "{label} should snapshot distinct bound and unbound one-arg host-backed emitters: {snapshot:?}"
                        ),
                    }
                }
                _ => panic!(
                    "{label} should snapshot bound and unbound one-arg host-backed emitters: {snapshot:?}"
                ),
            };
            ...
            let unmatched = engine
                ...
                .unwrap_or_else(|err| {
                    panic!(
                        "{label} unmatched one-arg host-backed event should not fail ingress: {err}"
                    )
                });
            assert!(
                unmatched,
                "{label}: one-arg host-backed event source should still resolve a binding set"
            );
            ...
            assert_eq!(err.phase(), DiagnosticPhase::Runtime);
            assert!(
                err.message().contains("runtime error: 242"),
                "{label}: bound one-arg host-backed source should route callback with its forwarded argument, got: {}",
                err.message()
            );
        }
    }
    ...
    #[test]
    fn formal_host_project_one_arg_withevents_prefers_host_injected_source_over_plain_project_name_match()
     {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("predeclared", "Attribute VB_PredeclaredId = True"),
        ...
        for (label, exposure_attr) in cases {
            ...
            let bound_handle = match snapshot.as_slice() {
                ...
                _ => panic!(
                    "{label} should snapshot bound one-arg host-backed emitter plus sink instance: {snapshot:?}"
                ),
            };
            ...
            let plain_project_routed = engine
                ...
                .unwrap_or_else(|err| {
                    panic!(
                        "{label} plain project one-arg event ingress should stay non-routing: {err}"
                    )
                });
            assert!(
                !plain_project_routed,
                "{label}: plain project type-name match must not own one-arg host-backed WithEvents routing"
            );
            ...
            let err = engine
                ...
                .expect_err(
                    "host-injected one-arg source should own the conflicting WithEvents route",
                );
            assert_eq!(err.phase(), DiagnosticPhase::Runtime);
            assert!(
                err.message().contains("runtime error: 244"),
                "{label}: host-injected one-arg source should route callback, got: {}",
                err.message()
            );
        }
    }
    ...
    #[test]
    fn formal_host_project_one_arg_event_ingress_rejects_more_than_one_forwarded_arg() {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("predeclared", "Attribute VB_PredeclaredId = True"),
            ("global namespace", "Attribute VB_GlobalNamespace = True"),
        ...
        for (label, exposure_attr) in cases {
            ...
            let err = engine
                ...
                .expect_err(
                    "host-backed one-arg event ingress should reject more than one forwarded arg",
                );
            assert_eq!(err.phase(), DiagnosticPhase::Runtime);
            assert!(
                err.message().contains("PMR-E-HOST-EVENT-ARITY-UNSUPPORTED"),
                "{label}: unexpected host-backed one-arg ingress diagnostic: {}",
                err.message()
            );
        }
    }
    ...
    #[test]
    fn formal_host_project_root_can_return_com_backed_object_for_dispatch_invoke() {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("predeclared", "Attribute VB_PredeclaredId = True"),
            ("global namespace", "Attribute VB_GlobalNamespace = True"),
    ...
    #[test]
    fn formal_host_project_root_returning_com_object_wins_over_same_name_plain_project_match() {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("predeclared", "Attribute VB_PredeclaredId = True"),
            ("global namespace", "Attribute VB_GlobalNamespace = True"),
    ...
    #[test]
    fn formal_host_project_root_returned_com_object_supports_imported_property_put_and_get() {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("predeclared", "Attribute VB_PredeclaredId = True"),
            ("global namespace", "Attribute VB_GlobalNamespace = True"),
        ...
        for (label, exposure_attr) in cases {
            ...
            let snapshot = engine
                ...
                .unwrap_or_else(|err| {
                    panic!(
                        "{label} host root should support imported property getter/setter traffic on the returned COM object: {err}"
                    )
                });
            match snapshot.as_slice() {
                [
    ...
    fn formal_host_project_root_returned_com_object_supports_imported_property_putref_and_get() {
        ...
        for (label, exposure_attr) in cases {
            ...
            let snapshot = engine
                ...
                .unwrap_or_else(|err| {
                    panic!(
                        "{label} host root should support imported property-putref traffic on the returned COM object: {err}"
                    )
                });
            match snapshot.as_slice() {
                [
    ...
    #[test]
    fn formal_host_project_root_returned_com_object_supports_imported_default_member_call() {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("predeclared", "Attribute VB_PredeclaredId = True"),
            ("global namespace", "Attribute VB_GlobalNamespace = True"),
        ...
        for (label, exposure_attr) in cases {
            ...
            let snapshot = engine
                ...
                .unwrap_or_else(|err| {
                    panic!(
                        "{label} host root should support imported default-member traffic on the returned COM object: {err}"
                    )
                });
            match snapshot.as_slice() {
                [
    ...
    #[test]
    fn formal_host_project_root_returned_com_object_default_member_call_wins_over_same_name_plain_project_match()
     {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("predeclared", "Attribute VB_PredeclaredId = True"),
    ...
    #[test]
    fn formal_host_project_root_returned_com_object_supports_imported_object_result_assignment_intents()
     {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("predeclared", "Attribute VB_PredeclaredId = True"),
        ...
        for (label, exposure_attr) in cases {
            ...
            let snapshot = engine
                ...
                .unwrap_or_else(|err| {
                    panic!(
                        "{label} host root should preserve imported object-result rebinding on the returned COM object: {err}"
                    )
                });
            match snapshot.as_slice() {
                [
                ...
                other => panic!(
                    "{label}: expected host-root COM object-result rebinding to preserve object handles plus Count() results tied to each returned handle, got: {other:?}"
                ),
            }
        }
    }
    ...
    #[test]
    fn formal_host_project_root_returned_com_object_object_results_win_over_same_name_plain_project_match()
     {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("predeclared", "Attribute VB_PredeclaredId = True"),
        ...
        for (label, exposure_attr) in cases {
            ...
            match snapshot.as_slice() {
                [
                    ...
                    && *wrapped_unknown_count == wrapped_unknown.raw() + 1 => {}
                other => panic!(
                    "{label}: expected host-injected Application.Value to win over the same-name plain project and preserve imported object-result rebinding plus Count() results tied to each returned handle, got: {other:?}"
                ),
            }
        }
    }
    ...
    #[test]
    fn formal_host_project_root_returned_com_object_property_gets_win_over_same_name_plain_project_match()
     {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("predeclared", "Attribute VB_PredeclaredId = True"),
        ...
        for (label, exposure_attr) in cases {
            ...
            match snapshot.as_slice() {
                [
                    ...
                    && *wrapped_unknown_count == wrapped_unknown.raw() + 1 => {}
                other => panic!(
                    "{label}: expected host-injected Application.Value to win over the same-name plain project and preserve imported object property-get rebinding plus Count() results tied to each returned handle, got: {other:?}"
                ),
            }
        }
    }
    ...
    #[test]
    fn formal_host_project_root_returned_com_object_property_putref_wins_over_same_name_plain_project_match()
     {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("predeclared", "Attribute VB_PredeclaredId = True"),
    ...
    #[test]
    fn formal_host_project_root_returned_com_object_supports_imported_indexed_property_setters() {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("predeclared", "Attribute VB_PredeclaredId = True"),
            ("global namespace", "Attribute VB_GlobalNamespace = True"),
        ...
        for (label, exposure_attr) in cases {
            ...
            let snapshot = engine
                ...
                .unwrap_or_else(|err| {
                    panic!(
                        "{label} host root should support imported indexed setter traffic on the returned COM object: {err}"
                    )
                });
            match snapshot.as_slice() {
                [
                    ...
                    && *after_set_named_indexed_value_ref == 5_013 => {}
                other => panic!(
                    "{label}: expected host-returned indexed setter traffic to stay on the shared-model COM handle path with deterministic getter witness 5013 on handles 5004/5004, got: {other:?}"
                ),
            }
        }
    }
    ...
    #[test]
    fn formal_host_project_root_returned_com_object_indexed_property_setters_win_over_same_name_plain_project_match()
     {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("predeclared", "Attribute VB_PredeclaredId = True"),
        ...
        for (label, exposure_attr) in cases {
            ...
            match snapshot.as_slice() {
                [
                    ...
                    && *after_set_named_indexed_value_ref == 5_013 => {}
                other => panic!(
                    "{label}: expected host-injected Application.Value to win over the same-name plain project and preserve the shared-model indexed setter witness 5013, got: {other:?}"
                ),
            }
        }
    }
    ...
    #[test]
    fn formal_host_project_root_returned_com_object_supports_parenthesized_imported_object_property_get_assignment_intents()
     {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("predeclared", "Attribute VB_PredeclaredId = True"),
        ...
        for (label, exposure_attr) in cases {
            ...
            let snapshot = engine
                ...
                .unwrap_or_else(|err| {
                    panic!(
                        "{label} host root should preserve parenthesized imported object property-get rebinding on the returned COM object: {err}"
                    )
                });
            match snapshot.as_slice() {
                [
                    ...
                    && *wrapped_unknown_count == wrapped_unknown.raw() + 1 => {}
                other => panic!(
                    "{label}: expected host-root parenthesized object property-get rebinding to preserve object handles plus Count() results tied to each returned handle, got: {other:?}"
                ),
            }
        }
    }
    ...
    #[test]
    fn formal_host_project_root_returned_parenthesized_object_property_gets_win_over_same_name_plain_project_match()
     {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("predeclared", "Attribute VB_PredeclaredId = True"),
        ...
        for (label, exposure_attr) in cases {
            ...
            match snapshot.as_slice() {
                [
                    ...
                    && *wrapped_unknown_count == wrapped_unknown.raw() + 1 => {}
                other => panic!(
                    "{label}: expected host-injected Application.Value to win over the same-name plain project and preserve parenthesized imported object property-get rebinding plus Count() results tied to each returned handle, got: {other:?}"
                ),
            }
        }
    }
    ...
    #[test]
    fn formal_host_project_root_returned_com_object_reports_imported_exception_invoke_forms() {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("predeclared", "Attribute VB_PredeclaredId = True"),
            ("global namespace", "Attribute VB_GlobalNamespace = True"),
    ...
    #[test]
    fn formal_host_project_root_returned_com_object_supports_imported_no_paren_call_statements() {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("predeclared", "Attribute VB_PredeclaredId = True"),
            ("global namespace", "Attribute VB_GlobalNamespace = True"),
        ...
        for (label, exposure_attr) in cases {
            ...
            let snapshot = engine
                ...
                .unwrap_or_else(|err| {
                    panic!(
                        "{label} host root should preserve imported no-paren Call-form traffic on the returned COM object: {err}"
                    )
                });
            match snapshot.as_slice() {
                [
    ...
    #[test]
    fn formal_host_project_root_returned_no_paren_call_statements_win_over_same_name_plain_project_match()
     {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("predeclared", "Attribute VB_PredeclaredId = True"),
    ...
    #[test]
    fn formal_host_project_root_returned_com_object_supports_imported_no_paren_statement_context() {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("predeclared", "Attribute VB_PredeclaredId = True"),
            ("global namespace", "Attribute VB_GlobalNamespace = True"),
        ...
        for (label, exposure_attr) in cases {
            ...
            let snapshot = engine
                ...
                .unwrap_or_else(|err| {
                    panic!(
                        "{label} host root should preserve imported no-paren statement-context traffic on the returned COM object: {err}"
                    )
                });
            match snapshot.as_slice() {
                [
    ...
    #[test]
    fn formal_host_project_root_returned_no_paren_statement_context_win_over_same_name_plain_project_match()
     {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("predeclared", "Attribute VB_PredeclaredId = True"),
    ...
    #[test]
    fn formal_host_project_root_returned_com_object_supports_imported_call_statements() {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("predeclared", "Attribute VB_PredeclaredId = True"),
            ("global namespace", "Attribute VB_GlobalNamespace = True"),
        ...
        for (label, exposure_attr) in cases {
            ...
            let snapshot = engine
                ...
                .unwrap_or_else(|err| {
                    panic!(
                        "{label} host root should preserve imported Call-form traffic on the returned COM object: {err}"
                    )
                });
            match snapshot.as_slice() {
                [
    ...
    #[test]
    fn formal_host_project_root_returned_call_statements_win_over_same_name_plain_project_match() {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("predeclared", "Attribute VB_PredeclaredId = True"),
            ("global namespace", "Attribute VB_GlobalNamespace = True"),
    ...
    #[test]
    fn formal_host_project_root_returned_com_object_supports_imported_statement_context() {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("predeclared", "Attribute VB_PredeclaredId = True"),
            ("global namespace", "Attribute VB_GlobalNamespace = True"),
        ...
        for (label, exposure_attr) in cases {
            ...
            let snapshot = engine
                ...
                .unwrap_or_else(|err| {
                    panic!(
                        "{label} host root should preserve imported statement-context traffic on the returned COM object: {err}"
                    )
                });
            match snapshot.as_slice() {
                [
    ...
    #[test]
    fn formal_host_project_root_returned_statement_context_win_over_same_name_plain_project_match()
    {
        let engine = Engine::new(HostConfig::default());
        let cases = [
            ("predeclared", "Attribute VB_PredeclaredId = True"),
