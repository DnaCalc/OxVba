...
fn t4_dictionary_every_scalar_vt_round_trip() {
    ...
    let early = run_clean_with_references_prefer_vtable(&dict_early(body), scripting_ref());
    let do_run = run_clean_with_references_dispatch_only(&dict_early(body), scripting_ref());
    // 9 Add + 9 Item-get default-member reads = 18 vtable calls. GREEN since FU#1
    // invoke-kind keying: the 9 default-member `d("…")` reads ride `Item`'s enriched get
    // slot (the `(memid, invoke_kind)` key), alongside the 9 unambiguous-memid Adds.
...
fn t5_dictionary_unicode_astral_bstr_round_trip() {
    ...
    //
    // GREEN since FU#3 UTF-16 code-unit string representation: `ChrW(&HD83D) &
    // ChrW(&HDE00)` is the canonical VBA idiom for an astral char (two surrogate-HALF
    // UTF-16 units, each < 65536). OxVba now builds `ChrW` directly from the u16 unit
    // (not `char::from_u32`, which rejected the surrogate range) and concatenates `&`/`+`
    // at the CODE-UNIT level, so the lone halves survive to form the astral pair; `Len`
    // counts UTF-16 units off the BSTR. The value (verdict=7) agrees on every transport.
    let body = "Dim verdict As Long\n\
    ...
    let early = run_clean_with_references_prefer_vtable(&dict_early(body), scripting_ref());
    let do_run = run_clean_with_references_dispatch_only(&dict_early(body), scripting_ref());
    // 2 Add + 3 Item default-member gets (d("a") twice, d("b") once) = 5 vtable calls
    // (every get rides FU#1's invoke-kind-keyed `Item` slot; DispatchOnly leg = 0).
    assert_differential("T5", late, early, Some(do_run), find_verdict, 7, Some(5));
}
