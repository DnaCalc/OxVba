...
fn rgb_packs_and_clamps_components() {
    ...
    assert!(has_long(&snap, 16_777_215), "RGB(255,255,255)=16777215: {snap:?}");
    assert!(has_long(&snap, 65_536), "RGB(0,0,1)=65536: {snap:?}");
    // clamped is also 16777215 (covered above); the distinct mid value:
    assert!(has_long(&snap, 8_421_504), "RGB(128,128,128)=8421504: {snap:?}");
}
...
fn qbcolor_returns_the_legacy_palette() {
    ...
    assert!(has_long(&snap, 16_777_215), "QBColor(15)=16777215: {snap:?}");
}

#[test]
fn qbcolor_out_of_range_raises_5() {
    let outcome = run(Executor::Vm3, "Sub Main()\n    Dim x As Long\n    x = QBColor(99)\nEnd Sub\n");
    assert!(outcome.unsupported.is_none(), "unsupported: {:?}", outcome.unsupported);
    assert!(outcome.result.is_err(), "QBColor(99) should raise, got {:?}", outcome.result);
    assert_eq!(outcome.err.number, 5, "QBColor out-of-range → 5; err={:?}", outcome.err);
}
