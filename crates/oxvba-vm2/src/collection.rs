...
//!
//! Each `New Collection` instance's ordered, optionally string-keyed contents
//! live VM-side, keyed by the instance's `compat_identity` (see `Vm::collections`),
//! rather than inside the refcounted object box. Because a `Set c2 = c1` clones
//! the `ObjectRef` (same identity), both names map to the same entry — so VBA's
//! reference semantics fall out for free. Methods reach here by dispatching the
//! class's native-bodied procedures (`NativeMethodId::Collection*`).
//!
//! This is the data model only; the dispatch site maps [`CollectionError`] onto
//! VBA run-time error numbers (9 / 457 / 5) and builds [`Selector`]s from the
//! call arguments. Keys are case-insensitive (folded to ASCII lowercase).
