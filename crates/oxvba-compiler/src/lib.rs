...
mod tests {
    ...
    #[test]
    fn compile_with_runtime_metadata_default_routes_untyped_string_const_scalar_concat_through_hir()
    {
        let source = "Const Prefix = \"v\"\nConst CNumber = 7\nConst CFlag = True\nConst CText = Prefix & CNumber & CFlag\nSub Main()\nDim text\ntext = CText: text = text & \"!\"\nEnd Sub\n";
        let legacy_err = super::compile_with_runtime_metadata_legacy(source).expect_err(
            "legacy path should reject the active inline sequence after untyped scalar-concat String const",
        );
        assert!(
            legacy_err.to_string().contains("unsupported statement"),
            "unexpected legacy error: {legacy_err}"
        ...
        .expect("direct HIR production lowering should support untyped scalar-concat String const");
        let (bytecode, metadata) = super::compile_with_runtime_metadata(source).expect(
            "default runtime metadata compile should route untyped scalar-concat String const through HIR",
        );
        assert_eq!(
            hir.1, metadata,
            "default route metadata should come from HIR production for untyped scalar-concat String const"
        );
        assert!(
            bytecode.instructions.iter().any(|instruction| matches!(
                instruction,
            ...
            "expected untyped scalar-concat String const bytecode: {bytecode:#?}"
        );
    }
    ...
    #[test]
    fn compile_with_runtime_metadata_default_routes_typed_currency_date_consts_through_hir() {
        let source = "Const CAmount As Currency = 1.25@\nConst CStamp As Date = #2026-02-28#\nSub Main()\nDim amount As Currency\nDim stamp As Date\namount = CAmount: stamp = CStamp\nEnd Sub\n";
        let legacy_err = super::compile_with_runtime_metadata_legacy(source).expect_err(
            "legacy path should reject the active inline sequence after typed Currency/Date consts",
        );
        assert!(
            legacy_err.to_string().contains("unsupported statement"),
            "unexpected legacy error: {legacy_err}"
        ...
        let hir =
            super::frontend_hir_lowering::compile_source_with_runtime_metadata_via_hir(source)
                .expect("direct HIR production lowering should support typed Currency/Date consts");
        let (bytecode, metadata) = super::compile_with_runtime_metadata(source).expect(
            "default runtime metadata compile should route typed Currency/Date consts through HIR",
        );
        assert_eq!(
            hir.1, metadata,
            "default route metadata should come from HIR production for typed Currency/Date consts"
        );
        assert!(
            bytecode.instructions.iter().any(|instruction| matches!(
                instruction,
            ...
            "expected typed Currency const bytecode: {bytecode:#?}"
        );
        assert!(
            bytecode.instructions.iter().any(|instruction| matches!(
                instruction,
            ...
            "expected typed Date const bytecode: {bytecode:#?}"
        );
    }
    ...
    fn compile_with_runtime_metadata_default_routes_typed_boolean_const_through_hir() {
        ...
        let hir =
            super::frontend_hir_lowering::compile_source_with_runtime_metadata_via_hir(source)
                .expect("direct HIR production lowering should support typed Boolean const");
        let (bytecode, metadata) = super::compile_with_runtime_metadata(source).expect(
            "default runtime metadata compile should route typed Boolean const through HIR",
        );
        assert_eq!(
            hir.1, metadata,
            "default route metadata should come from HIR production for typed Boolean const"
        );
        assert!(
            bytecode.instructions.iter().any(|instruction| matches!(
                instruction,
    ...
    #[test]
    fn compile_with_runtime_metadata_default_routes_typed_boolean_xor_const_through_hir() {
        let source = "Const Enabled As Boolean = True\nConst CFlag As Boolean = Enabled Xor True\nSub Main()\nDim flag As Boolean\nflag = CFlag: flag = Not flag\nEnd Sub\n";
        let legacy_err = super::compile_with_runtime_metadata_legacy(source).expect_err(
            "legacy path should reject the active inline sequence after typed Boolean Xor const",
        );
        assert!(
            legacy_err.to_string().contains("unsupported statement"),
            "unexpected legacy error: {legacy_err}"
        ...
        let hir =
            super::frontend_hir_lowering::compile_source_with_runtime_metadata_via_hir(source)
                .expect("direct HIR production lowering should support typed Boolean Xor const");
        let (bytecode, metadata) = super::compile_with_runtime_metadata(source).expect(
            "default runtime metadata compile should route typed Boolean Xor const through HIR",
        );
        assert_eq!(
            hir.1, metadata,
            "default route metadata should come from HIR production for typed Boolean Xor const"
        );
        assert!(
            bytecode.instructions.iter().any(|instruction| matches!(
                instruction,
            ...
            "expected typed Boolean Xor const bytecode: {bytecode:#?}"
        );
    }

    #[test]
    fn compile_with_runtime_metadata_default_routes_typed_boolean_eqv_imp_const_through_hir() {
        let source = "Const Enabled As Boolean = True\nConst CEqv As Boolean = Enabled Eqv False\nConst CImp As Boolean = Enabled Imp False\nSub Main()\nDim sameFlag As Boolean\nDim impliesFlag As Boolean\nsameFlag = CEqv: impliesFlag = CImp\nEnd Sub\n";
        let legacy_err = super::compile_with_runtime_metadata_legacy(source).expect_err(
            "legacy path should reject the active inline sequence after typed Boolean Eqv/Imp const",
        );
        assert!(
            legacy_err.to_string().contains("unsupported statement"),
            "unexpected legacy error: {legacy_err}"
        ...
        let hir =
            super::frontend_hir_lowering::compile_source_with_runtime_metadata_via_hir(source)
                .expect(
                    "direct HIR production lowering should support typed Boolean Eqv/Imp const",
                );
        let (bytecode, metadata) = super::compile_with_runtime_metadata(source).expect(
            "default runtime metadata compile should route typed Boolean Eqv/Imp const through HIR",
        );
        assert_eq!(
            hir.1, metadata,
            "default route metadata should come from HIR production for typed Boolean Eqv/Imp const"
        );
        ...
        assert!(
            false_count >= 2,
            "expected typed Boolean Eqv/Imp const bytecode: {bytecode:#?}"
        );
    }

    #[test]
    fn compile_with_runtime_metadata_default_routes_typed_boolean_like_const_through_hir() {
        let source = "Option Compare Text\nConst Prefix As String = \"he\"\nConst CFlag As Boolean = Prefix & \"llo\" Like \"HELLO\"\nSub Main()\nDim flag As Boolean\nflag = CFlag: flag = Not flag\nEnd Sub\n";
        let legacy_err = super::compile_with_runtime_metadata_legacy(source).expect_err(
            "legacy path should reject the active inline sequence after typed Boolean Like const",
        );
        assert!(
            legacy_err.to_string().contains("unsupported statement"),
            "unexpected legacy error: {legacy_err}"
        ...
        let hir =
            super::frontend_hir_lowering::compile_source_with_runtime_metadata_via_hir(source)
                .expect("direct HIR production lowering should support typed Boolean Like const");
        let (bytecode, metadata) = super::compile_with_runtime_metadata(source).expect(
            "default runtime metadata compile should route typed Boolean Like const through HIR",
        );
        assert_eq!(
            hir.1, metadata,
            "default route metadata should come from HIR production for typed Boolean Like const"
        );
        assert!(
            bytecode.instructions.iter().any(|instruction| matches!(
                instruction,
            ...
            "expected typed Boolean Like const bytecode: {bytecode:#?}"
        );
    }

    #[test]
    fn compile_default_routes_option_compare_text_boolean_const_through_hir() {
        let source = "Option Compare Text\nConst CFlag As Boolean = \"a\" = \"A\"\nSub Main()\nDim flag As Boolean\nflag = CFlag: flag = Not flag\nEnd Sub\n";
        let legacy_err = super::compile_with_runtime_metadata_legacy(source).expect_err(
            "legacy path should reject the active inline sequence after Option Compare Text const",
        );
        assert!(
            legacy_err.to_string().contains("unsupported statement"),
            "unexpected legacy error: {legacy_err}"
        ...
        let hir =
            super::frontend_hir_lowering::compile_source_with_runtime_metadata_via_hir(source)
                .expect("direct HIR production lowering should support text Boolean const");
        let (bytecode, metadata) = super::compile_with_runtime_metadata(source)
            .expect("default runtime metadata compile should route text Boolean const through HIR");
        assert_eq!(
            hir.1, metadata,
            "default route metadata should come from HIR production for text Boolean const"
        );
        assert!(
            bytecode.instructions.iter().any(|instruction| matches!(
                instruction,
            ...
            "expected Option Compare Text Boolean const bytecode: {bytecode:#?}"
        );
    }

    #[test]
    fn compile_with_runtime_metadata_default_routes_typed_string_const_through_hir() {
        let source = "Const Prefix As String = \"re\"\nConst CText As String = Prefix & \"ady\"\nSub Main()\nDim text As String\ntext = CText: text = text & \"!\"\nEnd Sub\n";
        let legacy_err = super::compile_with_runtime_metadata_legacy(source).expect_err(
            "legacy path should reject the active inline sequence after typed String const",
        );
        assert!(
            legacy_err.to_string().contains("unsupported statement"),
            "unexpected legacy error: {legacy_err}"
        ...
        let hir =
            super::frontend_hir_lowering::compile_source_with_runtime_metadata_via_hir(source)
                .expect("direct HIR production lowering should support typed String const");
        let (bytecode, metadata) = super::compile_with_runtime_metadata(source)
            .expect("default runtime metadata compile should route typed String const through HIR");
        assert_eq!(
            hir.1, metadata,
            "default route metadata should come from HIR production for typed String const"
        );
        assert!(
            bytecode.instructions.iter().any(|instruction| matches!(
                instruction,
            ...
            "expected typed String expression const bytecode: {bytecode:#?}"
        );
    }

    #[test]
    fn compile_with_runtime_metadata_default_routes_string_const_scalar_concat_through_hir() {
        ...
        let legacy_err = super::compile_with_runtime_metadata_legacy(source).expect_err(
            "legacy path should reject the active inline sequence after scalar-coerced String const",
        );
        assert!(
            legacy_err.to_string().contains("unsupported statement"),
            "unexpected legacy error: {legacy_err}"
        ...
        let hir =
            super::frontend_hir_lowering::compile_source_with_runtime_metadata_via_hir(source)
                .expect(
                    "direct HIR production lowering should support scalar-coerced String const",
                );
        let (bytecode, metadata) = super::compile_with_runtime_metadata(source).expect(
            "default runtime metadata compile should route scalar-coerced String const through HIR",
        );
        assert_eq!(
            hir.1, metadata,
            "default route metadata should come from HIR production for scalar-coerced String const"
        );
        assert!(
            bytecode.instructions.iter().any(|instruction| matches!(
                instruction,
            ...
            "expected scalar-coerced String const bytecode: {bytecode:#?}"
        );
    }
    ...
    #[test]
    fn optional_boolean_like_defaults_route_through_hir() {
        let source = "Const Prefix = \"he\"\nSub Use(Optional ByVal flag As Boolean = Prefix & \"llo\" Like \"hello\")\nEnd Sub\nSub Main()\nCall Use()\nEnd Sub\n";
        let (_bytecode, metadata) = super::compile_with_runtime_metadata(source)
            .expect("Boolean Like defaults should route through HIR");
        let use_metadata = metadata.get("use").expect("Use metadata");
        assert!(matches!(
            use_metadata.signature.parameters[0].optional_descriptor,
            OptionalParameterDescriptor::Optional {
    ...
    fn optional_date_currency_defaults_route_through_hir() {
        let source = "Const CAmount = 1.25@\nConst CStamp = 2.0\nSub Use(Optional ByVal amount As Currency = CAmount * 2@ - 1.0@, Optional ByVal stamp As Date = (CStamp + 3.0) / 2.0, Optional ByVal literalStamp As Date = #2026-02-28#, Optional ByVal numericStamp As Date = #2/28/2026#, Optional ByVal blankAmount As Currency, Optional ByVal blankStamp As Date)\nEnd Sub\nSub Main()\nCall Use()\nEnd Sub\n";
        let (_bytecode, metadata) =
            super::compile_with_runtime_metadata(source).expect("compile should route through HIR");
        let use_metadata = metadata.get("use").expect("Use metadata");
        assert!(matches!(
        ...
        assert!(matches!(
            use_metadata.signature.parameters[5].optional_descriptor,
            OptionalParameterDescriptor::Optional {
                default_value: OptionalDefaultValue::ExplicitDateSerialF64(bits),
                ..
        ...
        assert!(main.call_sites.iter().any(|call_site| {
            call_site.arguments.iter().any(|arg| {
                ...
                arg.parameter_name.as_deref() == Some("numericstamp")
                    && arg.optional_default
                        == Some(OptionalDefaultValue::ExplicitDateSerialF64(
                            46_081.0f64.to_bits(),
                        ))
