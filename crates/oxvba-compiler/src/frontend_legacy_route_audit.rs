...
pub fn run_production_legacy_route_audit() -> LegacyRouteAuditReport {
    ...
    let untyped_string_scalar_concat_const_statement = "Const Prefix = \"v\"\nConst CNumber = 7\nConst CFlag = True\nConst CText = Prefix & CNumber & CFlag\nSub Main()\nDim text\ntext = CText\nEnd Sub\n";
    findings.push(route_finding(
        "untyped string scalar-concat const fixture",
        untyped_string_scalar_concat_const_statement,
        "bd-aprs.9.9",
    ));
    ...
    let typed_double_const_statement = "Const CBase As Long = 1\nConst CTotal As Double = CBase + 0.5\nSub Main()\nDim x As Double\nx = CTotal\nEnd Sub\n";
    findings.push(route_finding(
        "typed double const expression fixture",
    ...
    findings.push(route_finding(
        ...
        typed_currency_date_const_statement,
        "bd-aprs.9.9",
    ));

    let typed_month_name_date_const_statement = "Const CStamp As Date = #February 28, 2026#, CNext As Date = CStamp + 1\nSub Main()\nDim stamp As Date\nDim nextStamp As Date\nstamp = CStamp\nnextStamp = CNext\nEnd Sub\n";
    findings.push(route_finding(
        "typed month-name date const fixture",
        typed_month_name_date_const_statement,
        "bd-aprs.9.9",
    ));
    ...
    findings.push(route_finding(
        "untyped date const fixture",
        untyped_date_const_statement,
        "bd-aprs.9.9",
    ));
    ...
    let typed_boolean_xor_const_statement = "Const Enabled As Boolean = True\nConst CFlag As Boolean = Enabled Xor True\nSub Main()\nDim flag As Boolean\nflag = CFlag\nEnd Sub\n";
    findings.push(route_finding(
        "typed boolean Xor const expression fixture",
        typed_boolean_xor_const_statement,
        "bd-aprs.9.9",
    ));
    ...
    let typed_string_scalar_concat_const_statement = "Const Prefix As String = \"v\"\nConst CNumber As Long = 7\nConst CFlag As Boolean = True\nConst CText As String = Prefix & CNumber & CFlag\nSub Main()\nDim text As String\ntext = CText\nEnd Sub\n";
    findings.push(route_finding(
        "typed string scalar-concat const fixture",
        typed_string_scalar_concat_const_statement,
        "bd-aprs.9.9",
    ));
    ...
    let optional_string_scalar_concat_default_statement = "Const Prefix = \"v\"\nConst CNumber = 7\nConst CFlag = True\nSub Use(Optional ByVal text As String = Prefix & CNumber & CFlag)\nEnd Sub\nSub Main()\nCall Use()\nEnd Sub\n";
    findings.push(route_finding(
        "optional string scalar concat default fixture",
        optional_string_scalar_concat_default_statement,
        "bd-aprs.9.9",
    ));

    let optional_boolean_like_default_statement = "Const Prefix = \"he\"\nSub Use(Optional ByVal flag As Boolean = Prefix & \"llo\" Like \"hello\")\nEnd Sub\nSub Main()\nCall Use()\nEnd Sub\n";
    findings.push(route_finding(
        "optional Boolean Like default fixture",
        optional_boolean_like_default_statement,
        "bd-aprs.9.9",
    ));
    ...
    let object_udt_erase =
        "Type Record\nChild As Object\nEnd Type\nSub Main()\nDim r As Record\nErase r\nEnd Sub\n";
    findings.push(route_finding(
        "object UDT Erase reset fixture",
        object_udt_erase,
        "bd-aprs.9.10",
    ));
    ...
    let member_array_erase = "Type Record\nNames(1 To 2) As String * 4\nEnd Type\nSub Main()\nDim r As Record\nr.Names(1) = \"abcde\"\nErase r.Names\nEnd Sub\n";
    findings.push(route_finding(
        "member array Erase target fixture",
        member_array_erase,
        "bd-aprs.9.10",
    ));
