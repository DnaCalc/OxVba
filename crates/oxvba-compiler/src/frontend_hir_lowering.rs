...
pub fn lower_typed_hir_to_bound_module_with_project_bindings(
    ...
    Ok(BoundModule {
        ...
        vb_creatable_attribute: collect_vb_bool_attribute(&lines, "VB_Creatable"),
...
fn parse_const_value_for_declared(
    ...
    if declared_type.is_none()
        && let Some(value) = parse_const_string_value(text, named_values)
    {
...
mod tests {
    ...
    fn hir_production_lowering_erase_resets_object_udt_aliases_to_nothing() {
        ...
        let child_slot = main
            .slots
            .iter()
            .find(|slot| slot.name == "r_child")
            .expect("object field alias slot")
            .slot;
        ...
        assert!(
            ...
            "expected Erase reset to Nothing for object UDT field slot {child_slot}: {:?}",
            bytecode.instructions
        );
    }
    ...
    #[test]
    fn hir_production_lowering_preserves_boolean_module_attributes() {
        let source = concat!(
    ...
    #[test]
    fn hir_production_lowering_folds_untyped_string_const_expression() {
        ...
        assert!(
            !main.slots.iter().any(|slot| {
                slot.name.eq_ignore_ascii_case("prefix") || slot.name.eq_ignore_ascii_case("ctext")
            }),
            "{main:#?}"
        );
    }
