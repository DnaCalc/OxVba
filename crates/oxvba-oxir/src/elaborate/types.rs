...
/// Lower a [`VarTypeRef`] to an [`OxTy`], resolving object/UDT names through `r`.
pub fn lower_var_type(ty: &VarTypeRef, r: &impl NameResolver) -> OxTy {
    match ty {
        VarTypeRef::Builtin(b) => lower_builtin(*b),
...
fn lower_builtin(b: BuiltinType) -> OxTy {
    match b {
        ...
        BuiltinType::LongPtr => OxTy::LongPtr,
...
mod tests {
    ...
    fn objects_resolve_by_name() {
        let r = resolver();
        assert_eq!(
            lower_var_type(&VarTypeRef::Object("widget".into()), &r),
            OxTy::Object(ObjClass::Class(ClassId(2)))
        );
        assert_eq!(
            lower_var_type(&VarTypeRef::Object("ishape".into()), &r),
            OxTy::Object(ObjClass::Iface(IfaceId(1)))
        );
        assert_eq!(
            lower_var_type(&VarTypeRef::Object("excel.range".into()), &r),
            OxTy::Object(ObjClass::ComIface(IfaceId(5)))
        );
        // An unresolvable / bare object name → untyped (late-bound).
        assert_eq!(
            lower_var_type(&VarTypeRef::Object("object".into()), &r),
    ...
    fn udt_and_enum_resolve_by_name_regardless_of_tag() {
        ...
        assert_eq!(
            lower_var_type(&VarTypeRef::Udt("point".into()), &r),
            OxTy::Record(RecordLayoutId(3))
        );
        assert_eq!(
            lower_var_type(&VarTypeRef::Object("point".into()), &r),
            OxTy::Record(RecordLayoutId(3))
        );
