...
function Get-DescendantNamesByControlType {
    ...
    try {
        ...
        # Ignore stale UIA trees; the next poll retries.
    }
    $names
}
...
function Invoke-AutomationElement {
    ...
    try {
        $hwnd = [IntPtr]::new($Element.Current.NativeWindowHandle)
        if ($hwnd -ne [IntPtr]::Zero) {
            [void][VbeWin32]::PostMessage($hwnd, [VbeWin32]::BM_CLICK, [IntPtr]::Zero, [IntPtr]::Zero)
            return $true
        }
    } catch {
        # No invokable surface.
    }
