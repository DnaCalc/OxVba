...
$certificationCase = @($caseDocument.cases | Where-Object case_id -eq "WIN14-WAC-CLEAN-CERT-ENV")[0]
...
Invoke-ExpectedCaptureFailure -Name "certification-case-artifact-binding" -Pattern "exact controlled environment-capture path" -Action {
    ...
    @($mutatedCase.artifacts | Where-Object kind -eq "environment-capture")[0].path = "artifacts/windows-x64/controlled-environments/v1/wrong/environment-capture.json"
