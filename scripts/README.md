...
- `run-com-registered-events.ps1`: runs strict registered event-callback success lane (`L2E`) for an event-capable ProgID and emits lane artifacts; supports callback arity contract via `-ExpectedArgCount` / `OXVBA_REGISTERED_EVENT_EXPECTED_ARGC` and auto-defaults `Excel.Application` to token/member `10` with expected arg count `0`.
