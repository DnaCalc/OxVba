...
try {
    if (-not $IsWindows) {
        throw "COM TestEventServer three-reference qualified broken-first oracle runner is Windows-only"
    }

    . "$PSScriptRoot/lib-run-context.ps1"
    . "$PSScriptRoot/lib-com-testeventserver-alt-project.ps1"
    $resolvedRunId = Resolve-RunId -Name "com-testeventserver-three-reference-qualified-broken-first-oracle" -RequestedRunId $RunId
    if ($NoArtifacts) {
        $OutputRoot = New-NoArtifactEvidenceDir -Scope "com-testeventserver-three-reference-qualified-broken-first-oracle" -RunId $resolvedRunId
    }

    $workspaceRoot = (Resolve-Path ".").Path
    $runRoot = if ([System.IO.Path]::IsPathRooted($OutputRoot)) { $OutputRoot } else { Join-Path $workspaceRoot $OutputRoot }
    $runDir = Join-Path $runRoot "com_testeventserver_three_reference_qualified_broken_first_oracle_$resolvedRunId"
    New-Item -ItemType Directory -Force -Path $runDir | Out-Null

    $generatedRoot = Join-Path $workspaceRoot "temp\generated\com_testeventserver_three_reference_qualified_broken_first\$resolvedRunId"
    ...
    $probeScriptPath = Join-Path $runDir "_three_reference_qualified_broken_first_probe.ps1"
    $probeScript = @'
param(
...
$root = Join-Path $env:TEMP ("oxvba_three_qualified_broken_first_" + [guid]::NewGuid().ToString("N"))
...
Public Function RunProbe()
    Dim obj As ${QualifiedNamespace}.TestEventServer
    Set obj = New ${QualifiedNamespace}.TestEventServer
    RunProbe = obj.Ping()
End Function
"@
...
'@
    ...
    function Get-HandlerLogMetadata {
        ...
        if ($observed -eq "unknown" -and (Test-Path $Path)) {
            $lines = Get-Content $Path
            if ($lines.Count -gt 0) { $observed = "false" }
        }
        @{ observed = $observed; classification = $classification }
    }

    function Invoke-ThreeReferenceQualifiedBrokenFirstProbe {
        param([string]$CaseId,[string]$FirstTypeLibPath,[string]$SecondTypeLibPath,[string]$ThirdTypeLibPath,[string]$QualifiedNamespace)
        ...
        $probeProcess = Start-Process `
            -FilePath (Get-Command pwsh).Source `
            -ArgumentList @("-NoProfile","-NonInteractive","-File",$probeScriptPath,$FirstTypeLibPath,$SecondTypeLibPath,$ThirdTypeLibPath,$QualifiedNamespace,$statePath,$vbaDialogHandlerScriptPath,$handlerLogPath,$ProbeTimeoutSeconds) `
        ...
        if (-not $completed) {
            ...
            $sawUiFailure = $handlerLogMeta.observed -eq "true"
            return @{
                status = if ($sawUiFailure) { "error" } else { "timeout" }
                observed = if ($sawUiFailure) {
    ...
    $cases = @(
        @{
            case_id = "CCT-043-TES-BROKEN-FIRST-QUAL3-001"
            scenario = "Saved workbook with base broken then alt valid then alt2 valid; qualified OxVbaAlt2 target still resolves later valid alt2 reference"
            first = $baseTypeLibPath
            second = $altTypeLibPath
            third = $alt2TypeLibPath
            qualified_namespace = "OxVbaAlt2"
            command = @("test","-p","oxvba-host","--test","com_early_project_end_to_end","early_bound_loaded_basproj_broken_base_then_valid_alt_then_valid_alt2_qualified_target_resolves_alt2_binding","--","--ignored","--exact","--test-threads=1","--nocapture")
            expected_vba = "126"
            expected_prog_id = "OxVba.TestEventServerAlt2"
        }
        @{
            case_id = "CCT-043-TES-BROKEN-FIRST-QUAL3-002"
            scenario = "Saved workbook with alt2 broken then base valid then alt valid; qualified OxVbaAlt target still resolves later valid alt reference"
            first = $alt2TypeLibPath
            second = $baseTypeLibPath
            third = $altTypeLibPath
            qualified_namespace = "OxVbaAlt"
            command = @("test","-p","oxvba-host","--test","com_early_project_end_to_end","early_bound_loaded_basproj_broken_alt2_then_valid_base_then_valid_alt_qualified_target_resolves_alt_binding","--","--ignored","--exact","--test-threads=1","--nocapture")
            expected_vba = "84"
    ...
    foreach ($case in $cases) {
        $probe = Invoke-ThreeReferenceQualifiedBrokenFirstProbe -CaseId $case.case_id -FirstTypeLibPath $case.first -SecondTypeLibPath $case.second -ThirdTypeLibPath $case.third -QualifiedNamespace $case.qualified_namespace
    ...
    Write-Host "com-testeventserver-three-reference-qualified-broken-first-oracle: complete"
