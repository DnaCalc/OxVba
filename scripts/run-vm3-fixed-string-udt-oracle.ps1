param(
    [string]$RunId = ("vm3_fixed_string_udt_oracle_{0:yyyyMMddTHHmmssZ}" -f (Get-Date).ToUniversalTime()),
...
$cases = @()
$cases += (New-OracleCase "UDT-FIXED-DEFAULT" "A fixed-string UDT field defaults to its declared width." @"
Private Type Person
    Name As String * 5
End Type
...
Public Function RunProbe() As Variant
    Dim p As Person
    RunProbe = CStr(Len(p.Name)) & ":|" & p.Name & "|:" & CStr(Asc(Mid(p.Name, 1, 1)))
End Function
"@)
$cases += (New-OracleCase "UDT-FIXED-SHORT" "Assignment pads a short value stored in a fixed-string UDT field." @"
Private Type Person
    Name As String * 5
End Type
...
Public Function RunProbe() As Variant
    Dim p As Person
    p.Name = "ab"
    RunProbe = CStr(Len(p.Name)) & ":|" & p.Name & "|:" & CStr(Asc(Mid(p.Name, 3, 1)))
End Function
"@)
$cases += (New-OracleCase "UDT-FIXED-LONG" "Assignment truncates a long value stored in a fixed-string UDT field." @"
Private Type Person
    Name As String * 5
End Type
...
Public Function RunProbe() As Variant
    Dim p As Person
    p.Name = "abcdef"
    RunProbe = CStr(Len(p.Name)) & ":|" & p.Name & "|"
End Function
"@)
$cases += (New-OracleCase "UDT-FIXED-ARRAY-ELEMENT" "Arrays of UDTs default and assign fixed-string fields per element." @"
Private Type Person
    Name As String * 5
End Type
...
Public Function RunProbe() As Variant
    ...
    people(1).Name = "xy"
    RunProbe = CStr(Len(people(0).Name)) & ":|" & people(0).Name & "|;" & CStr(Len(people(1).Name)) & ":|" & people(1).Name & "|"
End Function
"@)
$cases += (New-OracleCase "UDT-FIXED-WHOLE-COPY" "Whole-UDT assignment copies the padded fixed-string field value." @"
Private Type Person
    Name As String * 5
End Type
...
Public Function RunProbe() As Variant
    ...
    q = p
    p.Name = "zzzzz"
    RunProbe = CStr(Len(q.Name)) & ":|" & q.Name & "|"
End Function
"@)
$cases += (New-OracleCase "UDT-FIXED-NULL" "Null assignment error behavior for fixed-string UDT fields." @"
Private Type Person
    Name As String * 5
End Type
...
Public Function RunProbe() As Variant
    ...
    Dim p As Person
    p.Name = Null
    RunProbe = "ok:" & CStr(Len(p.Name)) & ":|" & p.Name & "|"
    Exit Function
EH:
    RunProbe = "err:" & CStr(Err.Number) & ":" & Err.Description & ":|" & p.Name & "|"
End Function
"@)
$cases += (New-OracleCase "UDT-FIXED-LEN-SIZE" "Len and LenB classify fixed-string UDT field record sizing." @"
...
Public Function RunProbe() As Variant
    ...
    RunProbe = CStr(Len(p)) & ":" & CStr(LenB(p))
End Function
"@)
...
$lines.Add("# VM3 Fixed-String UDT Excel Oracle")
$lines.Add("")
