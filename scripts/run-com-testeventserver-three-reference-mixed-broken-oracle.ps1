...
try {
    if (-not $IsWindows) {
        throw "COM TestEventServer three-reference mixed broken oracle runner is Windows-only"
    }

    . "$PSScriptRoot/lib-run-context.ps1"
    . "$PSScriptRoot/lib-com-testeventserver-alt-project.ps1"
    $resolvedRunId = Resolve-RunId -Name "com-testeventserver-three-reference-mixed-broken-oracle" -RequestedRunId $RunId
    if ($NoArtifacts) {
        $OutputRoot = New-NoArtifactEvidenceDir -Scope "com-testeventserver-three-reference-mixed-broken-oracle" -RunId $resolvedRunId
    }

    $workspaceRoot = (Resolve-Path ".").Path
    $runRoot = if ([System.IO.Path]::IsPathRooted($OutputRoot)) { $OutputRoot } else { Join-Path $workspaceRoot $OutputRoot }
    $runDir = Join-Path $runRoot "com_testeventserver_three_reference_mixed_broken_oracle_$resolvedRunId"
    New-Item -ItemType Directory -Force -Path $runDir | Out-Null

    $generatedRoot = Join-Path $workspaceRoot "temp\generated\com_testeventserver_three_reference_mixed_broken\$resolvedRunId"
    ...
    $probeScriptPath = Join-Path $runDir "_three_reference_mixed_broken_probe.ps1"
    $probeScript = @'
param(
...
$root = Join-Path $env:TEMP ("oxvba_three_mixed_broken_ref_" + [guid]::NewGuid().ToString("N"))
...
'@
    ...
    function Invoke-ThreeReferenceMixedBrokenProbe {
        ...
        if (-not $completed) {
            ...
            $orphanedWindowTitles = @(Get-Process -Id $orphanedPids -ErrorAction SilentlyContinue | Where-Object { -not [string]::IsNullOrWhiteSpace($_.MainWindowTitle) } | ForEach-Object { $_.MainWindowTitle })
            ...
            if ($state -and $state.stage -eq "completed") {
                return @{ status="ok"; observed=[string]$state.run; refs=($state.refs -join "|"); stage=[string]$state.stage; modal_observed=if($handlerLogMeta.observed -ne "unknown"){$handlerLogMeta.observed}else{"false"}; probe_exit_code="cleanup-timeout"; window_titles=($orphanedWindowTitles -join "|"); handler_log=$handlerLogPath; handler_signal="cleanup-timeout-after-completed-state" }
            }
            return @{
                status = if ($sawUiFailure) { "error" } else { "timeout" }
                observed = if ($sawUiFailure) { if ([string]::IsNullOrWhiteSpace($handlerLogMeta.classification)) { "ui-blocked-or-compile-failure" } else { $handlerLogMeta.classification } } else { "execution-did-not-return-within-${ProbeTimeoutSeconds}s" }
        ...
        $handlerLogMeta = Get-HandlerLogMetadata -Path $handlerLogPath
        if ($null -eq $state) {
            return @{ status="error"; observed="no-state-captured(exit=$probeExitCode)"; refs=""; stage=""; modal_observed="unknown"; probe_exit_code=$probeExitCode; window_titles=""; handler_log=$handlerLogPath; handler_signal=$handlerLogMeta.classification }
        }
        if ($state.stage -eq "completed") {
            return @{ status="ok"; observed=[string]$state.run; refs=($state.refs -join "|"); stage=[string]$state.stage; modal_observed=if($handlerLogMeta.observed -ne "unknown"){$handlerLogMeta.observed}else{"false"}; probe_exit_code=$probeExitCode; window_titles=""; handler_log=$handlerLogPath; handler_signal=$handlerLogMeta.classification }
        }
        @{ status="error"; observed=[string]$state.run_error; refs=($state.refs -join "|"); stage=[string]$state.stage; modal_observed=if($handlerLogMeta.observed -ne "unknown"){$handlerLogMeta.observed}else{"false"}; probe_exit_code=$probeExitCode; window_titles=""; handler_log=$handlerLogPath; handler_signal=$handlerLogMeta.classification }
    }

    $cases = @(
        @{
            case_id = "CCT-043-TES-MIXED3-001"
            scenario = "Saved workbook with base broken then alt valid then alt2 valid; unqualified TestEventServer"
            first = $baseTypeLibPath
            second = $altTypeLibPath
            third = $alt2TypeLibPath
            command = @("test","-p","oxvba-host","--test","com_early_project_end_to_end","early_bound_loaded_basproj_mixed_broken_base_then_valid_alt_then_valid_alt2_reports_unresolved_importlib","--","--ignored","--exact","--test-threads=1","--nocapture")
        }
        @{
            case_id = "CCT-043-TES-MIXED3-002"
            scenario = "Saved workbook with alt2 broken then base valid then alt valid; unqualified TestEventServer"
            first = $alt2TypeLibPath
            second = $baseTypeLibPath
            third = $altTypeLibPath
            command = @("test","-p","oxvba-host","--test","com_early_project_end_to_end","early_bound_loaded_basproj_mixed_broken_alt2_then_valid_base_then_valid_alt_reports_unresolved_importlib","--","--ignored","--exact","--test-threads=1","--nocapture")
        }
    )

    foreach ($case in $cases) {
        $probe = Invoke-ThreeReferenceMixedBrokenProbe -CaseId $case.case_id -FirstTypeLibPath $case.first -SecondTypeLibPath $case.second -ThirdTypeLibPath $case.third
        ...
        $oxObserved = if ($exitCode -eq 0) { "PMR-E-TYPELIB-IMPORTLIB-UNRESOLVED" } else { "lane-failed(exit=$exitCode)" }
        $match = if ($probe.status -eq "error" -and $exitCode -eq 0) { "true" } else { "false" }
    ...
    $summary = @(
        "# COM TestEventServer Three-Reference Mixed Broken Oracle Run",
        "",
    ...
    Write-Host "com-testeventserver-three-reference-mixed-broken-oracle: complete"
