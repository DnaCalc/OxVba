...
try {
    ...
    $resolvedRunId = Resolve-RunId -Name "com-early-conformance" -RequestedRunId $RunId
    $resolvedNoLatest = $NoLatest -or $NoArtifacts
    if ($NoArtifacts) {
        $EvidenceDir = New-NoArtifactEvidenceDir -Scope "com-early-conformance" -RunId $resolvedRunId
    }
    ...
    $csvPath = Join-Path $EvidenceDir ("COM_EARLY_CONFORMANCE_RUN_{0}.csv" -f $resolvedRunId)
    $mdPath = Join-Path $EvidenceDir ("COM_EARLY_CONFORMANCE_RUN_{0}.md" -f $resolvedRunId)
