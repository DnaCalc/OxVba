...
try {
    if (-not $IsWindows) {
        throw "COM TestEventServer qualified broken-first reference oracle runner is Windows-only"
    }

    . "$PSScriptRoot/lib-run-context.ps1"
    . "$PSScriptRoot/lib-com-testeventserver-alt-project.ps1"

    $resolvedRunId = Resolve-RunId -Name "com-testeventserver-qualified-broken-first-reference-oracle" -RequestedRunId $RunId
    if ($NoArtifacts) {
        $OutputRoot = New-NoArtifactEvidenceDir -Scope "com-testeventserver-qualified-broken-first-reference-oracle" -RunId $resolvedRunId
    }
    ...
    $runDir = Join-Path $runRoot "com_testeventserver_qualified_broken_first_reference_oracle_$resolvedRunId"
    New-Item -ItemType Directory -Force -Path $runDir | Out-Null

    $generatedRoot = Join-Path $workspaceRoot "temp\generated\com_testeventserver_qualified_broken_first_reference\$resolvedRunId"
    ...
    $probeScriptPath = Join-Path $runDir "_qualified_broken_first_reference_probe.ps1"
    $probeScript = @'
param(
...
$root = Join-Path $env:TEMP ("oxvba_qualified_broken_first_reference_" + [guid]::NewGuid().ToString("N"))
...
Public Function RunProbe()
    Dim obj As $QualifiedTypeName
    Set obj = New $QualifiedTypeName
    RunProbe = obj.Ping()
End Function
"@
...
'@
    ...
    function Invoke-QualifiedBrokenFirstProbe {
        param(
            [string]$CaseId,
            [string]$FirstTypeLibPath,
            [string]$SecondTypeLibPath,
    ...
    $cases = @(
        @{
            case_id = "CCT-043-TES-BROKEN-FIRST-QUAL-001"
            scenario = "Saved workbook with base broken then alt valid after reopen; explicit qualified alt target"
            ...
            command = @(
                ...
                "early_bound_loaded_basproj_broken_base_then_valid_alt_qualified_target_resolves_alt_binding",
        ...
        @{
            case_id = "CCT-043-TES-BROKEN-FIRST-QUAL-002"
            scenario = "Saved workbook with alt broken then base valid after reopen; explicit qualified base target"
            first = $altTypeLibPath
            second = $baseTypeLibPath
            qualified = "OxVba_TestEventServer.TestEventServer"
            command = @(
                "test", "-p", "oxvba-host", "--test", "com_early_project_end_to_end",
                "early_bound_loaded_basproj_broken_alt_then_valid_base_qualified_target_resolves_base_binding",
                "--", "--ignored", "--exact", "--test-threads=1", "--nocapture"
            )
            expected_vba = "42"
    ...
    foreach ($case in $cases) {
        $probe = Invoke-QualifiedBrokenFirstProbe `
            -CaseId $case.case_id `
    ...
    Write-Host "com-testeventserver-qualified-broken-first-reference-oracle: complete"
