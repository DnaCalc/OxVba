param(
    [string]$ProjectPath = "examples/xll/array_addin",
...
try {
    ...
    if (-not (Test-Path -LiteralPath $ProjectPath)) {
        throw "Missing XLL array Addin project path: $ProjectPath"
    }

    $resolvedProject = (Resolve-Path -LiteralPath $ProjectPath).Path
    $projectName = "ArrayAddin"
    $runDir = Join-Path $OutputRoot (Join-Path "array_addin" $RunId)
    ...
    Copy-Item -LiteralPath (Join-Path $resolvedProject "ArrayAddin.basproj") -Destination $sourceDir -Force
    Copy-Item -LiteralPath (Join-Path $resolvedProject "ArrayExports.bas") -Destination $sourceDir -Force
    ...
    if ($exitCode -ne 0) {
        throw "XLL array Addin staging failed with exit code $exitCode; see $buildLogPath"
    }
    if (-not (Test-Path -LiteralPath $artifactPath)) {
        throw "XLL array Addin staging did not produce artifact: $artifactPath"
    }
    ...
    Write-Host "xll array addin staged: $runDir"
