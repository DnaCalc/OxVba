...
try {
    ...
    $policyBypass = Write-MutatedManifest -Name "policy-bypass" -Mutation {
        param($manifest)
        $manifest.execution_policy.executable_state = "runner alone is sufficient"
    }
    Assert-ValidatorFailure -Name "execution policy bypasses producer and environment gates" -Path $policyBypass
    ...
    Write-Host "test-win14-certification-manifest: ok (positive case plus 12 fail-closed mutations)"
}
finally {
