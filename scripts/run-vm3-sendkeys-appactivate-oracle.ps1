param(
    [string]$RunId = ("vm3_sendkeys_appactivate_oracle_{0:yyyyMMddTHHmmssZ}" -f (Get-Date).ToUniversalTime()),
...
function Get-OwnedDialogSnapshot([int]$ExcelPid, $Vbe) {
    ...
    foreach ($window in $windows) {
        ...
        $hasDiagnostic = @($names | Where-Object {
            $_ -match "Compile error|Run-time error|runtime error"
        }).Count -gt 0
        if (-not $hasDiagnostic) {
            continue
        }
...
$cases += [pscustomobject]@{
    id = "SENDKEYS-EMPTY-STATEMENT"
    purpose = "SendKeys with an empty string compiles and runs as a statement without injecting input."
    code = @'
...
$cases += [pscustomobject]@{
    id = "APPACTIVATE-MISSING-WINDOW"
    purpose = "AppActivate reports its runtime error when no window matches the requested title."
    code = @'
Public Function RunProbe() As String
    On Error Resume Next
    Err.Clear
...
$cases += [pscustomobject]@{
    id = "SENDKEYS-EXPRESSION"
    purpose = "SendKeys used as a value expression is rejected by the compiler."
    code = @'
...
$cases += [pscustomobject]@{
    id = "APPACTIVATE-EXPRESSION"
    purpose = "AppActivate used as a value expression is rejected by the compiler."
    code = @'
Public Function RunProbe() As Variant
    RunProbe = AppActivate("__OXVBA_NO_SUCH_WINDOW_20260702__")
End Function
'@
}
...
$lines.Add("# VM3 SendKeys/AppActivate Excel Oracle")
$lines.Add("")
...
$lines.Add("- Probe safety: `SendKeys` uses an empty key string; no real keystrokes are injected.")
$lines.Add("")
