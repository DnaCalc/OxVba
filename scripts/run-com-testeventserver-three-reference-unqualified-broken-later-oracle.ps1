...
try {
    if (-not $IsWindows) {
        throw "COM TestEventServer three-reference unqualified broken-later oracle runner is Windows-only"
    }

    . "$PSScriptRoot/lib-run-context.ps1"
    . "$PSScriptRoot/lib-com-testeventserver-alt-project.ps1"
    $resolvedRunId = Resolve-RunId -Name "com-testeventserver-three-reference-unqualified-broken-later-oracle" -RequestedRunId $RunId
    if ($NoArtifacts) {
        $OutputRoot = New-NoArtifactEvidenceDir -Scope "com-testeventserver-three-reference-unqualified-broken-later-oracle" -RunId $resolvedRunId
    }

    $workspaceRoot = (Resolve-Path ".").Path
    $runRoot = if ([System.IO.Path]::IsPathRooted($OutputRoot)) { $OutputRoot } else { Join-Path $workspaceRoot $OutputRoot }
    $runDir = Join-Path $runRoot "com_testeventserver_three_reference_unqualified_broken_later_oracle_$resolvedRunId"
    New-Item -ItemType Directory -Force -Path $runDir | Out-Null

    $generatedRoot = Join-Path $workspaceRoot "temp\generated\com_testeventserver_three_reference_unqualified_broken_later\$resolvedRunId"
    ...
    $probeScriptPath = Join-Path $runDir "_three_reference_unqualified_broken_later_probe.ps1"
    $probeScript = @'
param(
    [string]$FirstTypeLibPath,
    [string]$SecondTypeLibPath,
    [string]$ThirdTypeLibPath,
    [string]$StatePath,
...
$root = Join-Path $env:TEMP ("oxvba_three_unqualified_broken_later_" + [guid]::NewGuid().ToString("N"))
...
$thirdCopy = Join-Path $root ([System.IO.Path]::GetFileName($ThirdTypeLibPath))
...
Copy-Item $ThirdTypeLibPath $thirdCopy -Force
...
try {
    ...
    if ($excelPid -gt 0 -and (Test-Path $VbaDialogHandlerScriptPath)) {
        ...
        $vbaDialogHandler = Start-Process `
            ...
            -ArgumentList @(
                "-NoProfile", "-NonInteractive", "-File", $VbaDialogHandlerScriptPath,
                $excelPid, $vbaDialogHandlerStop, $VbaDialogHandlerLogPath, $deadlineFile, 200
            ) `
            -PassThru `
            -WindowStyle Hidden
    }
    ...
    [void]$wb.VBProject.References.AddFromFile($thirdCopy)
    ...
    try {
        ...
        if (Test-Path $deadlineFile) { Remove-Item -Force -Path $deadlineFile }
    }
} finally {
    ...
    if ($vbaDialogHandler -ne $null) {
        ...
        if (-not $vbaDialogHandler.HasExited) { Stop-Process -Id $vbaDialogHandler.Id -Force -ErrorAction SilentlyContinue }
    }
    if ($reopened -ne $null) {
...
'@
    ...
    function Add-Row {
        param([string]$CaseId,[string]$Scenario,[string]$VbaStatus,[string]$VbaObserved,[string]$OxVbaStatus,[string]$OxVbaObserved,[string]$Match,[string]$Notes)
        $rows.Add([PSCustomObject]@{
            topic_id="CCT-043"; case_id=$CaseId; scenario=$Scenario; vba_status=$VbaStatus; vba_observed=$VbaObserved; oxvba_status=$OxVbaStatus; oxvba_observed=$OxVbaObserved; match=$Match; notes=$Notes
        }) | Out-Null
    }
    ...
    function Get-HandlerLogMetadata {
        ...
        if (Test-Path $Path) {
            ...
            $signalLines = @($lines | Where-Object { $_ -match "observed window=" -or $_ -match "deadline exceeded" })
            if ($signalLines.Count -gt 0) { $observed = "true"; $classification = "ui-blocked-or-compile-failure" }
            elseif ($lines.Count -gt 0) { $observed = "false" }
        }
        @{ observed = $observed; classification = $classification }
    }

    function Invoke-ThreeReferenceUnqualifiedBrokenLaterProbe {
        param([string]$CaseId,[string]$FirstTypeLibPath,[string]$SecondTypeLibPath,[string]$ThirdTypeLibPath)
        ...
        if (Test-Path $statePath) { Remove-Item -Force -Path $statePath }
        if (Test-Path $handlerLogPath) { Remove-Item -Force -Path $handlerLogPath }
        $probeProcess = Start-Process `
            -FilePath (Get-Command pwsh).Source `
            -ArgumentList @("-NoProfile","-NonInteractive","-File",$probeScriptPath,$FirstTypeLibPath,$SecondTypeLibPath,$ThirdTypeLibPath,$statePath,$vbaDialogHandlerScriptPath,$handlerLogPath,$ProbeTimeoutSeconds) `
        ...
        if (-not $completed) {
            ...
            foreach ($orphanedPid in $orphanedPids) { Stop-Process -Id $orphanedPid -Force -ErrorAction SilentlyContinue }
            $handlerLogMeta = Get-HandlerLogMetadata -Path $handlerLogPath
            if ($state -and $state.stage -eq "completed") {
                return @{ status="ok"; observed=[string]$state.run; refs=($state.refs -join "|"); stage=[string]$state.stage; modal_observed=if($handlerLogMeta.observed -ne "unknown"){$handlerLogMeta.observed}else{"false"}; probe_exit_code="cleanup-timeout"; handler_log=$handlerLogPath; handler_signal="cleanup-timeout-after-completed-state" }
            }
            return @{ status="timeout"; observed="execution-did-not-return-within-${ProbeTimeoutSeconds}s"; refs=if($state){($state.refs -join "|")}else{""}; stage=if($state){[string]$state.stage}else{""}; modal_observed=$handlerLogMeta.observed; probe_exit_code=""; handler_log=$handlerLogPath; handler_signal=$handlerLogMeta.classification }
        }

        $probeExitCode = [string]$probeProcess.ExitCode
        $handlerLogMeta = Get-HandlerLogMetadata -Path $handlerLogPath
        if ($null -eq $state) {
            return @{ status="error"; observed="no-state-captured(exit=$probeExitCode)"; refs=""; stage=""; modal_observed="unknown"; probe_exit_code=$probeExitCode; handler_log=$handlerLogPath; handler_signal=$handlerLogMeta.classification }
        }
        if ($state.stage -eq "completed") {
            return @{ status="ok"; observed=[string]$state.run; refs=($state.refs -join "|"); stage=[string]$state.stage; modal_observed=if($handlerLogMeta.observed -ne "unknown"){$handlerLogMeta.observed}else{"false"}; probe_exit_code=$probeExitCode; handler_log=$handlerLogPath; handler_signal=$handlerLogMeta.classification }
        }
        return @{ status="error"; observed=[string]$state.run_error; refs=($state.refs -join "|"); stage=[string]$state.stage; modal_observed=if($handlerLogMeta.observed -ne "unknown"){$handlerLogMeta.observed}else{"false"}; probe_exit_code=$probeExitCode; handler_log=$handlerLogPath; handler_signal=$handlerLogMeta.classification }
    }

    $cases = @(
        @{
            case_id = "CCT-043-TES-BROKEN-LATER3-001"
            scenario = "Saved workbook with base valid then alt broken after save then alt2 valid; unqualified TestEventServer still selects first valid base reference"
            first = $baseTypeLibPath
            second = $altTypeLibPath
            third = $alt2TypeLibPath
            command = @("test","-p","oxvba-host","--test","com_early_project_end_to_end","early_bound_loaded_basproj_valid_base_then_broken_alt_then_valid_alt2_prefers_base_for_unqualified_testeventserver","--","--ignored","--exact","--test-threads=1","--nocapture")
            expected_vba = "42"
            expected_prog_id = "OxVba.TestEventServer"
        }
        @{
            case_id = "CCT-043-TES-BROKEN-LATER3-002"
            scenario = "Saved workbook with alt2 valid then base broken after save then alt valid; unqualified TestEventServer still selects first valid alt2 reference"
            first = $alt2TypeLibPath
            second = $baseTypeLibPath
            third = $altTypeLibPath
            command = @("test","-p","oxvba-host","--test","com_early_project_end_to_end","early_bound_loaded_basproj_valid_alt2_then_broken_base_then_valid_alt_prefers_alt2_for_unqualified_testeventserver","--","--ignored","--exact","--test-threads=1","--nocapture")
            expected_vba = "126"
    ...
    foreach ($case in $cases) {
        $probe = Invoke-ThreeReferenceUnqualifiedBrokenLaterProbe -CaseId $case.case_id -FirstTypeLibPath $case.first -SecondTypeLibPath $case.second -ThirdTypeLibPath $case.third
        ...
        $oxObserved = if ($exitCode -eq 0) { "compile-selected-progid=$($case.expected_prog_id)" } else { "lane-failed(exit=$exitCode)" }
        $match = if ($probe.status -eq "ok" -and $exitCode -eq 0 -and $probe.observed -eq $case.expected_vba) { "true" } else { "false" }
        Add-Row -CaseId $case.case_id -Scenario $case.scenario -VbaStatus $probe.status -VbaObserved $probe.observed -OxVbaStatus $oxStatus -OxVbaObserved $oxObserved -Match $match -Notes (
    ...
    Write-Host "com-testeventserver-three-reference-unqualified-broken-later-oracle: complete"
