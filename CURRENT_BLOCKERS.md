...
- Current state:
  ...
  - but there is still no fully closed end-to-end model for `Set` vs `Let` intent parity in broader typed/object contexts beyond the now-covered plain-assignment, assignment-from-call, `Object`/`Variant` object-result success lanes, `Let`-rejects-object-call, and explicit native PMR `Let` lanes, plus ambiguous or broader non-authoritative default-member call resolution beyond the now-covered single-visible-candidate scalar/indexed native fallback in assignment, statement-context, and explicit `Call` getter contexts, and Office-style call-vs-value context parity.
- Exact unblock steps:
