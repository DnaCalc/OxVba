...
- Current state:
  ...
  - controlled direct-error coverage now also proves VM/JIT host parity for real `arg_err` indexing on `DISP_E_TYPEMISMATCH`, bounded `EXCEPINFO` source/description/scode detail on `DISP_E_EXCEPTION`, stable `DISP_E_BADPARAMCOUNT` classification, and stable `DISP_E_MEMBERNOTFOUND` classification, while the adapter boundary now also preserves stable `DISP_E_UNKNOWNNAME` classification for raw `GetIDsOfNames` failures,
