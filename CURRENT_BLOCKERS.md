...
- Current state:
  ...
  - controlled/native result conversion now also accepts `VT_I1`, `VT_I2`, `VT_I4`, `VT_I8`, `VT_INT`, `VT_UI1`, `VT_UI2`, `VT_UI4`, `VT_UI8`, and `VT_UINT` into the current integer-token lane when the payload fits the current `i32` carrier,
  - controlled/native result conversion now also accepts one-dimensional typed SAFEARRAY results with `VT_I1`, `VT_I2`, `VT_I4`, `VT_I8`, `VT_INT`, `VT_UI1`, `VT_UI2`, `VT_UI4`, `VT_UI8`, `VT_UINT`, `VT_BOOL`, and `VT_BSTR` element payloads into `RuntimeValue::ArrayIntent` when the integer payload fits the current `i32` carrier,
...
- Current state:
  ...
  - `oxvba-com` now also owns the extracted Windows `VARIANT`/one-dimensional `SAFEARRAY` translation bridge for the currently supported scalar/string/array subset, including typed `VT_I2`, `VT_I4`, `VT_I8`, `VT_UI2`, `VT_UI4`, `VT_UI8`, `VT_BOOL`, `VT_BSTR`, `VT_DISPATCH`, and `VT_UNKNOWN` SAFEARRAY result elements on the controlled `IDispatch`-exposing lane and checked scalar narrowing for the current `RuntimeValue::I32` carrier,
