...
- Current state:
  ...
  - but there is still no fully closed end-to-end model for `Set` vs `Let` intent parity in broader typed/object contexts beyond the now-covered plain-assignment, assignment-from-call, `Variant`-target object-result, and `Let`-rejects-object-call lanes, plus non-authoritative default-member call resolution and Office-style call-vs-value context parity.
- Exact unblock steps:
