...
- Current state:
  ...
  - controlled direct-error coverage now also proves VM/JIT host parity for real `arg_err` indexing on `DISP_E_TYPEMISMATCH`, bounded `EXCEPINFO` source/description/scode detail on `DISP_E_EXCEPTION`, stable `DISP_E_BADPARAMCOUNT` classification, stable `DISP_E_MEMBERNOTFOUND` classification, stable `DISP_E_UNKNOWNNAME` classification for runtime string member selectors, bounded runtime-string success for zero-arg method plus indexed property-get selectors via `DISP_E_BADPARAMCOUNT` flag fallback, bounded runtime-string named-argument success for method plus indexed property-get selectors, and bounded metadata-backed runtime-string property-put/property-putref plus indexed property-put/property-putref execution on the existing bound member path, while the adapter boundary also preserves the same stable `DISP_E_UNKNOWNNAME` classification for raw `GetIDsOfNames` failures,
