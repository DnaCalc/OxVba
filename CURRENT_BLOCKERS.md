...
- Current state:
  ...
  - native internal project-class `Property Get` / `Property Let` / `Property Set` now execute end to end through the shared PMR/dynamic-object substrate for explicit `DispatchInvoke(...)`, natural member syntax, natural indexed member syntax, and natural bare/indexed default-member `Get` / `Let` / `Set` syntax when authoritative default-member metadata exists,
