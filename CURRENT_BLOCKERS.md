...
- Current state:
  ...
  - oxvba-hal::standard no longer owns the high-level default-member/direct-DISPID/member-spec routing rules, and the event-side public `ComHal` surface is now typed on `ObjectHandle`/`ComMemberToken`/`ComSubscriptionToken`/`ComCallbackToken` end to end, but HAL still owns the last direct `IDispatch` invoke/result lifecycle glue and the final invoke-result/object-binding authority,
