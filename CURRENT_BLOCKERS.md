...
- Current state:
  ...
  - native internal project-class `Property Get` / `Property Let` / `Property Set` now execute end to end through the shared PMR/dynamic-object substrate for explicit `DispatchInvoke(...)`, natural member syntax, natural indexed member `Get` / `Let` / `Set` syntax, and natural bare/indexed default-member `Get` / `Let` / `Set` syntax when authoritative default-member metadata exists,
  - but there is still no fully closed end-to-end model for `Set` vs `Let` intent parity in broader typed/object contexts and Office-style call-vs-value context parity.
- Exact unblock steps:
